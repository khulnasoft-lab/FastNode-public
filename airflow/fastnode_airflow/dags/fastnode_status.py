...
import time
from fastnode_airflow.plugins.google import GoogleSheetsRangeOperator
...
KS_INDEX_PREFIX = 'fastnode_status'
...
def fastnode_status_convert_fn(docs, index_date_suffix, deployments):
    total_time = 0
    for i, doc in enumerate(docs):
        if i and i % 10000 == 0:
            logger.info('Done {} records, avg time / record={}'.format(i, total_time / i))
        start_time = time.perf_counter()
        if doc.get('event') != 'fastnode_status':
        ...
        if sum(doc['properties'].get('{}_events'.format(lang), 0) for lang in fastnode_status_config['languages']) == 0:
            total_time += (time.perf_counter() - start_time)
            continue

        index_name = '{}_active_{}'.format(KS_INDEX_PREFIX, index_date_suffix)

        doc = scrub(doc, fastnode_status_schema)
        ...
        # can be removed once the content of the PR https://github.com/khulnasoft-lab/fastnode/pull/10638/ has been released to
        # most of our users
...
fastnode_status_dag = DAG(
    'elastic_load_fastnode_status',
    description='Load fastnode_status to Kibana.',
    default_args={
        'retries': 1,
...
convert_fns = {'fastnode_status': fastnode_status_convert_fn, 'client_events': client_event_convert_fn}
...
for key, dag in [('fastnode_status', fastnode_status_dag), ('client_events', client_events_dag)]:
    list_ops = [
        S3ListOperator(
    ...
    list_ops >> ShortCircuitOperator(
        ...
        gcp_conn_id='google_cloud_fastnode_dev',
