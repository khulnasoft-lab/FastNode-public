...
DATA_LOC = 's3://fastnode-metrics/firehose/fastnode_status/'
PROD_RESULT_LOC_PREFIX = 's3://fastnode-metrics/athena-results'
...
for table_name in ['fastnode_status', 'fastnode_status_segment', 'fastnode_status_normalized']:
    schema_reload_ops.append(AWSAthenaOperator(
        ...
        database='fastnode_metrics',
        dag=dag,
        params={'schema': fastnode_status_schema, 'table_name': table_name}
    ))

insert_fastnode_status_normalized = AWSAthenaOperator(
    aws_conn_id='aws_us_east_1',
    task_id='insert_fastnode_status_normalized',
    query='athena/queries/fastnode_status_normalized.tmpl.sql',
    output_location='s3://fastnode-metrics-test/athena-results/ddl',
    database='fastnode_metrics',
    dag=dag,
...
cleanup_fastnode_status_normalized_table = AWSAthenaOperator(
    aws_conn_id='aws_us_east_1',
...
schema_reload_ops >> insert_fastnode_status_normalized >> cleanup_fastnode_status_normalized_table
...
def load_athena_to_elastic(task_instance, execution_date, **context):
    ...
    def iter():
        iter_records = read_s3_json_files('fastnode-metrics', task_instance.xcom_pull(task_ids='list_mixpanel_json_files'))
        for i, rec in enumerate(iter_records):
            try:
                if sum(rec.get('{}_events'.format(lang), 0) for lang in fastnode_status_config['languages']) == 0:
                    continue

                if rec['event'] != 'fastnode_status':
                    continue
                ...
                rec['timestamp'] = ts
                yield {'_index': 'fastnode_status_1d_{}'.format(execution_date.format('%Y%m')), '_id': rec_id, '_source': rec}
            except Exception:
...
event_names = {
    ...
    'anon_fastnode_status': 'anon_fastnode_status_1d',
    'fastnode_status': 'fastnode_status_1d',
}
...
def load_athena_to_mixpanel(task_instance, execution_date, dag_run, storage_task_name, **context):
    ...
    for i, rec in enumerate(iter_records):
        ...
        try:
            ...
            rec.update({
                'time': rec['end_time'],
                '_group': 'firehose/fastnode_status/{}/'.format(execution_date.strftime('%Y/%m/%d')),
                '_version': '1.0.0',
                '$insert_id': insert_id,
            })
            user_id = rec['userid']
...
def load_athena_to_cio(task_instance, execution_date, dag_run, storage_task_name, **context):
    ...
    start_row = task_instance.xcom_pull(task_ids=storage_task_name, key='progress')
    iter_records = read_s3_json_files('fastnode-metrics', task_instance.xcom_pull(task_ids='list_cio_json_files'))
...
for key, group_by, downstreams in [
    ('mixpanel', 'regexp_replace(fastnode_metrics.fastnode_status_normalized.userId, \'\p{Cntrl}\')', [(False, load_athena_to_elastic), (True, load_athena_to_mixpanel)]),
    ('cio', 'regexp_replace(coalesce(fastnode_metrics.fastnode_status_normalized.properties__forgetful_metrics_id, fastnode_metrics.fastnode_status_normalized.userId), \'\p{Cntrl}\')', [(True, load_athena_to_cio)])
]:
    operator = insert_fastnode_status_normalized >> AWSAthenaOperator(
        aws_conn_id='aws_us_east_1',
        task_id='insert_fastnode_status_1d_{}'.format(key),
        query='athena/queries/fastnode_status_1d.tmpl.sql',
        output_location='s3://fastnode-metrics-test/athena-results/ddl',
        database='fastnode_metrics',
        params={
            'key': key,
            'group_by': group_by,
            'languages': fastnode_status_config['languages'],
            'editors': fastnode_status_config['editors'],
            'lexical_providers': fastnode_status_config['lexical_providers'],
            'python_providers': fastnode_status_config['python_providers']
        },
        dag=dag,
    ) >> AWSAthenaOperator(
        aws_conn_id='aws_us_east_1',
        task_id='generate_{}_json'.format(key),
        query='athena/queries/fastnode_status_1d_json.tmpl.sql',
        output_location='s3://fastnode-metrics-test/athena-results/ddl',
        database='fastnode_metrics',
        params={'key': key, 'languages': fastnode_status_config['languages']},
        dag=dag,
    )
    operator >> AWSAthenaOperator(
        aws_conn_id='aws_us_east_1',
        task_id='cleanup_{}_table_json'.format(key),
        query='DROP TABLE fastnode_status_1d_{{params.key}}_{{ds_nodash}}_json',
        output_location='s3://fastnode-metrics-test/athena-results/ddl',
        database='fastnode_metrics',
        params={'key': key},
        dag=dag,
    ...
    operator >> AWSAthenaOperator(
        ...
        query='DROP TABLE fastnode_status_1d_{{params.key}}_{{ds_nodash}}',
        output_location='s3://fastnode-metrics-test/athena-results/ddl',
        database='fastnode_metrics',
        params={'key': key},
        dag=dag,
    ...
    operator = operator >> S3ListOperator(
        ...
        prefix='athena/fastnode_status_1d_{{params.key}}/json/{{ds}}/',
        delimiter='/',
        params={'key': key},
        dag=dag,
    )
...
update_schema_dag = DAG(
    'update_fastnode_status_schema',
    default_args=default_args,
    description='Update the fastnode_status and fastnode_status_normalized schemas.',
