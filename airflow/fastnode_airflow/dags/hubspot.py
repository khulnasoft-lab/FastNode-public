...
from fastnode_airflow.dags.fastnode_status_1d import dag as kits_status_1d_dag, read_s3_json_files
...
LANGS = fastnode_status_config['languages']
EDITORS = fastnode_status_config['editors']

contat_props_tmpl = Template(pkg_resources.resource_string('fastnode_airflow', 'files/hubspot_contactprops.yaml').decode('utf8'))
...
fastnode_status_dag_run_sensor = ExternalTaskSensor(
    task_id='fastnode_status_dag_run_sensor',
    dag=dag,
...
drop_intermediate_table = AWSAthenaOperator(
    ...
    query='DROP TABLE fastnode_metrics.hubspot_intermediate',
...
(previous_dag_run_sensor, fastnode_status_dag_run_sensor) >> drop_intermediate_table >> create_intermediate_table
...
(previous_dag_run_sensor, fastnode_status_dag_run_sensor) >> insert_deltas
...
def write_contact_prop_data(ti, **context):
    ...
    for file in sorted(ti.xcom_pull(task_ids='list_hubspot_json_files')):
        obj = s3.get_key(file, 'fastnode-metrics')
        for line in gzip.open(obj.get()['Body']):
            ...
            hs_props['fastnode_lifecycle_stages'] = 'User'  # This property is called 'Source' in HS
...
def copy_fastnode_users():
    ...
    s3.load_file(tf.name, 'enrichment/fastnode/users/users.csv', bucket_name='fastnode-metrics', replace=True)


copy_fastnode_users_operator = PythonOperator(
    python_callable=copy_fastnode_users,
    task_id=copy_fastnode_users.__name__,
    dag=dag,
)
...
(copy_fastnode_users_operator, setup_partitions) >> AWSAthenaOperator(
    aws_conn_id='aws_us_east_1',
