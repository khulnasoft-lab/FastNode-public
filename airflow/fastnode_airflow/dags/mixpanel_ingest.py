...
people_schema = yaml.load(pkg_resources.resource_stream('fastnode_airflow', 'files/mixpanel_people.schema.yaml'), Loader=yaml.FullLoader)
...
def copy_profile_deltas(task_instance, execution_date, prev_execution_date_success, next_execution_date, **context):
    ...
    s3.load_bytes(gz_file.getvalue(), key, 'fastnode-metrics')
...
SET LOCATION 's3://fastnode-metrics/mixpanel/people/rollups/year={{execution_date.year}}/month={{execution_date.month}}/day={{execution_date.day}}/'""",
    output_location='s3://fastnode-metrics-test/athena-results/ddl',
    database='fastnode_metrics',
    dag=dag,
    params={'schema': people_schema},
