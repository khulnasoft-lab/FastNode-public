...
def copy_mp_raw_events(task_instance, execution_date, **context):
    ...
    script = '''function main() {{
        return Events({{from_date: "{date}", to_date: "{date}"}}).filter(function(event) {{
            return !event.name.startsWith("fastnode_status") && event.time >= {start} && event.time < {end};
        }});
    }}'''.format(
    ...
    for prefix, (b_io, gz_file) in files.items():
        gz_file.close()
        s3.load_bytes(b_io.getvalue(), 'mixpanel/events/raw/{}/events.json.gz'.format(prefix), 'fastnode-metrics', replace=True)
