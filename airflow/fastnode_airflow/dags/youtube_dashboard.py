...
from fastnode_airflow.common import utils as common_utils
from fastnode_airflow.youtube_dashboard import api
from fastnode_airflow.youtube_dashboard import files
from fastnode_airflow.youtube_dashboard import utils
from fastnode_airflow.slack_alerts import task_fail_slack_alert


BUCKET = 'fastnode-youtube-data' if common_utils.is_production() else 'fastnode-metrics-test'
SCRATCH_SPACE_LOC = 's3://{}/athena-scratch-space/'.format(BUCKET)

DATABASE = 'prod_fastnode_link_stats_youtube' if common_utils.is_production() else 'fastnode_link_stats_youtube'
TABLE_CHANNELS = {
    'name': 'fastnode_link_stats_youtube_channels',
    'data_location': 's3://{}/youtube-dashboard/channels/'.format(BUCKET),
}
TABLE_VIDEOS = {
    'name': 'fastnode_link_stats_youtube_videos',
