...
WITH coding_stats AS (
  ...
  WHERE
    ...
    AND event = 'fastnode_status'
    AND regexp_like(fastnode_status_normalized.userid, '\\p{Cc}') = FALSE -- filter user id's that contains null
    AND regexp_replace(fastnode_status_normalized.userid, '\x{00}') != '' -- filter user id's were only null bytes
  GROUP BY
    1,
    2
)
