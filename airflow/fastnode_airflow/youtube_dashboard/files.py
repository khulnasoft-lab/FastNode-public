...
BUCKET = 'fastnode-youtube-data' if utils.is_production() else 'fastnode-metrics-test'
...
def get_cached_urls_from_file():
    ...
    for cached_url in cached_urls_list:
        cached_urls_dict[cached_url['url']] = bool(cached_url['is_a_fastnode_redirect'])
...
def write_cached_urls_on_file(cached_urls_dict):
    cached_urls_list = []
    for url, is_fastnode_redirect in cached_urls_dict.items():
        cached_urls_list.append(
            {
                'url': url,
                'is_a_fastnode_redirect': 'True' if is_fastnode_redirect else '' # empty string represents false
            }
        )
