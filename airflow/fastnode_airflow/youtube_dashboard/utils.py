...
def is_link_present_in_description(video_item, cached_urls_dict):
    '''
    Looks for fastnode link in the description and in case of shorten url also update the cache
    ...
    Returns:\n
        boolean:
            indicates if fastnode link was present
    '''

    fastnode_url = 'khulnasoft.com'
    description = get_description_of_video_item(video_item)

    # youtubers always uses word Fastnode in description so if it's not present
    # then no further search is needed
    if 'fastnode' not in description.lower():
        return False

    if fastnode_url in description:
        return True

    # some youtubers uses link shortener so for those we uses a combination of cache
    # and HEAD requests to look if fastnode redirects are present
    ...
    for url in unique_urls:
        if url in cached_urls_dict:
            if cached_urls_dict[url]:
                ...
                continue # not returning False because Fastnode link can be added after we have took the snapshot

        try:
            response = requests.head(url)
            location_header = response.headers.get('Location')
            is_a_fastnode_redirect = location_header and fastnode_url in location_header;
            cached_urls_dict[url] = 'True' if is_a_fastnode_redirect else ''; # empty string represents false

            if is_a_fastnode_redirect:
                return True
