...
@task
def initialize():
    host = "vpn.khulnasoft.com"
    if env.host == _azure_vpn_ip:
        host = "vpn-azure.khulnasoft.com"
...
def get_client_config(username):
    ...
    run("sed -n '/redirect-gateway/!p' %s.ovpn > %s-fastnode-vpn-%s.ovpn" % (username, username, provider))
    get(remote_path=("%s-fastnode-vpn-%s.ovpn" % (username, provider)))
    run("rm *.ovpn")
