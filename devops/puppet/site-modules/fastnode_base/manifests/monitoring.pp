class fastnode_base::monitoring {
  ...
  systemd::dropin_file { 'metricbeat_keystore.conf':
    ...
    content => file('fastnode_base/metricbeat_keystore.conf'),
  ...
  class { 'filebeat':
      ...
      manage_repo          => false,
      outputs              => {'elasticsearch' => {}},
      conf_template        => 'fastnode_base/etc/filebeat/filebeat.conf.erb',
      require              => [Apt::Source['elastic'], Class['apt::update']],
    }

  systemd::dropin_file { 'filebeat_keystore.conf':
    unit    => 'filebeat.service',
    content => file('fastnode_base/filebeat_keystore.conf'),
