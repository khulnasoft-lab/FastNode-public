terraform {
  backend "s3" {
    ...
    workspace_key_prefix = "deployments/fastnodeserver"
...
data "google_compute_network" "fastnode_prod" {
  name = "fastnode-prod"
}
...
module "service" {
  ...
  name                      = var.service_name
  network                   = data.google_compute_network.fastnode_prod.name
  ...
  fastnode_base_image           = "nvidia-docker-puppet-1602655040"
