...
data "aws_vpc" "prod" {
  filter {
    name   = "tag:Name"
    values = ["fastnode-prod"]
  }
}

data "aws_ami" "fastnode_base" {
  most_recent = true
  name_regex  = "^fastnode_base_bionic_[0-9]+$"
  owners      = ["self"]
}
...
data "aws_acm_certificate" "star_fastnode_com" {
...
resource "aws_alb_listener" "prod-alb-https-listener" {
  ...
  certificate_arn   = data.aws_acm_certificate.star_fastnode_com.arn
...
resource "aws_launch_template" "collector" {
  ...
  image_id      = data.aws_ami.fastnode_base.id
