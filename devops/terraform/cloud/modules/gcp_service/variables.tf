...
variable "fastnode_base_image" {
  type    = string
  default = "fastnode-base-1589586113"
}
...
// gcloud compute --project fastnode-prod-XXXXXXX --format json accelerator-types list | jq '.[] | .zone = (.zone | split("/"))[-1] | {"zone": .zone, "region": .zone[:-2], "name": .name}' | jq -sc '[group_by(.region)[] | {"key": .[0].region, "value": [group_by(.name)[] | {"key": .[0].name, "value": [.[] | .zone]}] | from_entries}] | from_entries'
variable "gpu_zones" {
