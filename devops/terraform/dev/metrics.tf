...
resource "aws_s3_bucket_notification" "fastnode-metrics" {
  provider = aws.east
  ...
  lambda_function {
    ...
    events              = ["s3:ObjectCreated:*"]
    filter_prefix       = "firehose/fastnode_status/"
  }
  lambda_function {
...
resource "aws_s3_bucket_notification" "fastnode-offline-metrics" {
  bucket = "fastnode-offline-metrics"
  lambda_function {
...
resource "aws_s3_bucket_notification" "fastnode-segment-backend-http-requests" {
