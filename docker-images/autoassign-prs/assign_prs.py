...
def main():
	...
	for pr in prs:
		...
		github.put("/repos/khulnasoft-lab/khulnasoft-lab/issues/%d/labels" % pr["number"], [label])

		if len(patch) > 0:
			print(patch)
			github.patch("/repos/khulnasoft-lab/khulnasoft-lab/issues/%d" % pr["number"], patch)
