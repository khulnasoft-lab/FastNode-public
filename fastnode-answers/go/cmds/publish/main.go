...
const postsPath = "/fastnode-answers/answers/posts"
...
func main() {
	...
	for path, raw := range raws {
		...
		rendered, err := render.Render(fastnodectx.TODO(), sandbox, rm, raw)
		if err != nil {
	...
	log.Printf("Done! Wrote index to %s.\nPlease update the S3 path in fastnode-go/lang/python/answers/index.go accordingly.", indexName)
}
