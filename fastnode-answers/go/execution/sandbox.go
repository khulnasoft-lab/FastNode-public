...
const (
	cacheSize        = 100
	dockerImage      = "khulnasoft-lab/answers-sandbox"
	containerWorkdir = "/fastnode/run/" // see sandbox/Dockerfile
)
...
func NewManager(ctx fastnodectx.Context) Manager {
...
func (m Manager) Run(ctx fastnodectx.Context, spec Spec, code []byte) ([]Block, error) {
	...
	err = ctx.WithTimeout(time.Duration(spec.Timeout)*time.Millisecond, func(ctx fastnodectx.Context) error {
		// docker cp
