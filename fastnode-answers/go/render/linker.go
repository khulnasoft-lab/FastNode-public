...
/*
 Syntax for links to Fastnode docs. We capture the link text in [] as well as the symbol path.
 [`baz`](fastnode-sym:foo.bar.baz)
*/
var linkRE = regexp.MustCompile("(?i)\\[([^\\[\\]]+?)\\]\\(fastnode-sym:([\\w\\d\\.]+?)\\)")
...
// Validates candidate string and other combination's in Fastnode's Python resource manager.
...
// Constructs Fastnode Doc link given path and text
func linkify(path string, text string) []byte {
	url := fmt.Sprintf("https://khulnasoft.com/python/docs/%s", path)
