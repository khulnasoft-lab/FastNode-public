...
RUN_DIR = os.path.join(KHULNASOFT-LAB_ROOT, "local-pipelines/python-ggnn-expr-completion")
OUT = f"/data/expr-ab-tests/{TEST_NAME}/out"
TENSORBOARD = f"/data/expr-ab-tests/{TEST_NAME}/tensorboard"
RUNDB = f"s3://fastnode-data/run-db/users/damian/tests/{TEST_NAME}"
...
def start_graph_server(log_dir: str):
    ...
    run(f"""
go run github.com/khulnasoft-lab/fastnode/fastnode-go/lang/python/cmds/graph-data-server > {log_dir}/graph-data-server.log 2>&1 &
""")
