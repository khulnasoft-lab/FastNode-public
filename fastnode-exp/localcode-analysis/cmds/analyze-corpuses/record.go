...
import (
	...
	"github.com/khulnasoft-lab/fastnode/fastnode-exp/localcode-analysis/index"
...
func sourceFunctionParams(res resources, sf *pythontype.SourceFunction) []paramRecord {
	...
	for i, p := range sf.Parameters {
		...
		val := pythontype.Translate(fastnodectx.Background(), pythontype.WidenConstants(p.Symbol.Value), res.rm)
		if val == nil {
			prs = append(prs, pr)
			continue
		}
...
func getSourceFunction(rm pythonresource.Manager, val pythontype.Value) (*pythontype.SourceFunction, error) {
	origVal := val
	val = pythontype.Translate(fastnodectx.Background(), val, rm)
	if val == nil {
