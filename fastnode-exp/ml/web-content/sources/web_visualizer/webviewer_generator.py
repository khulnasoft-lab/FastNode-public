...
AVERAGE_FASTNODE_CTR = 0.04597767391467217
# Obtained by doing total_clicks / total_imps over the 5k top queries bringing the most clicks to fastnode examples
...
class WebViewerGenerator(object):
    ...
    def prepare_dataset(self):
        ...
        self.predicted_click_so_posts.columns = ["URL", "fastnode_predicted_impressions"]
        self.pages_with_topic = self.pages_with_topic.merge(self.predicted_click_so_posts, 'left', on="URL")
        self.pages_with_topic["fastnode_predicted_clicks"] = self.pages_with_topic.fastnode_predicted_impressions * AVERAGE_FASTNODE_CTR
        ...
        def get_topic_features(t):
            ...
            fastnode_pages = t[t.page_type == 'fastnode_examples']
            so_pages = t[t.page_type == 'so'].sort_values("clicks", ascending=False)
            fastnode_volume = fastnode_pages.volume.sum()
            so_volume = so_pages.volume.sum()
            fastnode_clicks = fastnode_pages.clicks.sum()
            so_clicks = so_pages.clicks.sum()
            fastnode_predicted_clicks = so_pages.fastnode_predicted_clicks.sum()
            # fastnode_predicted_impressions = so_pages.fastnode_predicted_impressions.sum()
            # clicks_ratio = fastnode_clicks / so_clicks if so_clicks > 0 else 1
            # volume_ratio = fastnode_volume / so_volume if so_volume > 0 else 1
            so_rank = (so_pages['rank']*so_pages.volume).sum() / so_volume if so_volume > 0 else 0
            return pd.Series(dict(
                # fastnode_volume=fastnode_volume,
                # fastnode_predicted_volume=fastnode_predicted_impressions,
                # so_volume=so_volume,
                fastnode_clicks=fastnode_clicks,
                fastnode_predicted_clicks=int(fastnode_predicted_clicks),
                so_clicks=so_clicks,
                # clicks_ratio=clicks_ratio,
                # volume_ratio=volume_ratio,
                so_rank=round(so_rank, 4),
                fastnode_urls=format_url_list(fastnode_pages.URL.tolist()),
        ...
        # topics["available_clicks"] = topics.fastnode_predicted_clicks - topics.fastnode_clicks
        return topics

    def generate_topics_viewer(self):
        columns_and_size = [# ('Fastnode vol', 60),
                            # ('Fastnode pred vol', 60),
                            # ('SO volume', 60),
                            ('Fastnode clicks', 60),
                            ('Fastnode pred clicks', 80),
                            ('SO Views', 80),
                            # ('Click ratio', 80),
                            # ('Vol ratio', 80),
                            ('SO rank', 60),
    ...
    def prepare_so_pages_dataset(self):
        ...
        pages.fastnode_predicted_clicks = pages.fastnode_predicted_clicks.astype(np.int64)
        pages = pages[["clicks", "fastnode_predicted_clicks", "rank", "URL", "dominant_topic"]]
