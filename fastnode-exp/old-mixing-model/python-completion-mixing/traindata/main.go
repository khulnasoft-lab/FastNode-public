...
import (
	...
	"github.com/khulnasoft-lab/fastnode/fastnode-go/lang/python/pythonmixing"
...
const (
	s3Region    = "us-west-1"
	datasetPath = "s3://fastnode-emr/users/juan/python-dedupe-code/2018-07-26_13-53-43-PM/dedupe/output/"
)
...
func getState(src []byte, rm pythonresource.Manager, r *rand.Rand) (State, error) {
	...
	ast, err := pythonparser.ParseWords(fastnodectx.Background(), src, words, parseOpts)
	if err != nil {
	...
	ast2, err := pythonparser.ParseWords(fastnodectx.Background(), src, words2, withCursor)
	if ast2 == nil {
		return State{}, fmt.Errorf("unable to parse file: %v", err)
	}
...
func getCompFeatures(in State, cr compResources, mixIP []pythoncompletions.MixInput) []pythonmixing.CompFeatures {
	...
	for _, mip := range mixIP {
		...
		case pythoncompletions.MultiTokenCall:
			...
			score, _ := cb.CallProbs(fastnodectx.Background(), []pythoncompletions.Completion{comp})
			if len(score) == 0 {
				softmaxScore = comp.Score
			} else {
