...
var (
	...
	bashPresentationAPI = MustAsset("presentation_api/bash/fastnode.sh")

	// Python presentation API sources
	pythonPresentationAPI        = MustAsset("presentation_api/python/fastnode.py")
...
// parsePythonExample parses a python code example written with fastnode annotations
func parsePythonExample(example string) (*Stencil, error) {
	code := "##from fastnode import fastnode\n" + example
...
func parseBashExample(code string) (*Stencil, error) {
	...
	for lineNum := 0; lineNum < len(lines); lineNum++ {
		...
		if strings.HasPrefix(line, "##") {
			...
			run = append(run, fmt.Sprintf("fastnode_line %d", len(run)+len(continuedLines)))
			...
			run = append(run, fmt.Sprintf("fastnode_line %d", len(run)+len(blockLines)))
			run = append(run, blockLines...)
		} else {
			lineMap = append(lineMap, len(run))
			present = append(present, line)
			run = append(run, fmt.Sprintf("fastnode_line %d", len(run)+1))
			run = append(run, line)
		}
	}
