...
class fastnode(object):
	...
	@classmethod
	def show(cls, **kwargs):
		sys.stdout.write("[[FASTNODE[[SHOW %s]]FASTNODE]]\n" % json.dumps(kwargs))

	@classmethod
	def show_plaintext_str(cls, value, expression=""):
		fastnode.show(type="plaintext", expression=expression, value=str(value))

	@classmethod
	def show_plaintext(cls, value, expression=""):
		fastnode.show_plaintext_str(repr(value), expression)
	...
	def dir_entry_data(cls, path, cols=[]):
		...
		return {
			...
			"permissions": fastnode.format_permissions(info.st_mode),
			"modified": int(info.st_mtime),
			"created": int(info.st_ctime),
			"accessed": int(info.st_atime),
			"ownerid": int(info.st_uid),
			"owner": fastnode.owner_name(info.st_uid),
			"groupid": int(info.st_gid),
			"group": fastnode.group_name(info.st_gid)
		}
	...
	def show_dir_table(cls, path=".", caption=None, cols=None):
		...
		for entry in contents:
			data = {"name": entry}
			data.update(fastnode.dir_entry_data(os.path.join(path, entry), cols))
			directory.append(data)

		fastnode.show(type="dirtable", path=path, caption=caption, cols=cols, entries=directory)

	@classmethod
	def show_dir_tree(cls, path=".", caption=None, skip=None):
		fastnode.show(type="dirtree", path=path, caption=caption, entries=fastnode.path_flat(path, skip))
	...
	def show_image_path(cls, path, caption, encoding=None, data=None):
		...
		fastnode.show(type="image", path=path, caption=caption, encoding=encoding, data=data)
	...
	def show_file(cls, path, caption, data=None):
		...
		if len(data) > 10000:
			data = data[:10000]
		fastnode.show(type="file", path=path, caption=caption, data=base64.b64encode(data))

	@classmethod
	def show_region_delimiter(cls, region):
		fastnode.show(type="region", region=region)
