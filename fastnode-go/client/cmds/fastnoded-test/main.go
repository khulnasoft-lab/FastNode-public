...
func fatal(err error) {
	switch err {
	// ErrRunning occurs when fastnode's port was already in use
	// but fastnode was manually launched. In this case, we just launch
...
func main() {
	...
	commandLine.IntVar(&port, "port", 56624, "The port where fastnoded will be listenening")
	commandLine.StringVar(&rootDir, "root-dir", "", "The directory where fastnoded stores its configuration data. The equivalent of ~/.fastnode. Defaults to a new temporary directory.")
	commandLine.StringVar(&indexedDir, "index-dir", "", "The directory indexed by fastnoded. Defaults to a new temporary directory.")
	...
	if rootDir == "" {
		rootDir = newTempDir("fastnode-testing-root")
	}

	if indexedDir == "" {
		indexedDir = newTempDir("fastnode-testing-indexed")
	}
	...
	if dummyUser {
		...
		buf, err := json.Marshal(&community.User{
			ID:            -1,
			Name:          "Dummy User",
			Email:         "fastnode-int@khulnasoft.com",
			EmailVerified: true,
		})
		if err != nil {
		...
		buf, err = json.Marshal(&[]http.Cookie{
			{Name: "fastnode-session", Value: "value", Path: "", Domain: ""},
		})
		if err != nil {
