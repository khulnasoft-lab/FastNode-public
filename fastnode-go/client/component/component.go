...
// FastnodedInitialized delegates the call to all components which implement the interface FastnodedEventer
func (m *Manager) FastnodedInitialized() {
	m.components.Range(func(name, component interface{}) bool {
		if eventer, ok := component.(FastnodedEventer); ok {
			func() {
				defer m.panicRecovery("FastnodedInitialized", component.(Core))
				eventer.FastnodedInitialized()
			}()
		}
		return true
	})
...
// FastnodedUninitialized delegates the call to all components which implement the interface FastnodedEventer
func (m *Manager) FastnodedUninitialized() {
	m.components.Range(func(name, component interface{}) bool {
		if eventer, ok := component.(FastnodedEventer); ok {
			func() {
				defer m.panicRecovery("FastnodedUninitialized", component.(Core))
				eventer.FastnodedUninitialized()
			}()
		}
		return true
	})
