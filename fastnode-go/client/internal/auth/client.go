...
import (
	...
	"github.com/khulnasoft-lab/fastnode/fastnode-go/client/token"
...
func (c *Client) Initialize(opts component.InitializerOptions) {
	...
	c.filepath = filepath.Join(opts.Platform.FastnodeRoot, "session.json")
	c.licenseFilepath = filepath.Join(opts.Platform.FastnodeRoot, "license.json")
	c.machineID = opts.Platform.MachineID
	c.debug = opts.Platform.DevMode

	c.userCacheFile = filepath.Join(opts.Platform.FastnodeRoot, "user")
	...
	fastnodectx.Go(func() error {
		c.SettingUpdated(settings.FastnodeServer, fastnodeServerHost)
		return nil
	})
}
...
func (c *Client) SettingUpdated(key, value string) {
	if key == settings.FastnodeServer {
		_, _, err := fastnodeserver.GetHealth(value)
		c.mu.Lock()
		defer c.mu.Unlock()
		c.licenseStore.FastnodeServer = err == nil
	}
}
...
func (c *Client) SettingDeleted(key string) {
	if key == settings.FastnodeServer {
		c.mu.Lock()
		defer c.mu.Unlock()
		c.licenseStore.FastnodeServer = false
	}
}
...
func (c *Client) saveAuth() (err error) {
	...
	var fastnodeCookies []*http.Cookie
	if c.client != nil && c.client.Jar != nil {
		cookies := c.client.Jar.Cookies(c.target)
		for _, cookie := range cookies {
			if cookie.Name == sessionKey {
				fastnodeCookies = append(fastnodeCookies, cookie)
			}
		}
	}

	buf, err := json.Marshal(fastnodeCookies)
	if err != nil {
