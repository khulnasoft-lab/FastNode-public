...
func Test_TokenUpdate(t *testing.T) {
	...
	s.Backend.AddPrefixRequestHandler("/api/hmac1", []string{"GET"}, func(w http.ResponseWriter, r *http.Request) {
		w.Header().Set("Fastnode-Token", "fastnode-token-value")
		w.Header().Set("Fastnode-TokenData", "fastnode-token-data-value")
		w.WriteHeader(http.StatusOK)
	})

	s.Backend.AddPrefixRequestHandler("/api/hmac2", []string{"GET"}, func(w http.ResponseWriter, r *http.Request) {
		w.Header().Set("Fastnode-Token", "fastnode-token-value2")
		w.Header().Set("Fastnode-TokenData", "fastnode-token-data-value2")
		w.WriteHeader(http.StatusOK)
	})
	...
	assert.NoError(t, err)
	assert.Equal(t, "fastnode-token-value", req.Header.Get("Fastnode-Token"))
	assert.Equal(t, "fastnode-token-data-value", req.Header.Get("Fastnode-TokenData"))
	...
	req, err = s.AuthClient.NewRequest("GET", "/api/test", "text/plain", nil)
	assert.NoError(t, err)
	assert.Equal(t, "fastnode-token-value2", req.Header.Get("Fastnode-Token"))
	assert.Equal(t, "fastnode-token-data-value2", req.Header.Get("Fastnode-TokenData"))
}
...
func Test_ServerHTTPCookies(t *testing.T) {
	...
	s.Backend.AddPrefixRequestHandler("/cookie-request", []string{"GET"}, func(w http.ResponseWriter, r *http.Request) {
		if c, err := r.Cookie("fastnode-cookie"); err == nil && c.Value == "fastnode-cookie-value" {
			w.WriteHeader(http.StatusOK)
		} else {
			w.WriteHeader(http.StatusInternalServerError)
		}
	...
	auth.httpClient().Jar.SetCookies(auth.Target(), []*http.Cookie{{Name: "fastnode-cookie", Value: "fastnode-cookie-value"}})
...
func Test_DoRequest(t *testing.T) {
	...
	s.Backend.AddPrefixRequestHandler("/client-request", []string{"GET"}, func(w http.ResponseWriter, r *http.Request) {
		machine := r.Header.Get("Fastnode-Machine")
		token := r.Header.Get("Fastnode-Token")
		tokenData := r.Header.Get("Fastnode-Tokendata")
	...
	// Do() must send the headers Fastnode-Token and Fastnode-TokenData
	// It must not send the Fastnode-Machine header (this is how the initial implementation was)
	headers := map[string][]string{"Fastnode-Token": {"token-value"}, "Fastnode-Tokendata": {"token-data-value"}}
