...
// tests the AuthClient's login / logout requests using a mocked http backend and a mocked http fastnoded client
func Test_Client(t *testing.T) {
	...
	// requests /clientapi/login from Fastnoded, which then sends a login request to the backend mock server
	...
	// requests /clientapi/logout from Fastnoded, which then sends a logout request to the backend mock server
...
func Test_ComponentGoTick(t *testing.T) {
	...
	_, err = s.DoFastnodedGet("/api/account/authenticated")
	require.NoError(t, err)
