...
func Test_FetchUser(t *testing.T) {
	...
	s.Backend.AddPrefixRequestHandler("/cookie-request", []string{"GET"}, func(w http.ResponseWriter, r *http.Request) {
		...
		http.SetCookie(w, &http.Cookie{Name: sessionKey, Value: "fastnode-cookie-value"})
		w.WriteHeader(200)
	})
...
// newTestServer returns a new test server with fastnode local enabled by the fastnodeLocal parameter value
