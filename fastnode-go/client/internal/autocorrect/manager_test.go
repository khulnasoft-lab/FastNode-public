...
func Test_OnSave(t *testing.T) {
	...
	resp, err := s.DoFastnodedPost("/clientapi/editor/autocorrect/validation/on-save", data)
	assert.NoError(t, err)

	assert.EqualValues(t, http.StatusOK, resp.StatusCode, "Response: "+readResponse(resp))

	//bad json
	resp, err = s.DoFastnodedPost("/clientapi/editor/autocorrect/validation/on-save", strings.NewReader("not-json"))
	assert.NoError(t, err)
...
func Test_Autocorrect(t *testing.T) {
	...
	//atom, whitelisted
	resp, err := s.DoFastnodedPost("/clientapi/editor/autocorrect", atomWhitelisted)
	assert.NoError(t, err)
	assert.EqualValues(t, http.StatusOK, resp.StatusCode)

	//invalid JSON
	resp, err = s.DoFastnodedPost("/clientapi/editor/autocorrect", strings.NewReader("not-json"))
	assert.NoError(t, err)
...
func Test_AutocorrectSupportedEditors(t *testing.T) {
	...
	resp, err := s.DoFastnodedPost("/clientapi/editor/autocorrect", atom)
	assert.NoError(t, err)
	assert.EqualValues(t, http.StatusOK, resp.StatusCode)

	// vscode, enabled
	vscode := requestJSON(s.GetFilePath("test.py"), "content", "vscode")
	resp, err = s.DoFastnodedPost("/clientapi/editor/autocorrect", vscode)
	assert.NoError(t, err)
	assert.EqualValues(t, http.StatusOK, resp.StatusCode)

	// vim, disabled
	vim := requestJSON(s.GetFilePath("test.py"), "content", "vim")
	resp, err = s.DoFastnodedPost("/clientapi/editor/autocorrect", vim)
	assert.NoError(t, err)
	...
	resp, err = s.DoFastnodedPost("/clientapi/editor/autocorrect", intellij)
	assert.NoError(t, err)
	assert.EqualValues(t, http.StatusNotImplemented, resp.StatusCode)

	// sublime3, disabled
	sublime3 := requestJSON(s.GetFilePath("test.py"), "content", "sublime3")
	resp, err = s.DoFastnodedPost("/clientapi/editor/autocorrect", sublime3)
	assert.NoError(t, err)
	assert.EqualValues(t, http.StatusNotImplemented, resp.StatusCode)
}
...
func Test_AutocorrectDebugEnabled(t *testing.T) {
	...
	//other editor
	resp, err = s.DoFastnodedPost("/clientapi/editor/autocorrect", otherWhitelisted)
	assert.NoError(t, err)
...
func testClientProxy(t *testing.T, clientURL, backendURL string) {
	...
	//post JSON
	resp, err := s.DoFastnodedPost(clientURL, strings.NewReader("{}"))
	assert.NoError(t, err)
	assert.EqualValues(t, http.StatusOK, resp.StatusCode)

	//post non-JSON and make sure that status is proxied
	resp, err = s.DoFastnodedPost(clientURL, strings.NewReader("not-json"))
	assert.NoError(t, err)
