...
import (
	...
	local_permissions "github.com/khulnasoft-lab/fastnode/fastnode-go/client/internal/fastnodelocal/permissions"
	...
	"github.com/khulnasoft-lab/fastnode/fastnode-go/client/internal/network"
	"github.com/khulnasoft-lab/fastnode/fastnode-go/client/internal/settings"
	"github.com/khulnasoft-lab/fastnode/fastnode-go/client/internal/statusicon"
...
// PreviousLogsSuffix is the suffix for logs that are from previous runs of fastnoded
...
type Options struct {
	...
	// init options to pass to the fastnode local component
...
type Client struct {
	...
	fastnodelocal *fastnodelocal.Manager
...
func NewClient(opts Options) (*Client, error) {
	...
	// currently env FASTNODED_PYTHON_REMOTE="host:port" controls this setting
	if pythonRemoveEnv, ok := os.LookupEnv("FASTNODED_PYTHON_REMOTE"); ok {
	...
	klocal, err := fastnodelocal.NewManager(componentMgr, opts.LocalOpts)
	if err != nil {
		return nil, errors.Errorf("error building fastnodelocal manager: %s", err)
	}
...
func (c *Client) Initialize(router *mux.Router) {
	...
	if _, headless := os.LookupEnv("FASTNODE_HEADLESS"); !headless && !c.Platform.IsUnitTestMode {
