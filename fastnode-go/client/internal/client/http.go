...
func (c *Client) processHTTP(ctx context.Context, target string) error {
	...
	default:
		...
		if created, ok := c.Settings.GetBool("iid_registered"); !ok || !created {
			...
			clienttelemetry.InstallIDs.Event("fastnode_install_started", props)
	...
	c.components.FastnodedInitialized()
	defer func() {
		c.components.FastnodedUninitialized()
	}()
	...
	for {
		...
		case r := <-c.fastnodelocal.Responses:
