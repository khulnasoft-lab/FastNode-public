...
import (
	...
	"github.com/khulnasoft-lab/fastnode/fastnode-go/client/autostart"
	...
	"github.com/khulnasoft-lab/fastnode/fastnode-go/client/internal/activefile"
	"github.com/khulnasoft-lab/fastnode/fastnode-go/client/internal/autosearch"
	"github.com/khulnasoft-lab/fastnode/fastnode-go/client/internal/capture"
	...
	"github.com/khulnasoft-lab/fastnode/fastnode-go/client/internal/cpuinfo"
	"github.com/khulnasoft-lab/fastnode/fastnode-go/client/internal/debug"
	"github.com/khulnasoft-lab/fastnode/fastnode-go/client/internal/desktoplogin"
	"github.com/khulnasoft-lab/fastnode/fastnode-go/client/internal/health"
	...
	"github.com/khulnasoft-lab/fastnode/fastnode-go/client/internal/notifications"
	plugins "github.com/khulnasoft-lab/fastnode/fastnode-go/client/internal/plugins_new"
	"github.com/khulnasoft-lab/fastnode/fastnode-go/client/internal/plugins_new/system"
	"github.com/khulnasoft-lab/fastnode/fastnode-go/client/internal/proxy"
	"github.com/khulnasoft-lab/fastnode/fastnode-go/client/internal/remotecontent"
	"github.com/khulnasoft-lab/fastnode/fastnode-go/client/internal/settings"
	sidebarComp "github.com/khulnasoft-lab/fastnode/fastnode-go/client/internal/sidebar"
	"github.com/khulnasoft-lab/fastnode/fastnode-go/client/internal/status"
	"github.com/khulnasoft-lab/fastnode/fastnode-go/client/internal/systeminfo"
	...
	"github.com/khulnasoft-lab/fastnode/fastnode-go/client/startup"
	...
	"github.com/khulnasoft-lab/fastnode/fastnode-go/web/midware"
	"github.com/khulnasoft-lab/fastnode/fastnode-golib/applesilicon"
...
var (
	// ErrPortInUse is the error returned when Fastnode's HTTP port is in use.
	ErrPortInUse = errors.New("http port already in use")

	// ErrRunning is the error returned when fastnoded was already running
	// but we simply want to launch the sidebar and exit quietly.
	ErrRunning = errors.New("already running")

	// ErrAutostartDisabled is the error returned when fastnoded detected that it
	// was started at login/boot, but autostart was disabled
	ErrAutostartDisabled = errors.New("autostart is disabled")
)

const (
	// This is the port the fastnoded HTTP server listens on. Only listen on the
	// local interface because exposing fastnoded to the outside world would be a
...
// StartPort creates a fastnode application and returns the client.
func StartPort(ctx context.Context, port int, testCaseEnv bool, customOpts *client.Options, additionalComponents ...component.Core) (*client.Client, *http.Server, error) {
	...
	settingsFile := filepath.Join(p.FastnodeRoot, "settings.json")
	...
	// on Linux, the fastnode-autostart service relies on fastnoded to shutdown in this scenario
	...
	// do this before we initialize to avoid disrupting an existing fastnoded instance
	...
	// Initialize the logger after we've determined that this is a unique instance of fastnoded
	err = p.InitializeLogger()
	if err != nil {
		return nil, nil, fmt.Errorf("error initializing logger: %v", err)
	}

	// Touch file indicating fastnoded has run
	hasRunFile := filepath.Join(p.FastnodeRoot, "fastnoded_has_run")
	...
	log.Println("root dir:", p.FastnodeRoot)
	...
	remoteContentFile := filepath.Join(p.FastnodeRoot, "remotecontent.json")
...
func (s *segmentTracker) flushLocked() {
	...
	clienttelemetry.FastnodeTelemetry("Client HTTP Batch", map[string]interface{}{
		"requests": paths,
	})
}
