...
func Test_MainLoop(t *testing.T) {
	...
	// in fastnodelocal.Manager. Keeping this to 50 events ensures we never drop responses and
	...
	p.FastnodedClient.SendLogoutRequest(true)
	p.FastnodedClient.SendLogoutRequest(true)
	assertNoUser(t, p)
	...
	_, err = p.FastnodedClient.SendLoginRequest("user@example.com", "secret", true)
	require.NoError(t, err)
	...
	for i := 0; i < 50; i++ {
		p.FastnodedClient.SendLoginRequest("user@example.com", "secret", false)
		p.FastnodedClient.SendLoginRequest("pro@example.com", "secret", false)
		p.FastnodedClient.SendLoginRequest("unknown@example.com", "secret", false)
	}

	// logout and make sure that user, plan, and event handling is good
	_, err = p.FastnodedClient.SendLogoutRequest(true)
	require.NoError(t, err)
...
func TestAccountOptionalLogin(t *testing.T) {
	...
	u, err := p.FastnodedClient.CurrentUser()
	require.NoError(t, err)
...
func sendEditorEvents(p *clientapp.TestEnvironment, file string, count int) {
	for i := 0; i < count; i++ {
		p.FastnodedClient.PostEditEvent("test_client", file, strings.Repeat("x", i), int64(i))
	}
}

func assertUserAndPlan(t *testing.T, p *clientapp.TestEnvironment) {
	user, err := p.Fastnoded.AuthClient.GetUser()
	require.NoError(t, err, "expected user")
	require.NotNil(t, user)

	plan, err := p.FastnodedClient.CurrentPlan()
