...
var (
	...
	// immutable values
	startTime       = time.Now()
	fastnodeTracker     = telemetry.NewCommonClient(telemetry.StreamClientEvents)
...
// set the client for only the fastnode telemetry source, and nil out the mixpanel source (since both
// FastnodeEvent and MixpanelEvent send to to segment source, and we want to avoid duplicates
...
func Close() {
	if err := fastnodeTracker.Close(); err != nil {
