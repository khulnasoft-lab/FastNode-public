...
func (m *Manager) handleStartTrial(w http.ResponseWriter, r *http.Request) {
	...
	if src := r.URL.Query().Get("cta-source"); src != "" {
		...
		// TODO FastnodePro check if the source is valid?
	}
...
// SendToDesktopLogin writes a HTTP redirect into the HTTP response which redirect to the given destination on the Fastnode server
func (m *Manager) SendToDesktopLogin(w http.ResponseWriter, r *http.Request, destination string) {
	...
	// and then forward request to desktop login which will then redirect to www.khulnasoft.com/trial
