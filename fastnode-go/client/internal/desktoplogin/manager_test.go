...
func Test_SuccessfulRedirect(t *testing.T) {
	...
	response, err := s.DoFastnodedGet("/clientapi/desktoplogin?d=/docs/")
	assert.NoError(t, err)
...
func Test_MissingTargetUrl(t *testing.T) {
	...
	response, err := s.DoFastnodedGet("/clientapi/desktoplogin")
	assert.NoError(t, err)
...
func Test_InvalidTargetUrl(t *testing.T) {
	...
	response, err := s.DoFastnodedGet("/clientapi/desktoplogin?d=%INVALID_URL_HERE%")
	assert.NoError(t, err)
	assert.Equal(t, http.StatusBadRequest, response.StatusCode)
}
