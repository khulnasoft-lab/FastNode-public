...
func (m *Manager) state(ctx fastnodectx.Context, filename, text string, editor data.Editor, cursor int) (*driver.State, error) {
	ctx.CheckAbort()
...
func (m *Manager) idccCompletions(ctx fastnodectx.Context, req data.APIRequest, metricFn data.EngineMetricsCallback) data.APIResponse {
...
// Not warming up python models for now https://github.com/khulnasoft-lab/fastnode/issues/11788
...
s = json.dumps()
	...
	err := fastnodectx.Background().WithTimeout(30*time.Second, func(ctx fastnodectx.Context) error {
