...
func NewLibraryManager(homeDir, fastnodeDir string, walkLibs []string) *LibraryManager {
	m := &LibraryManager{
		libTypesUsed: make(map[string]int),
		homeDir:      homeDir,
		fastnodeLibDir:   filepath.Join(fastnodeDir, "libraries"),
		walkLibs:     walkLibs,
	}

	// set up libraries dir
	err := os.MkdirAll(m.fastnodeLibDir, os.ModePerm)
	if err != nil {
		log.Println(err)
	}
...
func (m *LibraryManager) fastnodeLibs() {
	paths := make(map[string]bool)
	for _, p := range readdir.List(m.fastnodeLibDir) {
		fpath := filepath.Join(m.fastnodeLibDir, p.Path)
	...
	for lib := range paths {
		...
		// do not overwrite type if previously found
		m.dirs.LoadOrStore(canonPath, FastnodeLibraries)
		log.Printf("Adding fastnode libraries lib: %s", canonPath)
	}
}
