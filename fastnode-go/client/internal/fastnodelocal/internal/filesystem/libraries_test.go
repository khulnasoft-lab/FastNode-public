...
func Test_FastnodeLibraries(t *testing.T) {
	usr, err := user.Current()
	require.NoError(t, err)

	fastnodeDir, err := ioutil.TempDir("", "fastnode-local-fs")
	require.NoError(t, err)
	defer os.RemoveAll(fastnodeDir)
	...
	require.NoError(t, err)
	fastnodeLibs := filepath.Join(fastnodeDir, "libraries")
	err = os.Mkdir(fastnodeLibs, os.ModePerm)
	require.NoError(t, err)
	linkPath := filepath.Join(fastnodeLibs, "foo")
	err = os.Symlink(lib, linkPath)
	require.NoError(t, err)

	m := NewLibraryManager(usr.HomeDir, fastnodeDir, nil)
