...
import (
	...
	"github.com/khulnasoft-lab/fastnode/fastnode-go/client/internal/watch"
	"github.com/khulnasoft-lab/fastnode/fastnode-go/client/platform"
	"github.com/khulnasoft-lab/fastnode/fastnode-go/client/readdir"
...
const (
	...
	permissionsWarningMessage = "Fastnode will request permission to look for Python libraries"
	permissionsWarningInfo    = `Fastnode accesses files in your home directory to automatically discover Python virtualenvs and to index your code. macOS may prompt you for permission.`
...
type Options struct {
	...
	RootDir string
	// FastnodeDir is the path to Fastnode's user data directory
	FastnodeDir string
...
// FastnodeDir returns Fastnode's user data directory
func (f *Manager) FastnodeDir() string {
	return f.fastnodeDir
}
...
func (l *libraryWalker) backgroundWalk() {
	...
	clienttelemetry.FastnodeTelemetry("Background Library Walk Completed", map[string]interface{}{
...
func (fs *LocalFS) Walk(ctx fastnodectx.Context, root string, walkFn localcode.WalkFunc) error {
	ctx.CheckAbort()
...
func (fs *LocalFS) walk(ctx fastnodectx.Context, root string, fi localcode.FileInfo, walkFn localcode.WalkFunc) error {
	ctx.CheckAbort()
