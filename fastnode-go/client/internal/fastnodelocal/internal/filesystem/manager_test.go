...
func Test_WalkDir(t *testing.T) {
	tempDir, err := ioutil.TempDir("", "fastnode-local-fs")
...
func Test_FileSystemWalk(t *testing.T) {
	...
	var files []string
	err = mgr.LocalFS.Walk(fastnodectx.Background(), filepath.Dir(file1), func(path string, fi localcode.FileInfo, err error) error {
		if err != nil {
			if fi.IsDir {
...
func Test_SkipLibraryDir(t *testing.T) {
	...
	var files []string
	err = mgr.LocalFS.Walk(fastnodectx.Background(), tempDir, func(path string, fi localcode.FileInfo, err error) error {
		if err != nil {
			if fi.IsDir {
				assert.Equal(t, localcode.ErrLibDir, err)
				return localcode.ErrSkipDir
...
func Test_DirNotAccepted(t *testing.T) {
	tempDir, err := ioutil.TempDir("", "fastnode-local-fs-bad")
	require.NoError(t, err)
