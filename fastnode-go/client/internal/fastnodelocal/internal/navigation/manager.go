...
import (
	...
	"github.com/khulnasoft-lab/fastnode/fastnode-go/client/internal/ws"
...
func (m *Manager) handleFetchRelated(w http.ResponseWriter, r *http.Request) {
	...
	if req.Offset == 0 {
		...
		clienttelemetry.EventWithFastnodeTelemetry("code_finder_results_displayed", map[string]interface{}{
			"editor": req.Editor,
		})
	}
}
...
var msgStillIndexing = "Fastnode is not done indexing your project yet. Please wait for the status icon to switch to ready before using Code Finder."
...
func (m *Manager) triggerEditorRelated(r *http.Request) (string, int) {
	...
	errorShown := func(err error) {
		clienttelemetry.EventWithFastnodeTelemetry("code_finder_query_error_shown", map[string]interface{}{
			"editor": req.Editor,
	...
	case codebase.ErrPathInFilteredDirectory:
		...
		return fmt.Sprintf("Code Finder cannot operate on %s, as it is in a private directory ignored by Fastnode.", fname), http.StatusMethodNotAllowed
	...
	clienttelemetry.EventWithFastnodeTelemetry("code_finder_query_sent", map[string]interface{}{
		"editor": req.Editor,
		"type":   reqType,
	})
