...
import (
	...
	"github.com/khulnasoft-lab/fastnode/fastnode-golib/wstest"
...
var (
	...
	nongit     = filepath.Join(github, "khulnasoft-lab", "foo.py")
	khulnasoft-lab     = filepath.Join(github, "khulnasoft-lab", "khulnasoft-lab")
	testDir    = filepath.Join(khulnasoft-lab, "fastnode-go", "navigation", "offline", "testdata")
...
func THandleDecorationLine(r *mux.Router, m *Manager) func(*testing.T) {
	return func(t *testing.T) {
		...
		assert.EqualValues(t, jsonresp["inline_message"], "Find related code in khulnasoft-lab")
		assert.EqualValues(t, jsonresp["hover_message"], "Search for code in khulnasoft-lab which may be related to this line")
...
func THandleRequestRelated(r *mux.Router, m *Manager) func(*testing.T) {
	return func(t *testing.T) {
		...
		rel, err := filepath.Rel(khulnasoft-lab, astPath)
		require.NoError(t, err)
