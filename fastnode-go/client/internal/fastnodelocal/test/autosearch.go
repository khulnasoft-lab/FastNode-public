...
// NewClient returns a configured client, which is using the fastnoded server of testEnv
func NewClient(testEnv *clientapp.TestEnvironment) (*AutosearchClient, error) {
	var mgr *autosearch.Manager
	for _, comp := range testEnv.Fastnoded.Components() {
		var ok bool
	...
	return NewConfiguredClient(*testEnv.Fastnoded.URL, mgr)
}
...
// BroadcastServerAutosearchMessage broadcasts a message in the style of fastnoded local
