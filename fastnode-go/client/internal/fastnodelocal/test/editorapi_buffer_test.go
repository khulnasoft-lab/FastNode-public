...
func Test_All(t *testing.T) {
	// this test has subtests to share the setup of fastnodelocal
	...
	project, err := startFastnodeLocal(keytypes.BuiltinDistribution3, keytypes.NumpyDistribution)
...
func hoverContentAttached(t *testing.T, project *clientapp.TestEnvironment) {
	...
	resp, err := project.FastnodedClient.EditorBufferRequest(req, &hover)
	require.NoError(t, err)
...
func hoverOutdatedHash(t *testing.T, project *clientapp.TestEnvironment) {
	...
	assertTrueN(t, 5,
		func() bool {
			resp, err := project.FastnodedClient.EditorBufferRequest(req, nil)
...
func hoverCursorRunes(t *testing.T, project *clientapp.TestEnvironment) {
	...
	_, err := project.FastnodedClient.EditorBufferRequest(req, nil)
	require.Error(t, err)
	req.CursorOffset = -1
	_, err = project.FastnodedClient.EditorBufferRequest(req, nil)
	require.Error(t, err)
...
func hoverCursorBytes(t *testing.T, project *clientapp.TestEnvironment) {
	...
	_, err := project.FastnodedClient.EditorBufferRequest(req, &hover)
	require.Error(t, err)
	req.CursorOffset = -1
	_, err = project.FastnodedClient.EditorBufferRequest(req, &hover)
	require.Error(t, err)
...
func onboardingCompletions(t *testing.T, project *clientapp.TestEnvironment) {
	...
	err := project.FastnodedClient.GetJSON("/clientapi/plugins/onboarding_file?editor="+r.Editor, &onboardingFile)
	require.NoError(t, err)
