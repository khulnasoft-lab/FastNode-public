...
func Test_FastnodeLocalSetup(t *testing.T) {
...
func Test_SymbolReports3(t *testing.T) {
	project, err := startFastnodeLocal(keytypes.BuiltinDistribution3)
	require.NoError(t, err)
	defer project.Close()
...
func Test_SymbolReportFunction(t *testing.T) {
	...
	require.NotEmpty(t, d.KwargParameters, "Python kwargs must not be empty in fastnode local")
}
...
func Test_SymbolReportModule(t *testing.T) {
	...
	assertIsSymbolReport(t, resp)
	// fixme fastnode is returning "." as the parent's name (both local and remote), possibly incorrect data?
...
func Test_SymbolReportType(t *testing.T) {
	...
	require.NoError(t, err)
	assertReportBasics(t, resp.Report)
	assert.Empty(t, resp.Report.Examples, "curated examples are empty in fastnode local")
...
func Test_ValueLinks(t *testing.T) {
	...
	resp, err := project.FastnodedClient.Get(fmt.Sprintf("/api/editor/value/%s/links", "python;;;;json"))
	require.NoError(t, err)
