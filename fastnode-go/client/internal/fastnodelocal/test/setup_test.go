...
// starts a fastnode local test instance which loads the given distributions.
// if no distribution was passed, then stdlib 2.7 will be loaded
func startFastnodeLocal(dists ...keytypes.Distribution) (*clientapp.TestEnvironment, error) {
	if dists == nil {
...
func symbolReport(project *clientapp.TestEnvironment, symbolID string) (*editorapi.ReportResponse, error) {
	...
	err := project.FastnodedClient.GetJSON(fmt.Sprintf("/api/editor/symbol/%s", symbolID), &resp)
	return &resp, err
}
...
func valueReport(project *clientapp.TestEnvironment, valudID string) (*editorapi.ReportResponse, error) {
	var resp editorapi.ReportResponse
	err := project.FastnodedClient.GetJSON(fmt.Sprintf("/api/editor/value/%s", valudID), &resp)
	return &resp, err
}
...
func valueMembers(project *clientapp.TestEnvironment, valueID string) (*editorapi.MembersResponse, error) {
	...
	err := project.FastnodedClient.GetJSON(fmt.Sprintf("/api/editor/value/%s/members", valueID), &resp)
	return &resp, err
}
...
func search(project *clientapp.TestEnvironment, query string) (*editorapi.SearchResults, error) {
	...
	err := project.FastnodedClient.GetJSON(fmt.Sprintf("/api/editor/search?q=%s", query), &resp)
	return &resp, err
}
...
func assertReportBasics(t *testing.T, r *editorapi.Report) {
	// fixme fastnoded's report descriptions are empty, probably incorrect data? (both in remote/local environments)
	...
	assert.Empty(t, r.Examples, "examples are unavailable in fastnode local")
