...
import (
	...
	tele "github.com/khulnasoft-lab/fastnode/fastnode-go/client/internal/clienttelemetry"
	...
	"github.com/khulnasoft-lab/fastnode/fastnode-go/client/internal/metrics/completions"
	...
	"github.com/khulnasoft-lab/fastnode/fastnode-go/fastnodestatus"
	"github.com/khulnasoft-lab/fastnode/fastnode-go/lang"
	navmetrics "github.com/khulnasoft-lab/fastnode/fastnode-go/navigation/metrics"
	...
	"github.com/khulnasoft-lab/fastnode/fastnode-golib/fastnodeserver"
	"github.com/khulnasoft-lab/fastnode/fastnode-golib/lexicalv0/tfserving"
	"github.com/khulnasoft-lab/fastnode/fastnode-golib/macaddr"
	"github.com/khulnasoft-lab/fastnode/fastnode-golib/mixpanel"
...
// SidebarStatus is used to unmarshall the JSON that is sent from the sidebar to fastnoded.
...
// The metrics manager needs to know about the current port of Fastnoded and access to the backend server
func (m *Manager) Initialize(opts component.InitializerOptions) {
	m.fastnodedURL = opts.FastnodedURL
	m.clientVersion = opts.Platform.ClientVersion
	m.mixpanelToken = opts.Configuration.MixpanelToken
	m.telemetry = telemetry.NewCommonClient(telemetry.StreamFastnodeStatus)
	...
	// communicate to other processes (e.g. FastnodeService) whether or not the user has disabled metrics
	m.saveDisabledStatus(m.disabled, opts.Platform.FastnodeRoot)
}
...
func (m *Manager) RegisterHandlers(mux *mux.Router) {
	// This endpoint allows the sidebar to send status updates to incorporate into fastnode_status
...
// manager is moved from new_fastnoded
...
func (m *Manager) statusAt(now time.Time, clear bool) (map[string]interface{}, *plugins.PluginResponse) {
	...
	// Add values from the fastnodestatus package
	ksValues := fastnodestatus.Get()
	for k, v := range ksValues {
		if _, ok := fastnodestatusAllowed[k]; ok {
			s[k] = v
		}
	}
...
func (m *Manager) editorStatusesResponse() *plugins.PluginResponse {
	targetURL, err := m.fastnodedURL.Parse("/clientapi/plugins")
	if err != nil {
		return nil
	}
...
// saveDisabledStatus touches a file in the fastnode root directory to indicate that the user disabled metrics, so that
// related processes (e.g. FastnodeService) know not to log metrics.
func (m *Manager) saveDisabledStatus(disabled bool, fastnodeRoot string) {
	file := filepath.Join(fastnodeRoot, "metrics-disabled")
...
// postAnonymousTelemetryEvent posts a new event to t.khulnasoft.com which uses anonymized data
...
// postTelemetryEvent posts a new event to t.khulnasoft.com using the metrics id and the full set of status properties
func (m *Manager) postTelemetryEvent(status map[string]interface{}) {
	...
	status["source"] = "fastnoded"
	status["os"] = runtime.GOOS
	status["client_version"] = m.clientVersion

	_ = m.telemetry.Track(context.Background(), m.userIds.MetricsID(), "fastnode_status", status)
}
...
func (m *Manager) teamServerUserHash() string {
	if nameOrURL, _ := m.settings.Get(settings.FastnodeServer); nameOrURL != "" {
		parsedURL, err := fastnodeserver.ParseFastnodeServerURL(nameOrURL)
