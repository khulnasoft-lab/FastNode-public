...
func Test_EditorStatuses(t *testing.T) {
	...
	// register the handler in Fastnoded
	// returns ATOM as installed&running, VSCode as installed & not running, and VIM as not installed
	s.Fastnoded.Router.HandleFunc("/clientapi/plugins", func(w http.ResponseWriter, r *http.Request) {
...
func Test_SendStatus(t *testing.T) {
	...
	// register plan endpoint in fastnoded, it's not yet a component
	s.Fastnoded.Router.HandleFunc("/clientapi/plan", func(w http.ResponseWriter, r *http.Request) {
...
func Test_CounterRequest(t *testing.T) {
	...
	resp, err := s.DoFastnodedPost("/clientapi/metrics/counters", strings.NewReader(`{"name":"my-counter", "value":1}`))
	assert.NoError(t, err)
...
func Test_StatusRequest(t *testing.T) {
	...
	resp, err := s.DoFastnodedGet("/clientapi/metrics")
	assert.NoError(t, err)
	assert.EqualValues(t, http.StatusOK, resp.StatusCode)
}
...
func Test_SidebarStatusRequest(t *testing.T) {
	...
	resp, err := s.DoFastnodedPost("/clientapi/metrics/sidebar", bytes.NewReader(body))
	assert.NoError(t, err)
	assert.EqualValues(t, http.StatusOK, resp.StatusCode)
