...
const (
	...
	tokenHeaderKey     = "Fastnode-Token"
	tokenDataHeaderKey = "Fastnode-TokenData"
)
...
func (s *MockBackendServer) handleNewUser(requirePassword, requireChannel bool, w http.ResponseWriter, r *http.Request) {
	...
	// fastnoded will only authenticate if we return a cookie
	http.SetCookie(w, &http.Cookie{
		Name:    sessionKey,
		Value:   "session-key",
		Path:    "/",
...
// handles /api/ping, adds a cookie named fastnode-ping-cookie
...
// handles /api/pingCookie, adds a cookie named fastnode-ping-cookie
func (s *MockBackendServer) handlePingCookie(w http.ResponseWriter, r *http.Request) {
	http.SetCookie(w, &http.Cookie{Name: "fastnode-ping-cookie", Value: "fastnode-ping-cookie-value"})
	w.Write([]byte("pingback"))
