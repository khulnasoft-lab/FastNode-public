...
func (d *DummyEventer) FastnodedInitialized() {
	d.Test = true
}

func (d *DummyEventer) FastnodedUninitialized() {
	d.Test = false
}
...
//test fastnoded (un)initializing
func Test_FastnodedEventing(t *testing.T) {
	//setup componentMgr to call events
	componentMgr := component.NewTestManager()
	eventer := &DummyEventer{}
	err := componentMgr.Add(eventer)
	assert.NoError(t, err)
	componentMgr.FastnodedInitialized()
	assert.True(t, eventer.Test)
	componentMgr.FastnodedUninitialized()
	assert.False(t, eventer.Test)
}
