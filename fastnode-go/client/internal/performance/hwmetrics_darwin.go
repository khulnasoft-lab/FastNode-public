...
#include <string.h>
#include <stdio.h>
#include "include/fastnode_smc.c"
...
static unsigned int fastnode_from_fpe2(uint8_t data[32])
{
    unsigned int ans = 0;
...
unsigned int get_fan_rpm(unsigned int fan_num)
{
    ...
    fastnode_smc_return_t  result_smc;

    sprintf(key, "F%dAc", fan_num);
    result = fastnode_read_smc(key, &result_smc);

    if (!(result == kIOReturnSuccess &&
          result_smc.data_size == 2 &&
          result_smc.data_type == fastnode_to_uint32(DATA_TYPE_FPE2))) {
        // Error
        return 0;
    }

    return fastnode_from_fpe2(result_smc.data);
}
...
int get_num_fans(void)
{
    ...
    result = fastnode_read_smc(NUM_FANS, &result_smc);

    if (!(result == kIOReturnSuccess &&
          result_smc.data_size == 1   &&
          result_smc.data_type == fastnode_to_uint32(DATA_TYPE_UINT8))) {
        // Error
        return -1;
    }
