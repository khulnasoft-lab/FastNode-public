...
func TestInstallFlow(t *testing.T) {
	dir, cleanup := shared.SetupTempDir(t, "fastnode-atom")
	defer cleanup()
	...
	processMgr := &process.MockManager{
		...
		RunResult: func(name string, arg ...string) ([]byte, error) {
			...
			if name == apmPath && args == "install fastnode" {
				isInstalled = true
				return []byte("Installing fastnode to ..."), nil
			}

			if name == apmPath && args == "uninstall fastnode" {
				if !isInstalled {
					return nil, fmt.Errorf("Uninstalling fastnode ✗\nFailed to delete fastnode: Does not exist")
				}
