...
const marketplacePluginXMLID = "com.fastnode.intellij"
...
func downloadBetaChannelPlugin(targetFile io.WriteCloser) error {
	...
	resp, err := http.DefaultClient.Get("https://fastnode-plugin-binaries.s3-us-west-1.amazonaws.com/latest/fastnode-pycharm.zip")
	if err != nil {
		return err
	}
...
// downloadFastnodePluginZip downloads the latest compatible release from the JetBrains marketplace.
...
func downloadFastnodePluginZip(targetFile io.WriteCloser, ideBuild buildnumber.BuildNumber) error {
	...
	// The fastnoded bootstrapping is already setting up the default client's proxy.
	...
	if err != nil {
		return errors.Errorf("error downloading fastnode plugin zip: %s", err.Error())
	}
	defer resp.Body.Close()
