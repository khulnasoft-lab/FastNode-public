...
func Test_Download(t *testing.T) {
	...
	err = downloadFastnodePluginZip(tempFile, ideBuild)
	require.NoError(t, err)
}
...
func Test_UnzipBetaBuild(t *testing.T) {
	tempDir, err := ioutil.TempDir("", "fastnode-pycharm")
	require.NoError(t, err)
...
func Test_URLs(t *testing.T) {
	...
	require.EqualValues(t, "https://plugins.jetbrains.com/pluginManager/?action=download&build=IC-201.1234.5&id=com.fastnode.intellij&uuid=unique-machine-id", downloadURL.String())
	downloadURL = downloadPluginURL(build, "")
	require.EqualValues(t, "https://plugins.jetbrains.com/pluginManager/?action=download&build=IC-201.1234.5&id=com.fastnode.intellij", downloadURL.String())
}
