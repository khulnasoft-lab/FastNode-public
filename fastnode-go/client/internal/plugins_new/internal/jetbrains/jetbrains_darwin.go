...
import (
	...
	errorsui "github.com/khulnasoft-lab/fastnode/fastnode-go/client/internal/plugins_new/errors"
...
// pluginDirectories returns the directory which contains all plugins and the directory to contain the fastnode-pycharm plugin and an optional error
// This most often is parent, parent/fastnode-pycharm
...
func (m *macJetBrains) pluginsDirectory(ideHome string) (string, string, error) {
	...
	if !fs.DirExists(pluginsDir) {
		msg := fmt.Sprintf("Plugin configuration directory %s doesn't exist.\nPlease try going through the %s first-time configuration flow and restarting Fastnode", pluginsDir, m.Name())
