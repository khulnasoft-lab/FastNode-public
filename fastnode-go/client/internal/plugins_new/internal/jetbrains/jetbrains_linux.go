...
// pluginDirectories returns the directory to store the fastnode-pycharm plugin and an optional error
func (i *linuxJetBrains) pluginsDirectory(ideHome string) (string, error) {
	...
	if !fs.DirExists(configDir) {
		msg := fmt.Sprintf("Configuration directory %s doesn't exist.\nPlease try going through the %s first-time configuration flow and restarting Fastnode", configDir, i.Name())
