...
import (
	...
	"github.com/khulnasoft-lab/fastnode/fastnode-go/client/internal/plugins_new/internal"
	...
	fastnodeerrors "github.com/khulnasoft-lab/fastnode/fastnode-golib/errors"
...
func findManagerByIDTest(id string, process process.LinuxProcessManager) (editor.Plugin, error) {
	...
	return nil, fastnodeerrors.New("unable for find JetBrains plugin manager with ID %s", id)
}
...
func TestPycharmFlow(t *testing.T) {
	baseDir, cleanup := shared.SetupTempDir(t, "fastnode-pycharm-ide")
	defer cleanup()
...
func TestGoLandFlow(t *testing.T) {
	baseDir, cleanup := shared.SetupTempDir(t, "fastnode-goland-ide")
	defer cleanup()
...
func TestWebStormFlow(t *testing.T) {
	baseDir, cleanup := shared.SetupTempDir(t, "fastnode-webstorm-ide")
	defer cleanup()
...
func Test2019_3(t *testing.T) {
	...
	pluginJars, err := filepath.Glob(fmt.Sprintf("%s/*/fastnode-pycharm-*.jar", pluginDir))
	require.NoError(t, err)
...
func Test2020_1(t *testing.T) {
	...
	// make sure that the plugin for was installed at the correct location
	pluginJars, err = filepath.Glob(fmt.Sprintf("%s/%s/*/fastnode-pycharm-*.jar", configDir, pluginDirName))
	require.NoError(t, err)
	require.Len(t, pluginJars, 1, "plugin must be installed")
}
...
func TestPyCharmAnaconda2019_3(t *testing.T) {
	...
	baseDir, cleanup := shared.SetupTempDir(t, "fastnode-pycharm-anaconda-ide")
	defer cleanup()
...
// https://github.com/khulnasoft-lab/fastnode/issues/8868
