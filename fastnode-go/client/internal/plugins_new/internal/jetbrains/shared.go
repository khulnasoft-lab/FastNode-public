...
const (
	pluginDirName = "fastnode-pycharm"
...
func extractPluginData(ideBuild buildnumber.BuildNumber, targetParentDir, installedPluginParentDir string, betaChannel bool) error {
	...
	tempFile, err := ioutil.TempFile("", "fastnode-pycharm-*.zip")
	if err != nil {
		return err
	}
	...
	if betaChannel {
		...
		if err := downloadFastnodePluginZip(tempFile, ideBuild); err != nil {
			rollbar.Error(err)
			return err
		}
	}
...
// shouldInstallNewRemoteVersion returns if an update of the Fastnode plugin is required.
