...
import (
	...
	"github.com/khulnasoft-lab/fastnode/fastnode-go/client/internal/plugins_new/internal/jetbrains/buildnumber"
...
func TestFindProductVersion(t *testing.T) {
	baseDir, cleanup := shared.SetupTempDir(t, "fastnode-intellij-ide")
	defer cleanup()
...
func TestFindEditors(t *testing.T) {
	baseDir, cleanup := shared.SetupTempDir(t, "fastnode-intellij")
	defer cleanup()
...
func TestIsIntelliJInstalled(t *testing.T) {
	...
	fastnodePluginDirUltimate := setupPluginsDir(t, baseDir, "IntelliJIdea2017.2")
	fastnodePluginDirCommunity := setupPluginsDir(t, baseDir, "IdeaIC2017.2")
	...
	err = os.MkdirAll(fastnodePluginDirUltimate, 0700)
	require.NoError(t, err)
	assert.True(t, mgr.IsInstalled(context.Background(), ideaUltimateDir), "with an existing plugin dir isInstalled must return true for %s and plugin dir %s", ideaUltimateDir, fastnodePluginDirUltimate)
	...
	err = os.MkdirAll(fastnodePluginDirCommunity, 0700)
	require.NoError(t, err)
...
func TestIsPyCharmInstalled(t *testing.T) {
	...
	fastnodePluginDirPro := setupPluginsDir(t, baseDir, "PyCharm2017.2")
	fastnodePluginDirCommunity := setupPluginsDir(t, baseDir, "PyCharmCE2017.2")
	...
	err = os.MkdirAll(fastnodePluginDirPro, 0700)
	require.NoError(t, err)
...
func TestDetectToolboxFolders(t *testing.T) {
	dir, cleanup := shared.SetupTempDir(t, "fastnode-jetbrains-ide")
	defer cleanup()
...
func extractWithVersion(t *testing.T, installedVersion string) error {
	...
	parentDir, cleanup := shared.SetupTempDir(t, "fastnode-jetbrains-ide")
	defer cleanup()
...
func Test_ShouldInstall(t *testing.T) {
	...
	tempDir, err := ioutil.TempDir("", "fastnode")
	require.NoError(t, err)
...
// creates the parent dir which will contain the fastnode-pycharm directory
...
// creates the parent dir which will contain the fastnode-pycharm directory, using the new locations of 2020.1+
// returns the path to the plugin directory, but does not create it
