...
func UpdatePluginAtomically(pluginDir string, extractPluginData func(parentDir string) error) error {
	...
	tempNewPluginParent, err := ioutil.TempDir("", "fastnode-plugin-new")
	if err != nil {
		return err
	}
	...
	tempOldPluginParent, err := ioutil.TempDir("", "fastnode-plugin-old")
	if err != nil {
		return err
	}
...
func UninstallPlugin(pluginDir string) error {
	...
	tempParent, err := ioutil.TempDir("", "fastnode-uninstall")
	if err != nil {
		return errors.Errorf("unable to create temp directory: %v", err)
	}

	tempPath := filepath.Join(tempParent, "fastnode-plugin")
