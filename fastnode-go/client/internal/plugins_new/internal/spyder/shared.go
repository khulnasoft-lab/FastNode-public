...
// isFastnodeEnabled returns if Fastnode is enabled in the spyder config file
func isFastnodeEnabled(configFilePath string) bool {
	value, err := getSpyderConfigValue(configFilePath, "fastnode", "enable")
	return err == nil && value == "True"
}

func setFastnodeEnabled(configFilePath string, enableFastnode bool) error {
	value := "False"
	if enableFastnode {
		value = "True"
	}
	return setSpyderConfigValue(configFilePath, "fastnode", "enable", value)
}
...
// findSectionOffset returns the start of the fastnode section in the config data
