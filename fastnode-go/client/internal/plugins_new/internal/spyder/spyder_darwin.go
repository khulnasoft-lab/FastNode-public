...
func (p *macSpyder) IsInstalled(ctx context.Context, configFilePath string) bool {
	return fs.FileExists(configFilePath) && isFastnodeEnabled(configFilePath)
}

func (p *macSpyder) Install(ctx context.Context, configFilePath string) error {
	if err := setFastnodeEnabled(configFilePath, true); err == errConfigFileNotFound {
		return errors.Errorf("This error message typically means that you have not run the Spyder IDE once yet. Please start Spyder to enable Fastnode for Spyder.")
	} else if err != nil {
		return err
	}
...
func (p *macSpyder) Uninstall(ctx context.Context, configPath string) error {
	return setFastnodeEnabled(configPath, false)
}

func (p *macSpyder) Update(ctx context.Context, configPath string) error {
	// no-op, because Spyder bundles Fastnode supported
	return nil
}
