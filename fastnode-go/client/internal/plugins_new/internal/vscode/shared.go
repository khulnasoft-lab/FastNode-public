...
const (
	// https://marketplace.visualstudio.com/items?itemName=khulnasoft-lab.fastnode
	vscodeMarketplaceID = "khulnasoft-lab.fastnode"
	vscodeID            = "vscode"
...
func isInstalledExtensionsDir(mgr vscodeManager, editorPath string) (bool, error) {
	...
	for _, fi := range fis {
		if strings.Contains(fi.Name(), "khulnasoft-lab.fastnode") {
			return true, nil
		}
	}
	return false, nil
...
// uninstallOldPlugin removes the data of the old plugin which was shipped with fastnoded
func uninstallOldPlugin(mgr vscodeManager, editorPath string) error {
	dest := filepath.Join(mgr.userExtensionsDir(editorPath), "fastnode.vscode")
