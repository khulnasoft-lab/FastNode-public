...
func TestInstallFlow(t *testing.T) {
	...
	fastnodeExtensionDir := filepath.Join(dir, ".vscode", "extensions", "khulnasoft-lab.fastnode")
	...
	processMgr := &process.MockManager{
		...
		RunResult: func(name string, arg ...string) ([]byte, error) {
			...
			if strings.HasSuffix(name, "code") && args == "--install-extension khulnasoft-lab.fastnode" {
				if fs.DirExists(fastnodeExtensionDir) {
					return nil, fmt.Errorf("extension 'khulnasoft-lab.fastnode' is already installed")
				}
