...
import (
	...
	"github.com/khulnasoft-lab/fastnode/fastnode-go/client/internal/plugins_new/fs"
...
func TestInstallFlow(t *testing.T) {
	dir, cleanup := shared.SetupTempDir(t, "fastnode-vscode")
	defer cleanup()
	...
	for _, exePath := range exePaths {
		...
		fastnodeExtensionDir := filepath.Join(dir, vsCodePluginDir, "extensions", "khulnasoft-lab.fastnode")
		if strings.Contains(exePath, "Insiders") {
			fastnodeExtensionDir = filepath.Join(dir, insidersPluginDir, "extensions", "khulnasoft-lab.fastnode")
		}
		...
		processMgr := &process.MockManager{
			...
			RunResultWithEnv: func(name string, additionalEnv []string, arg ...string) ([]byte, error) {
				...
				if name == cliPath && args == "--list-extensions" {
					if fs.DirExists(fastnodeExtensionDir) {
						return []byte("some.extension\nkhulnasoft-lab.fastnode\nanother.extension"), nil
					}
					return nil, nil
				}

				if name == cliPath && args == "--install-extension khulnasoft-lab.fastnode" {
					if fs.DirExists(fastnodeExtensionDir) {
						return nil, errors.Errorf("Extension 'khulnasoft-lab.fastnode' is already installed")
					}
					// this is needed for the uninstall which removes this directory
					os.MkdirAll(fastnodeExtensionDir, 0700)
					return []byte("Installing\nExtension 'khulnasoft-lab.fastnode' v0.74.0 was successfully installed!"), nil
				}
...
func TestIsInstalled(t *testing.T) {
	...
	for _, exePath := range exePaths {
		...
		// command output includes fastnode package
		processMgr := &process.MockManager{
			RunResultWithEnv: func(name string, additionalEnv []string, arg ...string) ([]byte, error) {
				args := strings.Join(arg, " ")
				if name == cliPath && args == "--list-extensions" {
...
func TestIsNotInstalled(t *testing.T) {
	...
	// the command package doesn't include the fastnode package
	processMgr := &process.MockManager{}
