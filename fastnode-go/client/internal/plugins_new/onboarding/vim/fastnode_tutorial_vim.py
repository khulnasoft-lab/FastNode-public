...
""" PYTHON TUTORIAL ============================================================
    ...
    Fastnode's Vim plugin only supports Python right now. Support for other 
...
""" PART 0: BEFORE WE START ====================================================
    ...
    Fastnode's Vim plugin will by default try to start the Fastnode backend when the
    editor first starts. You can disable this behavior with the command
    "FastnodeDisableAutoStart" and enable it with "FastnodeEnableAutoStart".

    Add "%{fastnode#statusline()}" to your statusline to see the status of the Fastnode
    Engine. If you don't have a statusline, run these commands to get a basic
    statusline:

    set statusline=%<%f\ %h%m%r%{fastnode#statusline()}%=%-14.(%l,%c%V%)\ %P
    set laststatus=2

    Make sure that Fastnode's status reads "Fastnode" - This means that Fastnode is ready
    and working. If the indicator reads "not running", then you'll have to
    start the Fastnode Engine manually before proceeding with the rest of this
    tutorial.
"""
...
""" PART 1: CODE COMPLETIONS ===================================================
    ...
    You can disable autocompletions with "let g:fastnode_auto_complete=0" and
    enable it with "let g:fastnode_auto_complete=1".
    ...
    completion by setting "let g:fastnode_tab_complete=1".

    Look for the Fastnode symbol on the right-hand side to see which completions
...
# experience with Fastnode may not be ideal because these plugins may conflict
# with Fastnode's behavior. We suggest temporarily disabling completions from other
# plugins while you try out Fastnode. You won't be disappointed!


# 1b. Configuring completions
#
# You can configure how completions behave with the completeopt option. If you
# haven't configured completeopt yourself, Fastnode configures it like so:
#
...
#
# Make sure that you either have "menu" or "menuone" set. Otherwise Fastnode will
# not be able to display the completions UI.
#
# If you have "preview" set, Fastnode will show you documentation for the currently
...
""" PART 2: FUNCTION ASSIST ====================================================
    ...
    * Set "let g:fastnode_auto_complete=0" to prevent function assist from
...
""" That's it!
    ...
    "FastnodeTutorial" from the command line.

    You can learn more about Fastnode's Vim plugin at its GitHub repo:

    https://github.com/khulnasoft-lab/vim-plugin
