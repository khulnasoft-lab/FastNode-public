...
import (
	...
	"github.com/khulnasoft-lab/fastnode/fastnode-go/client/internal/plugins_new/internal/jupyterlab"
	"github.com/khulnasoft-lab/fastnode/fastnode-go/client/internal/plugins_new/internal/neovim"
	"github.com/khulnasoft-lab/fastnode/fastnode-go/client/internal/plugins_new/internal/shared"
...
var spyderSuboptimalSettingsStatus = fastnodestatus.GetBooleanDefault("spyder_suboptimal_settings", false)
...
func (m *Manager) Initialize(opts component.InitializerOptions) {
	...
	m.editors = newEditorsList(filepath.Join(m.platform.FastnodeRoot, "editors.json"))
	_ = m.editors.load()
}

// GoTick implements component GoTicker
// every 60 minutes, it updates the fastnodestatus property "spyder_suboptimal_settings"
func (m *Manager) GoTick(ctx context.Context) {
	if time.Since(m.lastSettingsCheck) > settingsCheckInterval {
		...
		if optimalSettings, _, err := spyder.SettingsStatus(ctx, m.pluginManagers[spyder.ID]); err == nil {
			log.Printf("updating Fastnode_status flag for spyder settings: suboptimal = %v", !optimalSettings)
...
func (m *Manager) RegisterHandlers(mux *mux.Router) {
	...
	// request to help Copilot with Fastnode's optimized settings for Spyder
...
func (m *Manager) handleInstall(w http.ResponseWriter, r *http.Request) {
	...
	if err != nil {
		clienttelemetry.FastnodeTelemetry("Editor Plugin Install Failed", map[string]interface{}{
			"editor": manager.ID(),
			"path":   path,
			"error":  err,
		})
	...
	clienttelemetry.FastnodeTelemetry("Editor Plugin Installed", map[string]interface{}{
		"editor": manager.ID,
		"path":   path,
	})
	installs, errObj := m.detectEditors(r.Context(), manager)
...
func (m *Manager) handleUninstall(w http.ResponseWriter, r *http.Request) {
	...
	if err != nil {
		clienttelemetry.FastnodeTelemetry("Editor Plugin Uninstall Failed", map[string]interface{}{
			"editor": manager.ID(),
			"path":   path,
			"error":  err,
		})
	...
	// Setup response
	clienttelemetry.FastnodeTelemetry("Editor Plugin Uninstalled", map[string]interface{}{
		"editor": manager.ID(),
		"path":   path,
	})
	installs, errObj := m.detectEditors(r.Context(), manager)
...
func (m *Manager) getOnboardingData(editor, os string, language lang.Language) ([]byte, error) {
	...
	filenames := []string{
		fmt.Sprintf("onboarding/%s/fastnode_tutorial_%s_%s.%s", editor, editor, os, ext),
		fmt.Sprintf("onboarding/%s/fastnode_tutorial_%s.%s", editor, editor, ext),
	}
...
func (m *Manager) getOnboardingFilepath(language lang.Language, editor string) string {
	...
	return filepath.Join(m.onboardingDir, fmt.Sprintf("fastnode_tutorial.%s", ext))
}
...
func (m *Manager) handleOpenFile(w http.ResponseWriter, r *http.Request) {
	...
	if req.BlockRank != 0 {
		clienttelemetry.EventWithFastnodeTelemetry("code_finder_code_block_opened", map[string]interface{}{
			"editor":     mux.Vars(r)["id"],
			"block_rank": req.BlockRank,
			"file_rank":  req.FileRank,
		})
	} else {
		clienttelemetry.EventWithFastnodeTelemetry("code_finder_file_opened", map[string]interface{}{
			"editor":    mux.Vars(r)["id"],
			"file_rank": req.FileRank,
		})
	}
