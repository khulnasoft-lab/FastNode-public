...
func Test_GetEmptyEncounteredEditors(t *testing.T) {
	...
	resp, err := s.DoFastnodedGet("/clientapi/plugins/encountered")
	assert.NoError(t, err)
...
func Test_GetNonEmptyEncounteredEditors(t *testing.T) {
	...
	resp, err := s.DoFastnodedPost("/clientapi/plugins/encountered", strings.NewReader(string(body)))
	assert.NoError(t, err)
...
func Test_OnboardingTracking(t *testing.T) {
	...
	// the fastnodelocal component handles the event processing
	fastnodeLocalMgr, err := fastnodelocal.NewManager(s.Components, fastnodelocal.Options{
	...
	err = s.SetupComponents(nil,
		...
		pluginMgr, fastnodeLocalMgr)
	require.NoError(t, err)

	var filepath string
	err = s.FastnodedClient.GetJSON("/clientapi/plugins/onboarding_file?editor=atom", &filepath)
	require.NoError(t, err)
	...
	_, err = s.FastnodedClient.PostEventData("atom", filepath, string(content), "edit", int64(float32(len(content))*0.7), int64(float32(len(content))*0.7), stringindex.UTF8)
	require.NoError(t, err)
	...
	// atom event
	_, err = s.FastnodedClient.PostEventData("atom", filepath, string(content), "edit", int64(len(content)), int64(len(content)), stringindex.UTF8)
	require.NoError(t, err)
	...
	// pycharm event
	_, err = s.FastnodedClient.PostEventData("pycharm", filepath, string(content), "edit", int64(len(content)), int64(len(content)), stringindex.UTF8)
	require.NoError(t, err)
...
func Test_OnboardingFilesPython(t *testing.T) {
	...
	for _, editor := range []string{"atom", "intellij", "vscode"} {
		err = s.FastnodedClient.GetJSON("/clientapi/plugins/onboarding_file?editor="+editor, &filepath)
		require.NoError(t, err)
		...
		assetData, err := Asset(fmt.Sprintf("onboarding/%s/fastnode_tutorial_%s_%s.py", editor, editor, runtime.GOOS))
		require.NoError(t, err)
	...
	for _, editor := range []string{"spyder", "sublime3", "vim"} {
		...
		assetData, err := Asset(fmt.Sprintf("onboarding/%s/fastnode_tutorial_%s.py", editor, editor))
		require.NoError(t, err)
		assert.EqualValues(t, assetData, data)
	}
}
...
func Test_OnboardingFilesGolang(t *testing.T) {
	...
	for _, editor := range []string{"intellij"} {
		err = s.FastnodedClient.GetJSON("/clientapi/plugins/onboarding_file?language=go&editor="+editor, &filepath)
		require.NoError(t, err)
		...
		assetData, err := Asset(fmt.Sprintf("onboarding/%s/fastnode_tutorial_%s.golang", editor, editor))
		require.NoError(t, err)
		assert.EqualValues(t, assetData, data)
	}
}
