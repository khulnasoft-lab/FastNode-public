...
func (m *Manager) pluginsToUpdate(ctx context.Context) map[string][]string {
	...
	if len(installedEditors) > 0 {
		clienttelemetry.FastnodeTelemetry("Found Editors", flatten(installedEditors))
	} else {
		clienttelemetry.FastnodeTelemetry("Found No Editors", nil)
	}
	if len(toUpdate) > 0 {
		clienttelemetry.FastnodeTelemetry("Found Installed Plugins", flatten(toUpdate))
	} else {
		clienttelemetry.FastnodeTelemetry("Found No Installed Plugins", nil)
	}
	return toUpdate
}
...
func (m *Manager) update(ctx context.Context, toUpdate map[string][]string) map[string][]string {
	...
	if len(failed) > 0 {
		clienttelemetry.FastnodeTelemetry("Failed Plugin Updates", flatten(failed))
	}
	if len(succeeded) > 0 {
		clienttelemetry.FastnodeTelemetry("Successful Plugin Updates", flatten(succeeded))
	}
	return failed
}
