...
func MachineID() (string, error) {
	return get(registry.LOCAL_MACHINE, `Software\Fastnode`, "MachineID")
}

// IsDebug gets the value of the IsDebug key
func IsDebug() (string, error) {
	return get(registry.LOCAL_MACHINE, `Software\Fastnode`, "IsDebug")
}

// InstallPath gets the path where Fastnode was installed
func InstallPath() (string, error) {
	return get(registry.LOCAL_MACHINE, `Software\Fastnode\AppData`, "InstallPath")
}

// WasVisible returns the value of WasVisible
func WasVisible() (bool, error) {
	val, err := get(registry.CURRENT_USER, `Software\Fastnode\AppData`, "WasVisible")
	return val == "1", err
}
...
func SetWasVisible(val bool) error {
	...
	return set(registry.CURRENT_USER, `Software\Fastnode\AppData`, "WasVisible", setVal)
}

// UpdateHKCURun updates the registry entry that controls starting fastnode on startup.
func UpdateHKCURun() error {
	...
	return set(registry.CURRENT_USER, `Software\Microsoft\Windows\CurrentVersion\Run`, "Fastnode",
		fmt.Sprintf("\"%s\" --system-boot", filepath.Join(installdir, "fastnoded.exe")))
}

// RemoveHKCURun removes the registry entry that starts fastnode on startup.
func RemoveHKCURun() error {
	if val, err := get(registry.CURRENT_USER, `Software\Microsoft\Windows\CurrentVersion\Run`, "Fastnode"); err != nil || val == "" {
		return err
	}
	return deleteValue(registry.CURRENT_USER, `Software\Microsoft\Windows\CurrentVersion\Run`, "Fastnode")
}

// TrayIconHandle gets the value of the TrayIconHandle key
func TrayIconHandle() (uintptr, error) {
	s, err := get(registry.CURRENT_USER, `Software\Fastnode\AppData`, "LastTrayHwnd")
	if err != nil {
		return 0, err
	}
...
func SetTrayIconHandle(h uintptr) error {
	return set(registry.CURRENT_USER, `Software\Fastnode\AppData`, "LastTrayHwnd", fmt.Sprintf("%d", h))
}

// get looks up a string-valued name in the Fastnode registry key.
...
// set set a string-valued name in the Fastnode registry key.
...
// deleteValue removes a value in the Fastnode registry key.
