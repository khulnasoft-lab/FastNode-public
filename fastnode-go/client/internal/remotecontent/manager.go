...
import (
	...
	"github.com/khulnasoft-lab/fastnode/fastnode-go/conversion/remotecontent"
...
var rcFastnodeTarget = "https://" + domains.RemoteConfig
...
func (m *Manager) RegisterHandlers(mux *mux.Router) {
	...
	// fetch updates the fastnoded cache (m.remoteContent) and responds when that operation is complete
	mux.HandleFunc("/clientapi/remotecontent/fetch", m.handleFetchRemoteContent).Methods("POST")

	// get returns the contents of the fastnoded cache (m.remoteContent)
...
func (m *Manager) fetchRemoteContent() (json.RawMessage, error) {
	dest := fmt.Sprintf("%s/convcohort/remotecontent/", rcFastnodeTarget)
