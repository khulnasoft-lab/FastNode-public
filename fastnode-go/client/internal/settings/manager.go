...
const (
	...
	FastnodeServer = "fastnode_enterprise_server"
)
...
// NewManager creates a new Manager for handle settings of the fastnoded client
// exposes a simple REST api, see https://fastnode.quip.com/ZdxEAGXI75IC for details.
func NewManager(path string) *Manager {
	...
	// tracking existing users as of just before Fastnode Pro launch
	if err := m.save(); err != nil {
		...
		m.Set(FastnodeServer, *specs[FastnodeServer].defaultValue)
...
func (m *Manager) RegisterHandlers(mux *mux.Router) {
	mux.HandleFunc("/clientapi/settings/"+FastnodeServer+"/status", m.handleFastnodeServerStatus).Methods("GET")
...
func (m *Manager) GetDeploymentID() string {
	hostPort, _ := m.Get(FastnodeServer)
	id, _, _ := fastnodeserver.GetHealth(hostPort)
	return id
}

func (m *Manager) handleFastnodeServerStatus(w http.ResponseWriter, r *http.Request) {
	...
	hostPort, _ := m.Get(FastnodeServer)
	_, ping, err := fastnodeserver.GetHealth(hostPort)
