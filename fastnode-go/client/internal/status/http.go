...
import (
	...
	"github.com/khulnasoft-lab/fastnode/fastnode-golib/presentation"
)
...
func (m *Manager) handleStatus(w http.ResponseWriter, r *http.Request) {
	...
	case filetype != "":
		if filetype == "python" {
			...
			response.Long = "Fastnode can only index your code once it's saved"
		} else {
			response.Status = "unsupported"
			response.Short = "unsupported"
			response.Long = "Fastnode currently only supports Python"
		}
...
func (m *Manager) statusReponse(filename, canonical string, checkLoaded bool) enginestatus.Response {
	...
	if !ss.CompletionsSupported {
		if applesilicon.Detected {
			return enginestatus.Response{
				Status: "unsupported",
				Short:  "unsupported",
				Long:   "Fastnode only supports Python for Apple Silicon",
			}
		}
		return enginestatus.Response{
			Status: "unsupported",
			Short:  "unsupported",
			Long:   "Fastnode doesn't support the current file type",
		}
	}
	...
	if checkLoaded && !m.isLoaded(fext) && !applesilicon.Detected {
		return enginestatus.Response{
			Status: "initializing",
			Short:  "initializing",
			Long:   "Fastnode is warming up",
		}
	} else if naverr == codebase.ErrProjectStillIndexing {
		return enginestatus.Response{
			Status: "indexing",
			Short:  "indexing",
			Long:   "Fastnode is indexing your code, but should still function in the meanwhile.",
		}
	} else if fext == "py" {
		...
		if _, err := os.Stat(filename); os.IsNotExist(err) {
			return enginestatus.Response{
				...
				Long:   "Fastnode can only index your code once it's saved",
			}
		}
...
func (m *Manager) fileIndexed(filename string) bool {
	...
	for _, s := range m.localCodeStatus.Indices {
		// In Fastnode Local, we can check against the file hashes in the index
