...
func Test_Status(t *testing.T) {
	...
	resp, err := s.DoFastnodedGet("/clientapi/status?filename=" + file)
	...
	// unsupported
	resp, err = s.DoFastnodedGet("/clientapi/status?filename=" + filepath.Join(s.BasePath, "file.txt"))
	assert.Equal(t, http.StatusOK, resp.StatusCode)
	assert.NoError(t, json.NewDecoder(resp.Body).Decode(&sResp))
	assert.Equal(t, sResp.Status, "unsupported")

	resp, err = s.DoFastnodedGet("/clientapi/status?filetype=javascript")
	assert.Equal(t, http.StatusOK, resp.StatusCode)
	assert.NoError(t, json.NewDecoder(resp.Body).Decode(&sResp))
	assert.Equal(t, sResp.Status, "unsupported")

	// noIndex file (unsaved)
	resp, err = s.DoFastnodedGet("/clientapi/status?filetype=python")
	assert.Equal(t, http.StatusOK, resp.StatusCode)
	assert.NoError(t, json.NewDecoder(resp.Body).Decode(&sResp))
	assert.Equal(t, sResp.Status, "noIndex")

	resp, err = s.DoFastnodedGet("/clientapi/status?filename=" + url.QueryEscape(filepath.Join(s.BasePath, "unsaved.py")))
	assert.Equal(t, http.StatusOK, resp.StatusCode)
	assert.NoError(t, json.NewDecoder(resp.Body).Decode(&sResp))
	assert.Equal(t, sResp.Status, "noIndex")
	...
	resp, err = s.DoFastnodedGet("/clientapi/status?checkloaded=false&filename=" + file)
	assert.Equal(t, http.StatusOK, resp.StatusCode)
	assert.NoError(t, json.NewDecoder(resp.Body).Decode(&sResp))
	assert.Equal(t, sResp.Status, "indexing")
}
...
func Test_StatusInvalidPath(t *testing.T) {
	...
	resp, err := s.DoFastnodedGet("/clientapi/status?filename=home")
	assert.NoError(t, err)
	...
	//check invalid path
	resp, err = s.DoFastnodedGet("/clientapi/status?filename=home.py")
	assert.NoError(t, err)
	assert.Equal(t, http.StatusBadRequest, resp.StatusCode)

	//check empty path
	resp, err = s.DoFastnodedGet("/clientapi/status?filename=")
	assert.NoError(t, err)
	assert.Equal(t, http.StatusBadRequest, resp.StatusCode)
}
