...
import (
	...
	"github.com/khulnasoft-lab/fastnode/fastnode-golib/fastnodeinstall"
)
...
func restart() error {
	log.Println("restarting fastnoded to apply an update")
	// fastnode-update is treating exit code 10 as the signal to restart
	os.Exit(10)
	return nil
}
...
func start(ctx context.Context, bundle string, f Listener, lastEvent func() time.Time) error {
	// don't start the update loop if the fastnode-updater service is enabled
	if enabled, _ := fastnodeinstall.IsSystemdTimerEnabled(); enabled {
		return nil
	}
	...
	go func() {
		...
		for {
			...
			case <-nextUpdate.C:
				...
				if serviceEnabled, _ := fastnodeinstall.IsSystemdTimerEnabled(); serviceEnabled {
...
func checkAndDownloadUpdate() bool {
	...
	// fastnode-update is installed in the same dir as fastnoded
	updaterPath := filepath.Join(filepath.Dir(cmdPath), "fastnode-update")
