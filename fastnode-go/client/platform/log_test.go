...
// [fastnoded] 2018/01/12 18:12:07.867786 log_test.go:23: Log entry 1
var logLinematcher = regexp.MustCompile(`\[fastnoded] \d{4}/\d{2}/\d{2} \d{2}:\d{2}:\d{2}\.\d{6} log_test.go:\d+: Log entry \d+`)

func Test_Logger(t *testing.T) {
	tmpfile := filepath.Join(os.TempDir(), "fastnode_logger.txt")
...
func setupLogfile() (string, error) {
	tmpdir, err := ioutil.TempDir("", "fastnode-log")
	if err != nil {
		return "", err
	}
