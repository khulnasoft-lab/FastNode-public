...
import (
	...
	"github.com/khulnasoft-lab/fastnode/fastnode-go/client/platform/messagebox"
	"github.com/khulnasoft-lab/fastnode/fastnode-go/client/platform/version"
)
...
var (
	...
	LogPrefix = fmt.Sprintf("[%s] ", "fastnoded")
)

// Platform encapsulates platform specific code and is used in the client app
type Platform struct {
	// Logger is configured with Fastnode's patterns and writes into Fastnode's client.log file
	Logger *log.Logger
	// LogWriter writes into the client.log file
	LogWriter io.Writer
	// FastnodeRoot is the root path to Fastnode's user data directory
	FastnodeRoot string
	// LogDir is the root path to Fastnode's directory which contains the log files
	LogDir string
	// Version is the version of the currently executed instance of Fastnode
	...
	// it's true if the binary is a debug build or if $FASTNODEROOT/DEBUG exists
	DevMode bool
	...
	LogFile string
	// set if this installation was the first time Fastnode was run on this machine
	IsNewInstall bool
...
// If an empty root dir is passed then the directory $HOME/.fastnode will be used
func NewPlatform() (*Platform, error) {
	p := newPlatform(fastnodeRoot(), true)
...
func NewTestPlatformFeatures(rootDir string, featureOverride map[string]bool) (*Platform, error) {
	if rootDir == "" {
		root, err := ioutil.TempDir("", "fastnode-test")
		if err != nil {
			return nil, err
		}
...
func (p *Platform) Initialize() error {
	if p.IsNewInstall {
		_ = os.MkdirAll(p.FastnodeRoot, 0700)
	}
	...
	// setup install id
	p.InstallID, err = installid.LoadInstallID(p.FastnodeRoot)
	if err != nil {
		return err
	}
...
// IsFeatureEnabled returns true if $fastnodeRoot/$name exists in the filesystem
func (p *Platform) IsFeatureEnabled(name string) bool {
	...
	// allow to enable a feature by setting an environment variable FASTNODE_$name (with name in uppercase letters)
	if p.IsUnitTestMode && os.Getenv(fmt.Sprintf("FASTNODE_%s", strings.ToUpper(name))) != "" {
		return true
	}
...
func (p *Platform) featureFlagPath(name string) string {
	return filepath.Join(p.FastnodeRoot, name)
}
