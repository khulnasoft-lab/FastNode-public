...
func (d linuxController) cmdPath() string {
	// first, we check for linux-unpacked/fastnode as a sibling of the current process's binary
	...
	// fallback to ~/.local/share/fastnode/current/linux-unpacked/fastnode
	homeDir := os.ExpandEnv("$HOME")
	cmdPath := filepath.Join(homeDir, ".local", "share", "fastnode", "current", "linux-unpacked", processName)
	if _, err := os.Stat(cmdPath); err == nil {
		return cmdPath
	}
