...
const (
	remoteBundleFile = "/var/fastnode/upload/bundle.tar.gz"
	deployScriptName = "deploy-bundle.sh"
	sshKeyToUpload   = "fastnode-dev-azure"
)
...
func deployBundle(bundleFile string, clusterName string, cleanupClusters []string) error {
	...
	remoteDeployPath := path.Join("/var/fastnode/upload", deployScriptName)
	...
	for idx, ip := range ips {
		...
		wp.Add([]workerpool.Job{func() error {
			...
			identifyCmd := fmt.Sprintf("echo %d > /var/fastnode/instance-id && echo %d > /var/fastnode/instance-count", localIdx, len(ips))
			...
			runBundleCmd := fmt.Sprintf("nohup bash %s > /var/fastnode/log/%s.log 2>&1 &", remoteDeployPath, deployScriptName)
