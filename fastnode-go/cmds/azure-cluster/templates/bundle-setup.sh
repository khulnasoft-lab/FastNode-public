...
mkdir /var/fastnode/gputmp
cd /var/fastnode/gputmp
...
echo 'export LD_LIBRARY_PATH="$LD_LIBRARY_PATH:/usr/local/cuda/lib64:/usr/local/cuda/extras/CUPTI/lib64"' >> /var/fastnode/bundle/env.sh
echo 'export CUDA_HOME=/usr/local/cuda' >> /var/fastnode/bundle/env.sh
echo 'export PATH="$PATH:/usr/local/cuda/bin"' >> /var/fastnode/bundle/env.sh
{{end}}
...
# Install fastnode_ml into the container
current=$PWD
cd /var/fastnode/bundle
...
WORKDIR /
ADD khulnasoft-lab/fastnode-python/fastnode_ml /fastnode_ml
RUN cd /fastnode_ml && pip install -e .

ENV PATH="/var/fastnode/bundle/bin:${PATH}"
...
{{if .FastnodeML}}
# install dependencies and create/load virtualenv for fastnode_ml
sudo add-apt-repository ppa:deadsnakes/ppa -y
sudo apt-get update
sudo apt-get install python3.6 virtualenv -y

cd /var/fastnode/bundle/khulnasoft-lab/fastnode-python/fastnode_ml
...
cat << 'EOF' > /var/fastnode/bundle/activate.sh
source /var/fastnode/bundle/env.sh
if [ -d /var/fastnode/bundle/khulnasoft-lab/fastnode-python/fastnode_ml/venv ]; then
    source /var/fastnode/bundle/khulnasoft-lab/fastnode-python/fastnode_ml/venv/bin/activate
fi
EOF

cat << 'EOF' > /var/fastnode/bundle/start.sh
source /var/fastnode/bundle/activate.sh
bash /var/fastnode/bundle/run.sh
EOF

cat << 'EOF' > /home/ubuntu/.bash_aliases
source /var/fastnode/bundle/activate.sh
alias bundle='cd /var/fastnode/bundle'
alias train='cd /var/fastnode/bundle/khulnasoft-lab/local-pipelines/lexical/train'
EOF
