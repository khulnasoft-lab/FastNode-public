...
sudo mkdir -p /var/fastnode/log
sudo mkdir -p /var/fastnode/upload
sudo chown -R ubuntu.ubuntu /var/fastnode
...
sudo mkdir -p /data/fastnode
sudo chown -R ubuntu.ubuntu /data/

# update the ssh config to automatically be able to ssh into other cluster instances without prompting
# note that the key itself (~/.ssh/fastnode-dev-azure) is uploaded via ssh, along with the bundle,
...
touch /var/fastnode/provisioned
