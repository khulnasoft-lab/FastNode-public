...
# Get binary
RELEASE_DIR=/var/fastnode/releases/{{.ReleaseBin}}
mkdir -p $RELEASE_DIR
s3cmd get s3://fastnode-deploys/{{.ReleaseBin}}/{{.Process}} $RELEASE_DIR/{{.Process}}
...
<source>
  @type tail
  path /var/fastnode/log/user-mux.log
...
<source>
  @type tail
  path /var/fastnode/log/user-node.log
...
until `/var/fastnode/releases/{{.ReleaseBin}}/{{.Process}} &>> /var/fastnode/log/{{.Process}}.log`; do
