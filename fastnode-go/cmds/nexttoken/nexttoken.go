...
func main() {
	...
	ast, err := pythonparser.Parse(fastnodectx.Background(), srcbuf, pythonparser.Options{
		Approximate: true,
		Cursor:      &cursorPos,
	})
	if err != nil && ast == nil {
	...
	for _, t := range ts {
		...
		_, err := pythonparser.ParseWords(fastnodectx.Background(), srcbuf, pseudo, pythonparser.Options{
