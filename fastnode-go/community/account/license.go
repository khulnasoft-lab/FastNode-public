...
var (
	...
	fastnodeProLaunchDate    = time.Date(2020, 5, 8, 0, 0, 0, 0, time.UTC)
)
...
func (m *licenseManager) Create(ids community.UserIdentifier, fastnodeLicense licensing.License) (*license, error) {
...
func (m *manager) CreateLicense(ids community.UserIdentifier, fastnodeLicense licensing.License, reference string) (*license, error) {
	return m.license.Create(ids, fastnodeLicense)
}
...
func (s *Server) handleWebStartTrial(w http.ResponseWriter, r *http.Request) {
	...
	default:
		// other failure
		// TODO FastnodePro (naman) where to redirect for this? we need an error page of sorts
...
func (s *Server) handleCheckout(w http.ResponseWriter, r *http.Request) {
	...
	var data []byte
	if requestedTrial {
		data, err = fastnodestripe.CreateSubscriptionCheckoutOctobatWithTrial(trialDuration, user.Name, user.Email, user.IDString(), r.RemoteAddr, stripePlanID, r.Host, proInfoURL)
	} else {
		data, err = fastnodestripe.CreateSubscriptionCheckoutOctobat(user.Name, user.Email, user.IDString(), r.RemoteAddr, stripePlanID, r.Host, proInfoURL)
	}
...
func (s *Server) handleSubscriptionsDelete(w http.ResponseWriter, r *http.Request) {
	...
	subscriptions, _ := fastnodestripe.GetActiveStripeSubscriptions(user)
...
func (s *Server) findPlanID(r *http.Request, withTrial bool) (fastnodestripe.PlanID, error) {
