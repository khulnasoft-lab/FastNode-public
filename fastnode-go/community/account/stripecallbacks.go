...
import (
	...
	"github.com/khulnasoft-lab/fastnode/fastnode-golib/octobat"
	fastnodestripe "github.com/khulnasoft-lab/fastnode/fastnode-golib/stripe"
	"github.com/stripe/stripe-go/v72"
)

type stripeCallbacks struct {
	manager *manager
	plans   fastnodestripe.Plans
}

func (s *stripeCallbacks) CustomerSubscriptionUpdated(subscription stripe.Subscription, event stripe.Event) error {
	userID := subscription.Metadata["fastnode_user_id"]
	if userID == "" {
	...
	if event.Type == "customer.subscription.updated" {
		// Track Fastnode Pro subscriptions with trials.
		...
		fastnodePlan, ok := subscription.Plan.Metadata["fastnode_plan"]
		if ok && licensing.Plan(fastnodePlan).IsPaidPlanWithTrial() {
			...
			community.TrackProPurchase(userID, paidAfterTrial, s.createEventProps(licensing.Plan(fastnodePlan), subscription.LatestInvoice.ID, subscription.LatestInvoice.HostedInvoiceURL, end, paid))
		}
	}
...
func (s *stripeCallbacks) InvoicePaymentSucceed(invoice stripe.Invoice) error {
	...
	planID, ok := lineItem.Plan.Metadata["fastnode_plan"]
	if !ok {
		return errors.New("line item with plan with no fastnode_plan metadata")
	}
	plan = licensing.Plan(planID)

	userIDStr, ok := lineItem.Metadata["fastnode_user_id"]
	if !ok {
		return errors.New("line item with no fastnode_user_id metadata")
	}

	userID, err := strconv.ParseInt(userIDStr, 10, 64)
	if err != nil {
		return errors.Wrapf(err, "line item with non-uint64 fastnode_user_id metadata %s", userIDStr)
	}
	...
	if plan.IsPaidPlanWithTrial() {
		// we're using stripe's trial end data instead of now()+fastnode_trial_duration because the payment could've been delayed
...
func InitStripe(stripeSecret, stripeWebhookSecret, octobatSecret, octobatPublishable, beanieConfigID string) {
	...
	fastnodestripe.InitStripeWebhookSecret(stripeWebhookSecret)
	fastnodestripe.InitOctobat(octobatPublishable, octobatSecret, beanieConfigID)
}
...
func (s *stripeCallbacks) isProSubscriber(user *community.User) (bool, error) {
	...
	subs, err := fastnodestripe.GetActiveStripeSubscriptions(user)
	if err != nil {
