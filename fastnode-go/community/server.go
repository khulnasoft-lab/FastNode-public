...
import (
	...
	"github.com/khulnasoft-lab/fastnode/fastnode-golib/email"
...
const (
	...
	MachineHeader = "X-Fastnode-Machine"
...
func (s *Server) SetupRoutes(mux *mux.Router) {
	...
	// Used by front end to signup and invite person interested in trying out Fastnode
...
// connected to Fastnode's servers (rather than, eg, a wifi paywall).
...
// The redirect target, dst, defaults to https://khulnasoft.com/login (if passed "").
//
// A redirect=<URL> query parameter is added to dst if one doesn't already exist.
// If dst's host is "khulnasoft.com" or "www.khulnasoft.com", and the request is coming to staging,
// we dynamically redirect to ga-staging.khulnasoft.com instead.
...
func (s *Server) emailVerification(v *EmailVerification, host string) error {
	...
	return emailer(sender, email.Message{
		HTML:    true,
		To:      []string{v.Email},
		Subject: "Fastnode: verify your email",
		Body:    body.Bytes(),
	})
}
...
func (s *Server) EmailPasswordReset(user *User, reset *PasswordReset, host string) error {
	...
	return emailer(sender, email.Message{
		...
		Subject: "Reset your Fastnode password",
		Body:    body.Bytes(),
	})
}
