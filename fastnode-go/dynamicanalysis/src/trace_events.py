...
class Tracer(object):
    ...
    def last_value_of(self, node):
        # unwrap the node
        while hasattr(node, "fastnode_wrapped"):
            node = node.fastnode_wrapped
...
class TraceInserter(ast.NodeTransformer):
    ...
    def literal(self, value):
        varname = "__fastnode_literal_%d" % len(self.literals)
    ...
    def traced(self, node, func, *args):
        ...
        newnode.fastnode_wrapped = node  # so that we can retrieve the original node later
