...
#       `from trace import macros, fastnode_trace, get_all_traced_ast_reprs`,
#       and then use `with fastnode_trace:` to open a block that will be traced
#   * after the code in your block has executed, call `fastnode_trace.get_all_traced_ast_reprs()`
...
#
# k_type                     will be '__fastnode_mixed' if the value of the expression changes across evaluations
...
class Node(object):
    ...
    def annotate(self, key, value):
        if key in self._annotations and self._annotations[key] != value:
            self._annotations[key] = '__fastnode_mixed'
        else:
...
class FastnodeWalker(macropy.core.macros.Walker):

    def walk_children(self, tree, *args, **kw):
        if isinstance(tree, list):
            def inner(node_tracker):
                return super(FastnodeWalker, self).walk_children(tree, *args, **kw)
...
@macros.expr
def fastnode_trace(tree, exact_src, **kw):
