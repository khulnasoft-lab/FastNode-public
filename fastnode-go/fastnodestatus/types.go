...
type Metric interface {
	// Value returns a map of metric name -> value/object to be added to fastnode_status
	Value() map[string]interface{}

	// Reset will reset the metrics, determined by the cadence of fastnode_status metrics updates
	Reset()
}

// GetMetric returns a handle to the Metric object registered with fastnodestatus. This method
// ensures you are always using the correct object (that fastnodestatus is aware of) when manipulating a metric.
func GetMetric(name string, m Metric) Metric {
	obj, loaded := metrics.LoadOrStore(name, m)
	if loaded {
		panic(fmt.Sprintf("called fastnodestatus.GetMetric on '%s' multiple times", name))
	}
	return obj.(Metric)
}
...
func GetCounter(name string) *Counter {
	...
	if loaded {
		panic(fmt.Sprintf("called fastnodestatus.GetCounter on '%s' multiple times", name))
	}
	return obj.(*Counter)
}
...
func GetBooleanDefault(name string, defaultValue bool) *Boolean {
	...
	if loaded {
		panic(fmt.Sprintf("called fastnodestatus.GetBooleanDefault on '%s' multiple times", name))
	}
	return obj.(*Boolean)
}
