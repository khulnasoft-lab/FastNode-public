...
var ClusterListRepresentation = function() {
  ...
  var drawSuggestedQueries = function(appendToObject) {
    ...
    $.ajax({
      url: 'http://curation.khulnasoft.com:8080/suggestions?query=' + urlParams.query,
      type: 'POST',
      async: false,
    })
    .done(function(response) {
      ...
      for (var i = 0; i < response.length; i++) {
        $('<a href="http://curation.khulnasoft.com:8090/stackoverflow?query=' + response[i] +'" class="suggestedQuery" target="_blank">' + response[i] +'</a>').appendTo($queryContainer);
      }
