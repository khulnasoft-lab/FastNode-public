...
func makeFileURL(path string) (string, error) {
	...
	var suffix string
	if runtime.GOOS != "windows" {
		parts = strings.Split(path, "github.com/khulnasoft-lab/fastnode/")
		if len(parts) != 2 {
			return "", errors.New("bad file path")
		}
		suffix = parts[1]
	} else {
		parts = strings.Split(path, "github.com\\khulnasoft-lab\\khulnasoft-lab\\")
		if len(parts) != 2 {
			return "", errors.New("bad file path")
		}
		suffix = filepath.ToSlash(parts[1])
	}
	return fmt.Sprintf("https://github.com/khulnasoft-lab/fastnode/blob/master/%s", suffix), nil
}
