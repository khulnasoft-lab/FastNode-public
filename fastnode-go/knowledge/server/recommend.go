...
import (
	...
	errors "github.com/khulnasoft-lab/fastnode/fastnode-golib/errors"
...
func (a *App) setupRecommender(paths PathConfig) error {
	...
	s, err := git.NewStorage(git.StorageOptions{
		...
		Path: filepath.Join(
			...
			"fastnode-go", "knowledge", "git-cache.json",
		),
	})
	if err != nil {
		return err
	}
	a.recommender, err = recommend.NewRecommender(fastnodectx.Background(), opts, ignorer, s)
	if err != nil {
		return err
	}
...
func (s *Server) MakeRecommendDisplay(request recommend.Request) (RecommendDisplay, error) {
	...
	if s.app.index.checkIsCode(request.Location.CurrentPath) {
		display.IsCode = true
		files, err := s.app.recommender.Recommend(fastnodectx.Background(), request)
		if err != nil {
			return RecommendDisplay{}, err
		}
		...
		recs, err := s.app.recommender.RecommendBlocks(fastnodectx.Background(), blockRequest)
		if err != nil {
			return RecommendDisplay{}, err
		}
