...
import (
	...
	"github.com/khulnasoft-lab/fastnode/fastnode-go/navigation/offline/validation"
...
func (a *App) addFile(file recommend.File, pull Pull) (Base, error) {
	...
	files, err := a.recommender.Recommend(fastnodectx.Background(), request)
	if err != nil {
		return Base{}, err
	}
	...
	recs, err := a.recommender.RecommendBlocks(fastnodectx.Background(), blockRequest)
	if err != nil {
		return Base{}, err
	}
