...
func TestJavascript_CuratedContext(t *testing.T) {
	tcs := []curatedContextTC{
		...
		enableFastnodeGo = () => {
		  this.props.setFastnodeGoEnabled(true).then(() => {
		    track({ event: 'copilot_settings_fastnode_lexical_enabled' })
		      this.r$
		`,
			expected: data.Completion{
				Snippet: data.NewSnippet("restartFastnode"),
				Replace: data.Selection{Begin: -1, End: 0},
			},
		},
		curatedContextTC{
