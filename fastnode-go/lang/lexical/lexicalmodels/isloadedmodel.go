...
func (m *model) PredictChan(ctx fastnodectx.Context, i predict.Inputs) (chan predict.Predicted, chan error) {
	pchan, echan := m.ModelBase.PredictChan(ctx, i)
	fwdPred := make(chan predict.Predicted, cap(pchan))

	return fwdPred, fastnodectx.Go(func() error {
		defer close(fwdPred)
...
func emit(ctx fastnodectx.Context, first predict.Predicted, rest chan predict.Predicted, to chan predict.Predicted) {
	to <- first
	for {
		select {
		case <-ctx.AbortChan():
