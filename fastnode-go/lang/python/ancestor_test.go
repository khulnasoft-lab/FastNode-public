...
func assertAncestors(t *testing.T, graph *pythonimports.Graph, manager pythonresource.Manager, files map[string]string, addr pythontype.Address, expected []ancestorTestData) {
	...
	val, err := st.FindValue(fastnodectx.Background(), addr.File, addr.Path.Parts)
	require.NoError(t, err)
	...
	actual := editor.renderValueAncestors(fastnodectx.Background(), vb)
...
func TestAncestorsEdgeCases(t *testing.T) {
	...
	assert.Nil(t, editor.renderValueAncestors(fastnodectx.Background(), valueBundle{}))
