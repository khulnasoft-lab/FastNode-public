...
func newBufferIndex(ctx fastnodectx.Context, resolved *pythonanalyzer.ResolvedAST, buf []byte, filepath string) *bufferIndex {
	ctx.CheckAbort()
...
func (b *bufferIndex) FindValue(ctx fastnodectx.Context, file string, path []string) (pythontype.Value, error) {
	if file == b.filepath {
...
func (b *bufferIndex) FindSymbol(ctx fastnodectx.Context, file string, path []string, attr string) (pythontype.Value, pythontype.Value, error) {
	if file == b.filepath {
...
func (b *bufferIndex) ArgSpec(ctx fastnodectx.Context, v pythontype.Value) (*pythonimports.ArgSpec, error) {
	ctx.CheckAbort()
	id := pythonlocal.LookupID(v)
	if v == nil || id == "" {
		return nil, errNotFound
