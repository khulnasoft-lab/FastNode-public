...
type ContextInputs struct {
	...
	D io.Writer
	// EventSource is either the name of the editor that triggered the event or fastnoded
	EventSource string
...
func NewContext(ctx fastnodectx.Context, in ContextInputs) (*Context, error) {
	ctx.CheckAbort()
	...
	err := ctx.WithTimeout(in.ResolverTimeout, func(ctx fastnodectx.Context) (err error) {
		ctx.CheckAbort()
...
func (d *UnifiedDriver) updateContextWithSelection(ctx fastnodectx.Context, pyctx *Context, evt *event.Event, cursor int64) {
	ctx.CheckAbort()
	...
	default:
		// This index came from fastnode local. Compare build time to determine whether
	...
	var resolved *pythonanalyzer.ResolvedAST
	err := ctx.WithTimeout(pyctx.ResolverTimeout, func(ctx fastnodectx.Context) (err error) {
