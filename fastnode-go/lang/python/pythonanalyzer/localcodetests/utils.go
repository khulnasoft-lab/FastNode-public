...
func managerAdd(t testing.TB, batchOpts pythonbatch.Options, m *pythonbatch.BatchManager, path, src string) {
	...
	mod, _ := pythonparser.Parse(fastnodectx.Background(), []byte(src), parseOpts)
...
func assertResolveOpts(t *testing.T, opts opts) *pythonanalyzer.ResolvedAST {
	...
	if len(opts.localfiles) > 0 {
		...
		mgr := pythonbatch.NewBatchManager(fastnodectx.Background(), bi, batchOpts, nil)
		...
		batch, err := mgr.Build(fastnodectx.Background())
		require.NoError(t, err)

		flatsources, err := batch.Assembly.Sources.Flatten(fastnodectx.Background())
		require.NoError(t, err)
	...
	mod, err := pythonparser.Parse(fastnodectx.Background(), []byte(opts.src), parseOpts)
	require.NoError(t, err)
	...
	if sourceTree != nil {
		for _, ref := range result.References {
			...
			if !pythonenv.IsLocator(loc) {
				...
				var err error
				val, err = sourceTree.Locate(fastnodectx.Background(), loc)
				if err != nil {
					t.Errorf("got error %v while trying to locate %s (%v)", err, loc, ref)
					continue
				}
