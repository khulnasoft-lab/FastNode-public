...
func (r *Resolver) Resolve(module *pythonast.Module) (*ResolvedAST, error) {
	return r.ResolveContext(fastnodectx.Background(), module, false)
}

// ResolveContext computes the types for expressions in a syntax tree with a context
func (r *Resolver) ResolveContext(ctx fastnodectx.Context, module *pythonast.Module, allowValueMutation bool) (rast *ResolvedAST, err error) {
	ctx.CheckAbort()
