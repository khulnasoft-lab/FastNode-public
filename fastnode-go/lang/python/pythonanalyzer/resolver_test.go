...
func assertResolve(t *testing.T, src string, graph pythonresource.Manager, expected map[string]string) *ResolvedAST {
	...
	mod, err := pythonparser.Parse(fastnodectx.Background(), []byte(src), parseOpts)
...
`
	...
	mod, _ := pythonparser.Parse(fastnodectx.Background(), []byte(src), pythonparser.Options{
		Approximate: true,
	})
...
from fastnode import ranking, test as foo, nothing as bar
from fastnode.test import util, blah
from a import *
b
`
	infos := pythonresource.InfosFromKinds(map[string]pythonimports.Kind{
		"numpy": pythonimports.Module,
		"fastnode":  pythonimports.Module,
		"a":     pythonimports.Module,
	})

	graph := pythonresource.MockManager(t, infos, "numpy.ndarray", "fastnode.ranking", "fastnode.test.util", "a.b", "a.c")
	assertResolve(t, src, graph, map[string]string{
		"numpy":    "external:numpy:numpy",
		"np":       "external:numpy:numpy",
		"missing":  "unknown",
		"whatever": "unknown",
		"fastnode":     "external:fastnode:fastnode",
		"ranking":  "external:fastnode:fastnode.ranking",
		"test":     "external:fastnode:fastnode.test",
		"foo":      "external:fastnode:fastnode.test",
		"nothing":  "unknown",
		"bar":      "unknown",
		"util":     "external:fastnode:fastnode.test.util",
		"blah":     "unknown",
		"a":        "external:a:a",
		"b":        "external:a:a.b",
	})
