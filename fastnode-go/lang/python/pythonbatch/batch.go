...
var (
	...
	// DefaultLocalOptions is a set of reasonable defaults for analysis for fastnode local
...
func NewBatchManager(ctx fastnodectx.Context, bi BatchInputs, opts Options, buildDurations map[string]time.Duration) *BatchManager {
	ctx.CheckAbort()
...
func (b *BatchManager) Build(ctx fastnodectx.Context) (batch *Batch, err error) {
	ctx.CheckAbort()
...
func countValues(ctx fastnodectx.Context, counts map[string]int, val pythontype.Value, expr pythonast.Expr, src *SourceUnit) {
	ctx.CheckAbort()
