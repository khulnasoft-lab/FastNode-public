...
func managerAdd(t testing.TB, m *BatchManager, path, hash string, contents []byte) {
	...
	mod, _ := pythonparser.Parse(fastnodectx.Background(), contents, opts)
	require.NotNil(t, mod)
	m.Add(&SourceUnit{
		ASTBundle: pythonstatic.ASTBundle{AST: mod, Path: path, Imports: pythonstatic.FindImports(fastnodectx.Background(), path, mod)},
		Contents:  contents,
		Hash:      hash,
	})
}
...
func TestDefinitions(t *testing.T) {
	...
	manager := NewBatchManager(fastnodectx.Background(), bi, DefaultOptions, nil)
	managerAdd(t, manager, "/fastnode.py", "hash", mustReadFile("testdata/fastnode.py"))
	managerAdd(t, manager, "/man.py", "hash", mustReadFile("testdata/man.py"))

	results, err := manager.Build(fastnodectx.Background())
	...
	names := []string{
		locator("/fastnode.py", "Fastnode"),
		locator("/fastnode.py", "Fastnode.foo"),
		locator("/fastnode.py", "Fastnode.__init__"),
		locator("/man.py", "Man"),
		locator("/man.py", "Man.car"),
		locator("/man.py", "Man.__init__"),
		locator("/man.py", "print_code"),
		locator("/man.py", "some_fastnode"),
	}
...
func TestNameOverload(t *testing.T) {
	...
	managerAdd(t, manager, "/testdata/fastnode.py", "hash", mustReadFile("testdata/fastnode.py"))
	...
	res, _ := pythontype.AttrNoCtx(pkg, "fastnode")
	require.NotNil(t, res.Value())
	fastnode := res.Value()
	require.NotNil(t, fastnode)
	require.NotNil(t, fastnode)
	assert.Equal(t, fastnode.Kind(), pythontype.FunctionKind)
}
