...
func TestGetInSameDir(t *testing.T) {
	...
	actual, err := GetInSameDir(fastnodectx.Background(), fpath, files)
	require.NoError(t, err)
	require.Equal(t, expected, actual)
}

// https://github.com/khulnasoft-lab/fastnode/issues/11947
func TestSelectFiles_FilteredProjectDir(t *testing.T) {
	...
	projectDirs := findProjectDirs(fastnodectx.Background(), []string{projectDirPath}, localcode.LocalFileSystem{}, log.Printf)
