...
func testProviderPerformances(stats map[string]time.Duration, t *testing.T) {
	...
	if len(errors) > 0 {
		assert.FailNowf(t, "One (or more) provider failed performance test", "%s\n\nPlease fix them or update the content of the thresholds variable (in fastnode-go/lang/python/pythoncomplete/performancetest/performance_test.go)", strings.Join(errors, "\n"))
	}
}
