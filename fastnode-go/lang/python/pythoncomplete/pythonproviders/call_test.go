...
func requireTestCase(t *testing.T, src string) testCase {
	...
	inputs, err := NewInputs(fastnodectx.Background(), global, sb, false, false)
	require.NoError(t, err)
	return testCase{
		Orig:   src,
		Global: global,
...
func requireCompletions(t *testing.T, tc testCase, providers ...Provider) []MetaCompletion {
	...
	for _, p := range providers {
		err := p.Provide(fastnodectx.Background(), tc.Global, tc.Inputs, func(ctx fastnodectx.Context, sb data.SelectedBuffer, mc MetaCompletion) {
