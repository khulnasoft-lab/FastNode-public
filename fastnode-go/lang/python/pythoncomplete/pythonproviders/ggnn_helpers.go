...
func (g GGNNModelAccumulator) Provide(ctx fastnodectx.Context, global Global, in Inputs, out OutputFunc) error {
	...
	for len(workingQueue) > 0 {
		...
		err = provider.Provide(fastnodectx.Background(), global, inputs, func(ctx fastnodectx.Context, sb data.SelectedBuffer, mc MetaCompletion) {
			...
			if p := mc.GGNNMeta.Predictor; p != nil {
				...
				nextInputs, err := NewInputs(fastnodectx.Background(), global, nextBuffer, false, true)
				if err != nil {
					panic(err)
				}
