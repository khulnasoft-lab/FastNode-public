...
import (
	...
	pythonlang "github.com/khulnasoft-lab/fastnode/fastnode-go/lang/python"
...
const fastnodePlaceholder = "fastnode_placeholder_representation"
...
func (p Lexical) Provide(ctx fastnodectx.Context, g Global, in Inputs, out OutputFunc) error {
	...
	lexOut := func(c fastnodectx.Context, b data.SelectedBuffer, lexMc lexicalproviders.MetaCompletion) {
...
func newSemanticAnalyzer(c fastnodectx.Context, buf data.SelectedBuffer) (semanticAnalyzer, error) {
...
func (s semanticAnalyzer) filter(c fastnodectx.Context, b data.SelectedBuffer, completion data.Snippet) LexicalFiltersMeta {
...
func (s semanticAnalyzer) isNewName(expr pythonast.Expr) bool {
	...
	if nameExpr.Ident.Literal == fastnodePlaceholder {
		return false
	}
	return true
}
...
func represent(completion data.Snippet) string {
	...
	completion.Iterate(func(text string, ph bool) bool {
		...
		if ph {
			parts = append(parts, fastnodePlaceholder)
			return true
		}
...
func parse(c fastnodectx.Context, buffer data.SelectedBuffer) *pythonast.Module {
