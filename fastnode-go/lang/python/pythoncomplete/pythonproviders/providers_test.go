...
func (o testOutputs) add(ctx fastnodectx.Context, buf data.SelectedBuffer, compl MetaCompletion) {
	o[buf.Hash()] = append(o[buf.Hash()], compl)
}

func nopOutputFunc(fastnodectx.Context, data.SelectedBuffer, MetaCompletion) {
}
...
func runProviderWithOpts(t *testing.T, p Provider, template string, usePartialDecoder bool) (provisionResult, error) {
	...
	in, err := NewInputs(fastnodectx.Background(), global, buf, false, usePartialDecoder)
	require.NoError(t, err)

	out := make(testOutputs)
	err = p.Provide(fastnodectx.Background(), global, in, out.add)
