...
func extractPyQt4Class(module *Module, subModule string, doc *goquery.Document) {
	...
	details.Children().AddClass("fastnode_highlight")
	details.NextUntil("hr").AddClass("fastnode_highlight").Each(func(j int, g *goquery.Selection) {
		h, err := g.Html()
		if err != nil {
...
func extractDescriptions(doc *goquery.Document, header string) map[string]string {
	...
	subHeaders.Each(func(i int, s *goquery.Selection) {
		...
		s.NextUntil("hr").Each(func(j int, g *goquery.Selection) {
			...
			var description string
			g.NextUntil("h3").AddClass("fastnode_highlight").Each(func(j int, k *goquery.Selection) {
				h, err := k.Html()
				if err != nil {
