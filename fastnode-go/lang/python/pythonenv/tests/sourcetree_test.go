...
import (
	...
	"github.com/khulnasoft-lab/fastnode/fastnode-go/lang/python/pythontest"
...
func requireSourceTree(t *testing.T, manager pythonresource.Manager, files map[string]string) *pythonenv.SourceTree {
	...
	for file := range files {
		...
		val, err := st.FindValue(fastnodectx.Background(), "/", nil)
		if err != nil {
...
func requireValue(t *testing.T, st *pythonenv.SourceTree, addr pythontype.Address) pythontype.Value {
	addr.User = uid
	addr.Machine = mid
	val, err := st.Locate(fastnodectx.Background(), pythonenv.LocatorForAddress(addr))
	require.NoError(t, err)
...
func requireSymbol(t *testing.T, st *pythonenv.SourceTree, nsAddr pythontype.Address, attr string) (pythontype.Value, pythontype.Value) {
	...
	nsValTemp, attrVal, attrTemp, err := st.LocateSymbol(fastnodectx.Background(), pythonenv.SymbolLocator(nsVal, attr))
	require.NoError(t, err)
