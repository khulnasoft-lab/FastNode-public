...
`
	...
	model.Predict(fastnodectx.Background(), Input{RAST: requireRAST(t, file1)})
	...
	model.Predict(fastnodectx.Background(), Input{RAST: requireRAST(t, file2)})
	require.Equal(t, model.models[0].(*mockModel).getCalledCount("Predict"), 1)
...
func requireAST(t *testing.T, buf string) *pythonast.Module {
	...
	err = fastnodectx.Background().WithTimeout(time.Second, func(ctx fastnodectx.Context) error {
		var err error
