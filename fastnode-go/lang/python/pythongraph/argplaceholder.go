...
func NewArgPlaceholderTrainSample(config TrainConfig, params TrainParams, in ArgPlaceholderTrainInputs) (*InferProductionSample, error) {
	...
	site1, positiveSample := builder1.randomArgPlaceholderSite(fastnodectx.Background(), params.Rand, sym)
...
func (b *graphBuilder) randomArgPlaceholderSite(ctx fastnodectx.Context, rand *rand.Rand, sym pythonresource.Symbol) (placeholderSite, bool) {
	ctx.CheckAbort()
...
func modifyArgPlaceholderGraphWithScope(builder *graphBuilder, hops int, site placeholderSite, scope scope, stats *pythonresource.SigStats, contextNodes []*Node) {
	...
	if hops > 0 {
		...
		builder.Prune(fastnodectx.TODO(), keep, hops)
	}
}
