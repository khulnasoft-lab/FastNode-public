...
func reduceScopeForAttrBase(ctx fastnodectx.Context, a *analysis, scope scope) scope {
	return filterScopeByKinds(ctx, a, scope)
}

func (b *graphBuilder) ScopeForAttrBase(ctx fastnodectx.Context, node pythonast.Node) scope {
	return reduceScopeForAttrBase(ctx, b.a, b.vm.InScope(node, false))
}

func (b *graphBuilder) BuildAttrBaseSites(ctx fastnodectx.Context, sym pythonresource.Symbol) []*pythonast.AttributeExpr {
	ctx.CheckAbort()
...
func NewAttrBaseTrainSample(config TrainConfig, params TrainParams, in AttrBaseTrainInputs) (*InferNameSample, error) {
	...
	edited, label, err := func() ([]byte, string, error) {
		...
		attrs := builder.BuildAttrBaseSites(fastnodectx.Background(), sym)
		if len(attrs) == 0 {
	...
	a, err := analyze(fastnodectx.Background(), in.RM, edited)
	if err != nil {
	...
	nameSite, err := func() (nameSite, error) {
		scope := builder.ScopeForAttrBase(fastnodectx.TODO(), target)
