...
func NewAttributeTrainSample(config TrainConfig, params TrainParams, in AttributeTrainInputs) (*InferProductionSample, error) {
	...
	builder1 := newBuilder(fastnodectx.Background(), a1, false, true)
	...
	scope := builder2.ScopeForAttr(fastnodectx.TODO(), site2.expr)
	if len(scope) == 0 {
...
func (b *graphBuilder) ScopeForAttr(ctx fastnodectx.Context, node pythonast.Node) scope {
...
func (b *graphBuilder) randomAttributeSite(rand *rand.Rand, sym string, canonToSym map[string][]string) attributeSite {
	...
	match:
		...
		// see https://github.com/khulnasoft-lab/fastnode/issues/6636
