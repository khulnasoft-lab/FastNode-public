...
func NewCallTrainSample(config TrainConfig, params TrainParams, in CallTrainInputs) (*InferNameSample, error) {
	...
	call, arg, err := func() (callSite, callSiteArg, error) {
		...
		builder := newBuilder(fastnodectx.Background(), a, false, true)

		builder.BuildEdges(config.Graph.EdgeSet)

		// get valid call sites and select an argument
		calls := builder.BuildCallSites(fastnodectx.Background(), sym, patterns)
		if len(calls) == 0 {
	...
	arg, builder, newBuffer, err := func() (callSiteArg, *graphBuilder, []byte, error) {
		...
		a, err := analyze(fastnodectx.Background(), in.RM, newBuffer)
		if err != nil {
		...
		// a.SetResolved(fastnodectx.Background(), newCall, nil)

		builder := newBuilder(fastnodectx.Background(), a, false, true)

		builder.BuildEdges(config.Graph.EdgeSet)

		nameSite, err := builder.BuildNameSite(builder.ScopeForCall(fastnodectx.Background(), newCall), name)
		if err != nil {
...
func reduceScopeForCall(ctx fastnodectx.Context, a *analysis, s scope) scope {
	return filterScopeByKinds(ctx, a, s, pythontype.ModuleKind, pythontype.FunctionKind)
}

func (b *graphBuilder) ScopeForCall(ctx fastnodectx.Context, node pythonast.Node) scope {
	return reduceScopeForCall(ctx, b.a, b.vm.InScope(node, true))
}

func (b *graphBuilder) BuildCallSites(ctx fastnodectx.Context, canonical pythonresource.Symbol, patterns *traindata.CallPatterns) []callSite {
	ctx.CheckAbort()
	var calls []callSite
