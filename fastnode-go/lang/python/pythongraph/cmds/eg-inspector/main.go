...
func (a app) getCallModelCompletions(src, ordering string, filtering bool) (*returnedComp, int) {
	...
	inputs, err := pythonproviders.NewInputs(fastnodectx.Background(), global, sb, false, true)
	...
	err = provider.Provide(fastnodectx.Background(), global, inputs, func(ctx fastnodectx.Context, sb data.SelectedBuffer, mc pythonproviders.MetaCompletion) {
	...
	for len(workingQueue) > 0 {
		...
		err = provider.Provide(fastnodectx.Background(), global, inputs, func(ctx fastnodectx.Context, sb data.SelectedBuffer, mc pythonproviders.MetaCompletion) {
			...
			if p := mc.GGNNMeta.Predictor; p != nil {
				...
				nextInputs, err := pythonproviders.NewInputs(fastnodectx.Background(), global, nextBuffer, false, true)
				if err != nil {
					log.Println(err)
					return
				}
