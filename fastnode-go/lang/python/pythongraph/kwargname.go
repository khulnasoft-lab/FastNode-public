...
func NewKwargNameTrainSample(config TrainConfig, params TrainParams, in KwargNameTrainInputs) (*InferProductionSample, error) {
	...
	builder1 := newBuilder(fastnodectx.Background(), a, false, true)
	...
	site1 := builder1.randomKwargNameSite(fastnodectx.Background(), params.Rand, sym, in.Keywords)
	...
	a2, err := analyze(fastnodectx.Background(), in.RM, newSrc)
	if err != nil {
		return nil, err
	}

	builder2 := newBuilder(fastnodectx.Background(), a2, false, true)
	...
	scope := builder2.ScopeForCall(fastnodectx.TODO(), site2.call)
	if len(scope) == 0 {
	...
	if config.MaxHops > 0 {
		...
		builder2.Prune(fastnodectx.TODO(), keep, config.MaxHops)
	}
...
func (b *graphBuilder) randomKwargNameSite(ctx fastnodectx.Context, rand *rand.Rand, sym pythonresource.Symbol, keywords []string) kwargNameSite {
	ctx.CheckAbort()
