...
func parseWithCursor(t testing.TB, snippet string) (*pythonast.Module, token.Pos) {
	...
	mod, err := pythonparser.Parse(fastnodectx.Background(), []byte(src), pythonparser.Options{})
...
func doDeepestNameTest(t testing.TB, snip string, expected string) {
	...
	node := DeepestContainingSelection(fastnodectx.Background(), mod, int64(start), int64(end))
