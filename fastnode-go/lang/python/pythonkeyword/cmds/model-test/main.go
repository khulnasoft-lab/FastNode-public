...
func newResult(ex example, model *pythonkeyword.Model, compStruct *comparisonStruct) (result, error) {
	...
	ast, _ := pythonparser.ParseWords(fastnodectx.Background(), src, words, parseOpts)
	if ast == nil {
	...
	features, err := pythonkeyword.NewFeatures(fastnodectx.Background(), modelInputs, pythonkeyword.ModelLookback)
	if err != nil {
...
func main() {
	...
	_ = ioutil.WriteFile("/data/fastnode/keywords_model/result_golang.json", content, 0644)
