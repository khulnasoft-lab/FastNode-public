...
func requireLexParse(t *testing.T, src []byte, cursor int64) ([]pythonscanner.Word, *pythonast.Module) {
	...
	mod, _ := pythonparser.ParseWords(fastnodectx.Background(), src, words, pythonparser.Options{
		Approximate: true,
		Cursor:      &cursorPos,
	})
...
func assertFeatures(t *testing.T, tstSrc string, expected Features) {
	...
	features, err := NewFeatures(fastnodectx.Background(), inputs, ModelLookback)
	require.NoError(t, err)
