...
// ReferenceResolutionComparison compare the resolution level of a reference between IntelliJ and Fastnode
type ReferenceResolutionComparison struct {
	intelliJResolutionLevel ResolutionLevel
	fastnodeResolutionLevel     ResolutionLevel
}

func (comp ReferenceResolutionComparison) String() string {
	delta := comp.fastnodeResolutionLevel - comp.intelliJResolutionLevel
	if delta > 0 {
		return "%%% Fastnode wins : " + comp.fastnodeResolutionLevel.String() + " > " + comp.intelliJResolutionLevel.String()
	} else if delta < 0 {
		return "### IntelliJ wins : " + comp.intelliJResolutionLevel.String() + " > " + comp.fastnodeResolutionLevel.String()
	} else {
		return "••• Same resolution level : " + comp.fastnodeResolutionLevel.String()
	}
}
...
type ReferenceInfo struct {
	...
	FoundInFastnode   bool
}

// ReferenceComparisonOnline contains all the details of a comparison of a reference resolution between Fastnode and IntelliJ
...
type ReferenceComparisonOnline struct {
	...
	FastnodeSymbolResolved      bool            `json:"fastnode_symbol_resolved"`
	FastnodeResolutionLevel     ResolutionLevel `json:"fastnode_resolution_level"`
	FastnodeASTType             string          `json:"fastnode_ast_type"`
	FastnodeValueType           string          `json:"fastnode_value_type"`
}
...
type ReferenceComparison struct {
	...
	End          token.Pos                 `json:"end"`
	FastnodeValue    string                    `json:"fastnode_value"`
	FastnodeSymbol   string                    `json:"fastnode_symbol"`
...
// ReferenceMap is an alias for the map used in the matching algorithm (to match a reference between IntelliJ and Fastnode)
...
// The offset needs to be converted from runes (intelliJ offset) to bytes (Fastnode offset)
