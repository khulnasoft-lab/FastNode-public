...
func ApproxParse(src []byte, opts Options) (*pythonast.Module, error) {
	...
	return Parse(fastnodectx.Background(), src, opts)
}
...
NameExpr[a]
	...
	dotExprs := extractDotExprs(fastnodectx.Background(), words, []byte(src[2:]), 2)
...
`,
	...
	dotExprs := extractDotExprs(fastnodectx.Background(), words, []byte(src), 0)
...
func TestDotExprBegin(t *testing.T) {
	...
	dotExprs := extractDotExprs(fastnodectx.Background(), words, []byte(src[3:]), 3)
...
`
	...
	calls := extractFunctionCalls(fastnodectx.Background(), words, []byte(src[2:]), 2)
...
func TestNotFunction(t *testing.T) {
	src := `a`
	words, _ := pythonscanner.Lex([]byte(src), pythonscanner.Options{})
	calls := extractFunctionCalls(fastnodectx.Background(), words, []byte(src), 0)
	assert.Nil(t, calls)
}
...
AssignStmt
	...
	returns := extractReturnAssignments(fastnodectx.Background(), words, []byte(src[4:]), 4)
...
AssignStmt
	...
	returns := extractReturnAssignments(fastnodectx.Background(), words, []byte(src), 0)
...
`
	...
	a := parseDotExpr(fastnodectx.Background(), []byte(src), 0)
...
func TestFunctionNonNameKeywordArg(t *testing.T) {
	// Issue https://github.com/khulnasoft-lab/fastnode/issues/7528
	src := `
x.fn(self.v=1)
`
	expected := `
...
`}
	...
	words, _ := pythonscanner.Lex([]byte(src), pythonscanner.Options{})
	classes := extractNamedDefinitions(fastnodectx.Background(), words, []byte(src), 0)
...
`}
	...
	words, _ := pythonscanner.Lex([]byte(src), pythonscanner.Options{})
	funcs := extractNamedDefinitions(fastnodectx.Background(), words, []byte(src), 0)
...
`}
	...
	words, _ := pythonscanner.Lex([]byte(src), pythonscanner.Options{})
	ifs := extractKeywordStatements(fastnodectx.Background(), words, []byte(src), 0)
...
`}
	...
	words, _ := pythonscanner.Lex([]byte(src), pythonscanner.Options{})
	withs := extractKeywordStatements(fastnodectx.Background(), words, []byte(src), 0)
...
`}
	...
	words, _ := pythonscanner.Lex([]byte(src), pythonscanner.Options{})
	whiles := extractKeywordStatements(fastnodectx.Background(), words, []byte(src), 0)
