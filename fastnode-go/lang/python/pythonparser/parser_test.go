...
func assertParse(t *testing.T, expected string, src string) {
	t.Log(src)
	mod, err := Parse(fastnodectx.Background(), []byte(src), opts)
...
Module
	...
	t.Log(src)
	mod, err := Parse(fastnodectx.Background(), []byte(src), Options{
		ErrorMode: Recover,
	})
	assert.NotNil(t, err)
...
Module
	BadStmt
`
	mod, _ := Parse(fastnodectx.Background(), []byte(src), Options{
		ErrorMode: Recover,
	})
...
async if x:
    pass
`
	_, err := Parse(fastnodectx.Background(), []byte(src), opts)
	require.Error(t, err)
}
...
func TestNonNameKeywordArg(t *testing.T) {
	// Bug from issue https://github.com/khulnasoft-lab/fastnode/issues/7528
	src := `
x.fn(self.v=1)
`
	_, err := Parse(fastnodectx.Background(), []byte(src), opts)
