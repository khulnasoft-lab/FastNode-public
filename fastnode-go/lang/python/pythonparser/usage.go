...
func MarkUsages(ctx fastnodectx.Context, node pythonast.Node) {
...
func mark(ctx fastnodectx.Context, usage pythonast.Usage, node pythonast.Node) {
	v := usageVisitors.Get().(*usageVisitor)
...
type usageVisitor struct {
	ctx fastnodectx.Context // TODO(naman) avoid storing the fastnodectx here by threading it through pythonast.Walk
	cur pythonast.Usage
}
