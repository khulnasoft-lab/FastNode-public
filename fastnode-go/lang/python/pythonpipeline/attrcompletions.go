...
func ExprAttributeSituations(s pipeline.Sample) pipeline.Sample {
	...
	sym, err := python.GetExternalSymbol(fastnodectx.Background(), expr.AnalyzedEvent.Context.Importer.Global, parentVal)
	if err != nil {
...
func AttributeSituationsAllowedByModel(model pythonexpr.Model) transform.IncludeFn {
	return func(s pipeline.Sample) bool {
		...
		sym, err := python.GetExternalSymbol(fastnodectx.Background(), rm, parentVal)
		if err != nil {
			return false
		}
...
func PopAttributeCompletions(s pipeline.Sample) pipeline.Sample {
	...
	inps, err := pythonproviders.NewInputsFromPyCtx(fastnodectx.Background(), global, data.NewBuffer(string(sit.AnalyzedEvent.Context.Buffer)).Select(data.Cursor(int(sit.AnalyzedEvent.Context.Cursor))), false, sit.AnalyzedEvent.Context, false)
	if err != nil {
		panic(err)
	}
	pythonproviders.Attributes{UseDefaultReferences: true}.Provide(fastnodectx.Background(), global, inps,
		func(_ fastnodectx.Context, _ data.SelectedBuffer, c pythonproviders.MetaCompletion) {
...
func GGNNAttributeCompletions(recreator *servercontext.Recreator, models *pythonmodels.Models, tm TrimMethod, inferOnName bool, localIndex bool) transform.OneInOneOutFn {
	return func(s pipeline.Sample) pipeline.Sample {
		...
		err = fastnodectx.Background().WithTimeout(3*time.Second, func(ctx fastnodectx.Context) error {
