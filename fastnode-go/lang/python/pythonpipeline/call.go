...
func ModelCanPredictCall(ctx fastnodectx.Context, rm pythonresource.Manager, model pythonexpr.Model, rast *pythonanalyzer.ResolvedAST, call *pythonast.CallExpr) bool {
...
func CallArgSituationsAllowedByModel(model pythonexpr.Model) transform.IncludeFn {
	return func(s pipeline.Sample) bool {
		...
		return ModelCanPredictCall(fastnodectx.Background(), rm, model, situation.AnalyzedEvent.Context.Resolved, situation.CallExpr)
	}
}
