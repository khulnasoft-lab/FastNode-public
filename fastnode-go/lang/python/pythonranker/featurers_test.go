...
var (
	keywords = []string{"fastnodeman", "superman", "spiderman", "batman", "fastnodeman"}
...
func TestKeywordMatcherMatch(t *testing.T) {
	km := newKeywordMatcher(keywords)
	score, matched := km.match([]string{"fastnodeman"})

	assert.EqualValues(t, 1, score)
	assert.Equal(t, struct{}{}, matched["fastnodeman"])
...
func TestFuzzyKeywordMatcher(t *testing.T) {
	...
	score, matched := km.match([]string{"man", "men", "fastnode"})
