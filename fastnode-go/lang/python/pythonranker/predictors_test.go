...
func TestNewLmMethodPredictor(t *testing.T) {
	...
	for i, name := range mp.MethodNames {
		act := mp.MethodLMs[i].LogLikelihood([]string{"fastnode"})
		var exp float64
		switch name {
		case "fastnodeman":
	...
	act := mp.BackgroundLM.LogLikelihood([]string{"fastnode"})
...
func TestLmMethodPredictorScore(t *testing.T) {
	...
	scores = mp.Scores([]string{"fastnode"})

	bg := (1 + 1/float64(wordVecLen)) / 9
	fastnode := (1 + 1/float64(wordVecLen)) / 3
	...
	for _, ms := range scores {
		...
		switch ms.Name {
		case "fastnodeman":
			exp = (0.5*bg + 0.5*fastnode) / (4*0.5*bg + 0.5*fastnode + 0.5*3*others)
		default:
			exp = (0.5*bg + 0.5*others) / (4*0.5*bg + 0.5*fastnode + 0.5*3*others)
		}
		if math.Abs(exp-ms.Score) > 1e-6 {
			t.Errorf("expected %f, but got %f\n", exp, ms.Score)
		}
