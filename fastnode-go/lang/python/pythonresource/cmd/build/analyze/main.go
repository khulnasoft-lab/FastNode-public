...
import (
	...
	filehelpers "github.com/khulnasoft-lab/fastnode/local-pipelines/python-import-exploration/helpers"
...
func loadAST(ctx fastnodectx.Context, path string, pseudopath string) (pythonstatic.ASTBundle, error) {
	f, err := os.Open(path)
	fail(err)
	defer f.Close()
...
func analyze(ctx fastnodectx.Context, graph pythonresource.Manager, privateImports bool, roots ...string) (*pythonenv.SourceTree, error) {
	ctx.CheckAbort()
...
func analyzePip(ctx fastnodectx.Context, graph pythonresource.Manager, reqStr string, resC chan<- buildResult) {
	ctx.CheckAbort()
...
func analyzeTypeshed(ctx fastnodectx.Context, graph pythonresource.Manager, typeshedRoot string, resC chan<- buildResult) {
	ctx.CheckAbort()
...
func build(cmd *cobra.Command, args []string) {
	...
	if typeshedRoot != "" {
		jobs = append(jobs, func() error { analyzeTypeshed(fastnodectx.Background(), rm, typeshedRoot, resC); return nil })
	}
	for _, reqStr := range reqStrs {
		ctx := fastnodectx.Background()
		closeOver := reqStr
		jobs = append(jobs, func() error {
			err := ctx.WithTimeout(analysisTimeout, func(ctx fastnodectx.Context) (err error) {
				ctx.CheckAbort()
