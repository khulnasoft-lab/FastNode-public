...
import (
	...
	"github.com/khulnasoft-lab/fastnode/fastnode-go/dynamicanalysis"
	...
	"github.com/khulnasoft-lab/fastnode/fastnode-go/lang/python/pythonresource/cmd/build/helpers"
	"github.com/khulnasoft-lab/fastnode/fastnode-go/lang/python/pythonresource/cmd/build/helpers/rettypes"
	"github.com/khulnasoft-lab/fastnode/fastnode-go/lang/python/pythonresource/internal/builder"
	"github.com/khulnasoft-lab/fastnode/fastnode-go/lang/python/pythonresource/internal/distidx"
	"github.com/khulnasoft-lab/fastnode/fastnode-go/lang/python/pythonresource/internal/manifest"
	"github.com/khulnasoft-lab/fastnode/fastnode-go/lang/python/pythonresource/internal/resources/returntypes"
...
var (
	dynamicUsagesPath      = "s3://fastnode-data/dynamic-usages/20190320.json.gz"
	...
	for _, tlSym := range analyzed.ListAbs("/") {
		...
		for len(q) > 0 {
			...
			case *pythontype.SourceFunction:
				...
				syms, err := rm.PathSymbols(fastnodectx.Background(), path)
				if err != nil {
				...
				for _, retType := range pythontype.DisjunctsNoCtx(pythontype.TranslateNoCtx(ret.Type(), rm)) {
					...
					retSyms, _ := rm.PathSymbols(fastnodectx.Background(), retPath)
					if err != nil {
...
func addFromDynamicReturnTypes(rm pythonresource.Manager, out map[keytypes.Distribution]returntypes.Entities) {
	...
	for funcPath, retPaths := range types {
		funcSyms, err := rm.PathSymbols(fastnodectx.Background(), pythonimports.NewDottedPath(funcPath))
		if err != nil {
...
func addFromDynamicUsages(rm pythonresource.Manager, out map[keytypes.Distribution]returntypes.Entities) {
	fail(serialization.Decode(dynamicUsagesPath, func(u *dynamicanalysis.Usage) {
		...
		funcSyms, err := rm.PathSymbols(fastnodectx.Background(), funcPath)
		if err != nil {
...
func init() {
	cmd.Flags().StringVarP(&manifestPath, "graph", "g", "", "symbol graph manifest path (defaults to compiled-in FastnodeManifest)")
	cmd.Flags().StringVarP(&distidxPath, "distidx", "d", "", "distribution index path (defaults to compiled-in FastnodeIndex)")
