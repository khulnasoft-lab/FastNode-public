...
// see TestFastnodeManifest for actually testing the underlying data is reasonable
...
// TestFastnodeManifest tests actual manifest data from the Fastnode manifest (? or maybe a fixturized manifest)
func TestFastnodeManifestDistributions(t *testing.T) {
	var distNames []string
	for _, dist := range manifest.FastnodeManifest.Distributions() {
...
func TestBindata(t *testing.T) {
	...
	manifestPath := path.Join(gopath, "src/github.com/khulnasoft-lab/fastnode/fastnode-go/lang/python/pythonresource/manifest.json")
