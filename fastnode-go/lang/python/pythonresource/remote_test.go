...
func Test_ErrorManager(t *testing.T) {
	...
	for i := 0; i < 5; i++ {
		...
		mgr.PathSymbols(fastnodectx.Background(), pythonimports.NewPath("json", "dumps"))
...
func testWithManager(t *testing.T, mgr Manager, errorCount func() int) {
	...
	allJSONDumps, err := mgr.PathSymbols(fastnodectx.Background(), pythonimports.NewPath("json", "dumps"))
	require.NoError(t, err)
	...
	// canonical symbols
	// this is a very expensive operation, which doesn't seem to be used by fastnoded's features
