...
import (
	...
	"github.com/khulnasoft-lab/fastnode/fastnode-go/lang/python/pythonresource/internal/toplevel"
...
func (rm *manager) PathSymbol(path pythonimports.DottedPath) (Symbol, error) {
	syms, err := rm.PathSymbols(fastnodectx.TODO(), path)
	if err != nil {
		return Symbol{}, err
	}
	return syms[0], nil
...
func (rm *manager) PathSymbols(ctx fastnodectx.Context, path pythonimports.DottedPath) ([]Symbol, error) {
	var syms []Symbol
	err := ctx.WithCallLimit(maxRecursionDepth, func(ctx fastnodectx.CallContext) error {
		var err error
...
func (rm *manager) NewSymbol(dist keytypes.Distribution, path pythonimports.DottedPath) (Symbol, error) {
	var sym Symbol
	err := fastnodectx.TODO().WithCallLimit(maxRecursionDepth, func(ctx fastnodectx.CallContext) error {
		var err error
...
func (rm *manager) pathSymbols(ctx fastnodectx.CallContext, path pythonimports.DottedPath) ([]Symbol, error) {
	if ctx.AtCallLimit() {
...
func (rm *manager) canonicalize(ctx fastnodectx.CallContext, sym keytypes.Symbol) (Symbol, error) {
	if ctx.AtCallLimit() {
