...
func assertAssemblerBatchWithMissing(
	...
	for name, src := range srcs {
		ast, err := pythonparser.Parse(fastnodectx.Background(), []byte(src), pythonparser.Options{})
		require.NoError(t, err)
		assembler.AddSource(ASTBundle{AST: ast, Path: name, Imports: FindImports(fastnodectx.Background(), name, ast)})
	}

	var buf bytes.Buffer
	assembler.SetTrace(&buf)
	result, err := assembler.Build(fastnodectx.Background())
...
func assertBatchTypesWithMissing(t *testing.T,
	...
	assembler := NewAssembler(fastnodectx.Background(), ai, opts)
...
`
	...
	assert.False(t, res1.Found())
	assert.Equal(t, fastnodectx.ContextExpiredError{Err: fastnodectx.CallLimitError{}}, err)
...
func TestAssembler_Cancel(t *testing.T) {
	...
	assembler := NewAssembler(fastnodectx.Background(), ai, DefaultOptions)
...
`
	...
	ast, err := pythonparser.Parse(fastnodectx.Background(), []byte(src), pythonparser.Options{})
	require.NoError(t, err)
	assembler.AddSource(ASTBundle{AST: ast, Path: "/test.py", Imports: FindImports(fastnodectx.Background(), "/test.py", ast)})

	err = fastnodectx.Background().WithCancel(func(ctx fastnodectx.Context, cancel fastnodectx.CancelFunc) error {
		cancel()
		ctx.WaitExpiry(t)
		assembler.Build(ctx)
		return nil
