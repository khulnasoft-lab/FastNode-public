...
func (e estimator) propagateAlongEdge(start, end *pythontype.Symbol) {
	...
	for _, cap := range capabilities {
		...
		if !found {
			...
			for _, class := range e.assembly.Classes {
				if _, err := pythontype.Attr(fastnodectx.TODO(), class, cap.Attr); err == nil {
	...
	proposals := pythontype.Disjuncts(fastnodectx.TODO(), end.Value)
	...
	if len(accepted) > 0 {
		end.Value = pythontype.Unite(fastnodectx.TODO(), accepted...)
	}
}

func supportsCapabilities(v pythontype.Value, caps []Capability) bool {
	for _, c := range caps {
		if _, err := pythontype.Attr(fastnodectx.TODO(), v, c.Attr); err != nil {
			return false
		}
	}
	return true
