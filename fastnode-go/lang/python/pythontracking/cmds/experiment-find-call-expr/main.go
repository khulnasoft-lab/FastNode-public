...
func process(meta analyze.Metadata, track *pythontracking.Event) {
	...
	mod, _ := pythonparser.ParseWords(fastnodectx.Background(), buffer, incrLexer.Words(), parseOpts)

	trackingURL := fmt.Sprintf("http://test-6.khulnasoft.com:3031/event?uri=%s&message_id=%s", url.QueryEscape(meta.ID.URI), url.QueryEscape(meta.ID.ID))
	log.Printf("LIVE_FAILURE %s %s", track.Callee.Failure, trackingURL)
	if mod == nil {
		log.Printf("PARSE_FAILURE %s", trackingURL)
	} else {
		callExpr, outsideParens, _ := python.FindCallExpr(fastnodectx.Background(), mod, buffer, int64(cursor))
		if callExpr == nil {
