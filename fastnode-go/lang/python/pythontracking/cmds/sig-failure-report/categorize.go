...
func (a analyzer) catUnresolved(id analyze.MessageID, track *pythontracking.Event, ctx *python.Context) *category {
	callExpr, _, inBad := python.FindCallExpr(fastnodectx.Background(), ctx.AST, ctx.Buffer, ctx.Cursor)
	if callExpr == nil {
	...
	if name, ok := cur.(*pythonast.NameExpr); ok {
		...
		if v, ok := ctx.Importer.ImportAbs(fastnodectx.Background(), lit); ok && v != nil {
