...
func MostSpecific(ctx fastnodectx.Context, v Value) Value {
	ctx.CheckAbort()
	if _, ok := v.(Union); !ok {
...
func ChooseExternal(ctx fastnodectx.Context, rm pythonresource.Manager, val Value) (pythonresource.Symbol, error) {
	ctx.CheckAbort()
...
func AllExternals(ctx fastnodectx.Context, rm pythonresource.Manager, val Value) []pythonresource.Symbol {
	ctx.CheckAbort()
	if val == nil {
		return nil
	}
