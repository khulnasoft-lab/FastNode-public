...
import "github.com/khulnasoft-lab/fastnode/fastnode-golib/fastnodectx"
...
func WidenConstants(v Value) Value {
	...
	// TODO(naman) we should either accept a fastnodectx.Context here, or, preferably,
	...
	// the Union case here non-recursively, with no need for a fastnodectx.Context.
	...
	case Union:
		var wide []Value
		for _, vi := range Disjuncts(fastnodectx.TODO(), v) {
			wide = append(wide, WidenConstants(vi))
		}
		return Unite(fastnodectx.TODO(), wide...)
	default:
		return v
	}
}
...
func pyEnumerate(args Args) Value {
	...
	if start, found := args.Keyword("start"); found {
		index = Unite(fastnodectx.TODO(), index, WidenConstants(start))
	}
...
func pyZip(args Args) Value {
	...
	if args.HasVararg {
		...
		//      list<list< unknown >>
		return NewList(NewList(Unite(fastnodectx.TODO(), elemTypes...)))
	}
...
func pyGetattr(args Args) Value {
	...
	for _, vi := range Disjuncts(fastnodectx.TODO(), attr) {
		if vi, ok := vi.(StrConstant); ok {
			if res, _ := Attr(fastnodectx.TODO(), obj, string(vi)); res.Found() {
...
func pyNext(args Args) Value {
	...
	v := seq.Elem()
	if defaultVal, ok := args.Keyword("default"); ok {
		v = Unite(fastnodectx.TODO(), v, defaultVal)
	} else if len(args.Positional) >= 2 {
		v = Unite(fastnodectx.TODO(), v, args.Positional[1])
	}
	return v
}
