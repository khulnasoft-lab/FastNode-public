...
func (v *KwargDict) Add(name string, value Value) {
	v.m.Lock()
	defer v.m.Unlock()
	v.Entries[name] = Unite(fastnodectx.TODO(), v.Entries[name], value)
}
...
func (v *KwargDict) attr(ctx fastnodectx.CallContext, name string) (AttrResult, error) {
	ctx.CheckAbort()
...
func (v *KwargDict) equal(ctx fastnodectx.CallContext, u Value) bool {
...
func (v *KwargDict) hash(ctx fastnodectx.CallContext) FlatID {
...
func (v *KwargDict) pyGet(args Args) Value {
	...
	v.m.RLock()
	defer v.m.RUnlock()
	return Unite(fastnodectx.TODO(), v.Entries[string(key)], defaultVal)
}
