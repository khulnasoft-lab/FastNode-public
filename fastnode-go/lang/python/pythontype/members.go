...
func Members(ctx fastnodectx.Context, rm pythonresource.Manager, val Value) map[string]Value {
	...
	if typ != nil {
		...
		ctx.WithCallLimit(100, func(ctx fastnodectx.CallContext) error {
...
func addValueMembers(ctx fastnodectx.CallContext, rm pythonresource.Manager, val Value, members map[string]Value) {
	if ctx.AtCallLimit() {
		return
	}
