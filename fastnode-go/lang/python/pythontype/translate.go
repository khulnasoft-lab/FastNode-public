...
func TranslateNoCtx(val Value, graph pythonresource.Manager) Value {
	return Translate(fastnodectx.Background(), val, graph)
}

// Translate translates a Value into a GlobalValue, SourceValue, ConstantValue, or a Union thereof.
func Translate(ctx fastnodectx.Context, val Value, graph pythonresource.Manager) Value {
	var res Value
	ctx.WithCallLimit(20, func(ctx fastnodectx.CallContext) error {
...
func translate(ctx fastnodectx.CallContext, val Value, graph pythonresource.Manager) Value {
