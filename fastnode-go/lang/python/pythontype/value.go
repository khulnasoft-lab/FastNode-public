...
type Value interface {
	...
	hash(ctx fastnodectx.CallContext) FlatID
	equal(fastnodectx.CallContext, Value) bool
	attr(fastnodectx.CallContext, string) (AttrResult, error)
}
...
type Mutable interface {
	...
	AttrSymbol(ctx fastnodectx.Context, attr string, create bool) []*Symbol
}

// AttrNoCtx is equivalent to Attr
func AttrNoCtx(v Value, name string) (AttrResult, error) {
	return Attr(fastnodectx.Background(), v, name)
}

// Attr gets the named attribute Value of v, representing `{v}.{name}` in Python
// WARNING:
//  - The semantics are currently unclear around the return values of this function (SEE https://github.com/khulnasoft-lab/fastnode/issues/9393),
//    until this is resolved the best way to check if an attribute was sucessfully found is to check `res.Found()`.
func Attr(ctx fastnodectx.Context, v Value, name string) (res AttrResult, err error) {
	err = ctx.WithCallLimit(maxSteps, func(ctx fastnodectx.CallContext) (err error) {
...
func attr(ctx fastnodectx.CallContext, v Value, name string) (AttrResult, error) {
	ctx.CheckAbort()
	if v == nil {
...
func EqualNoCtx(u, v Value) bool {
	return Equal(fastnodectx.Background(), u, v)
}
...
func Equal(ctx fastnodectx.Context, u, v Value) bool {
	var res bool
	err := ctx.WithCallLimit(maxSteps, func(ctx fastnodectx.CallContext) error {
		res = equal(ctx, u, v)
		return nil
	})
	if err != nil {
...
func equal(ctx fastnodectx.CallContext, u, v Value) bool {
	ctx.CheckAbort()
	if u == nil && v == nil {
		return true
	}
...
func hash(ctx fastnodectx.CallContext, v Value) FlatID {
	ctx.CheckAbort()
	if v == nil {
		return 0
	}
...
func Hash(ctx fastnodectx.Context, v Value) (FlatID, error) {
	if v == nil {
		return 0, nil
	}

	var h FlatID
	err := ctx.WithCallLimit(50, func(ctx fastnodectx.CallContext) error {
		h = hash(ctx, v)
		return nil
	})
	if err != nil {
...
func MustHash(v Value) FlatID {
	h, err := Hash(fastnodectx.Background(), v)
	if err != nil {
		panic(err)
	}
	return h
