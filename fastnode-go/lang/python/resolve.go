...
func sourceValueNamespace(ctx fastnodectx.Context, idx *pythonlocal.SymbolIndex, val pythontype.SourceValue) pythontype.Value {
	ctx.CheckAbort()
...
func resolveName(ctx fastnodectx.Context, inputs resolveInputs, name *pythonast.NameExpr) []symbolBundle {
	...
	// https://github.com/khulnasoft-lab/fastnode/issues/5743
...
func resolveAttr(ctx fastnodectx.Context, inputs resolveInputs, attr *pythonast.AttributeExpr) []symbolBundle {
	safePrint := inputs.print
...
func resolveNode(ctx fastnodectx.Context, node pythonast.Node, inputs resolveInputs) (string, []symbolBundle, error) {
	var nodeType string
	var sbs []symbolBundle
