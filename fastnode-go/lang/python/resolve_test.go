...
import (
	...
	"github.com/khulnasoft-lab/fastnode/fastnode-go/lang/python/testcorpus"
...
// TODO(naman) eventually, in Fastnode Local world, we should enforce consistency guarantees about the up-to-date-ness of the buffer,
...
func hackNonUnionsEqual(path string, u, v pythontype.Value) bool {
	...
	return pythontype.Equal(fastnodectx.Background(), u, v)
}
...
func hackValuesEqual(path string, u, v pythontype.Value) bool {
	uVals := pythontype.Disjuncts(fastnodectx.Background(), u)
	vVals := pythontype.Disjuncts(fastnodectx.Background(), v)
