...
func DoTest(t *testing.T, subdir string, check func(builder *pythonbatch.BuilderLoader, index *pythonlocal.SymbolIndex, path string)) {
	...
	corpusDir := filepath.Join(gopath, "src/github.com/khulnasoft-lab/fastnode/fastnode-go/lang/python/testcorpus", subdir)
	mainPath := filepath.Join(corpusDir, "main.py")
	bufferPath := filepath.Join(corpusDir, "buffer.py")

	// simulate what Fastnode Local does for creating the builder
	...
	// build
	build, err := builder.Build(fastnodectx.Background(), localcode.BuilderParams{
		UserID:     UserID,
		MachineID:  MachineID,
		Filename:   mainPath,
		FileGetter: fs,
