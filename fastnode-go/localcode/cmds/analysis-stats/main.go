...
import (
	...
	"github.com/khulnasoft-lab/fastnode/fastnode-go/client/distranking"
...
func build(builder *pythonbatch.BuilderLoader, params localcode.BuilderParams) (buildData, error) {
	...
	ts := time.Now()
	buildResult, err := builder.Build(fastnodectx.Background(), params)
	if err != nil {
		return b, err
	}
	...
	ts = time.Now()
	mod, err := pythonparser.Parse(fastnodectx.Background(), contents, pythonparser.Options{
		Approximate: true,
	...
	resolved, err := pythonanalyzer.Resolve(fastnodectx.Background(), pythonanalyzer.Models{
...
func mustLoadBuilder() *pythonbatch.BuilderLoader {
	// simulate what Fastnode Local does
