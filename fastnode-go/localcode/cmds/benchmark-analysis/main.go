...
import (
	...
	"github.com/khulnasoft-lab/fastnode/fastnode-go/client/fastnodelocal"
...
func (g zipFS) Walk(ctx fastnodectx.Context, path string, walkFn localcode.WalkFunc) error {
	for name := range g {
...
func loadBuilder() (*pythonbatch.BuilderLoader, error) {
	graph, err := fastnodelocal.LoadResourceManager(context.Background(), fastnodelocal.LoadOptions{Blocking: true})
	if err != nil {
		return nil, err
	}
	...
	debug.SetGCPercent(15) // matches the setting in fastnode-go/client/internal/fastnodelocal/manager.go
...
func main() {
	...
	_, err = builder.Build(fastnodectx.Background(), params)
	fail(err)
