...
func (h *Handlers) Completion(params types.CompletionParams) (types.CompletionList, error) {
	...
	fastnodeResponse := data.NewAPIResponse(r)
	err = json.NewDecoder(resp.Body).Decode(&fastnodeResponse)
	if err != nil {
		return types.CompletionList{}, err
	}

	var items []types.CompletionItem
	for _, c := range fastnodeResponse.Completions {
...
// translateCompletion turns a Fastnode Completion into an LSP Completion
func (h *Handlers) translateCompletion(c data.RCompletion, text string) types.CompletionItem {
	...
	// LSP clients must send {fastnodeTypesEnabled: true} to receive a hint string.
	opts, ok := h.Options.(map[string]interface{})
	kte, ok := opts[fastnodeTypesEnabled].(bool)

	if c.Hint != "" {
		if ok && kte {
			item.FastnodeHint = c.Hint
		} else {
