...
var (
	...
	lexPath           = filepath.Join(testDirString, "fastnode-go", "lang", "lexical", "lexicalcomplete", "api", "api.go")
	pyPath            = filepath.Join(testDirString, "fastnode-go", "lang", "python", "pythoncomplete", "api", "api.go")
	encPath           = filepath.Join(testDirString, "fastnode-golib", "lexicalv0", "encoder.go")
	trainPath         = filepath.Join(testDirString, "local-pipelines", "lexical", "train", "train.py")
	modelPath         = filepath.Join(testDirString, "fastnode-python", "fastnode_ml", "fastnode", "model", "model.py")
...
func TestPath(t *testing.T) {
	tcs := []pathTC{
		...
		pathTC{
			operatingSystem:    "darwin",
			path:               "/go/src/github.com/khulnasoft-lab/fastnode/windows/client/FastnodeService/LibraryIO.cs",
			expectedNormalized: "/go/src/github.com/khulnasoft-lab/fastnode/windows/client/FastnodeService/LibraryIO.cs",
			// nil would be better
			expectedBlocked: ErrPathInFilteredDirectory,
		},
		pathTC{
...
func TestIsProjectRoot(t *testing.T) {
	...
	tcs := []isProjectRootTC{
		...
		isProjectRootTC{
			path: khulnasoft-lab.Join("fastnode-go"),
		},
		isProjectRootTC{
			path:         khulnasoft-lab.Join("fastnode-haskell"),
			doesNotExist: true,
		},
	}
	for _, tc := range tcs {
...
func TestProjectInfo(t *testing.T) {
	...
	n := Navigator{
		...
		projects:   cache,
		gitStorage: s,
		load: func(ctx fastnodectx.Context, s git.Storage, ignoreOpts ignore.Options, recOpts recommend.Options) projectState {
...
func TestProjectsLRU(t *testing.T) {
	...
	require.True(t, ok)
	require.Equal(t, testDir.Join("fastnode-golib"), k)

	require.True(t, n.projects.Contains(testDir.Join("fastnode-python")))
	require.True(t, n.projects.Contains(testDir.Join("local-pipelines")))
	require.False(t, n.projects.Contains(testDir.Join("fastnode-go")))
