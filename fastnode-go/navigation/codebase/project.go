...
type loadFunc func(fastnodectx.Context, git.Storage, ignore.Options, recommend.Options) projectState
...
func (p *projectNavigator) maybeLoad(ctx fastnodectx.Context, s git.Storage, maxFileSize int64, maxFiles int) {
	if !p.shouldLoad() {
		return
	}
	...
	ignoreOpts := ignore.Options{
		...
		IgnoreFilenames: []localpath.Relative{ignore.GitIgnoreFilename, ignore.FastnodeIgnoreFilename},
	}
...
func (p *projectNavigator) navigate(ctx fastnodectx.Context, request recommend.Request) (FileIterator, error) {
	state := p.getState()
	switch state.status {
	case Inactive:
...
func load(ctx fastnodectx.Context, s git.Storage, ignoreOpts ignore.Options, recOpts recommend.Options) projectState {
