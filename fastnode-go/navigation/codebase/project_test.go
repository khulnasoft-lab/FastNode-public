...
func TestMaybeLoad(t *testing.T) {
	...
	for _, tc := range tcs {
		...
		project.maybeLoad(fastnodectx.Background(), s, 1e6, 1e5)
...
func TestProjectNavigate(t *testing.T) {
	...
	iter, err := project.navigate(fastnodectx.Background(), recommend.Request{})
	require.NoError(t, err)
...
func TestProjectNavigateNotActive(t *testing.T) {
	...
	for _, tc := range tcs {
		...
		_, err := project.navigate(fastnodectx.Background(), recommend.Request{})
