...
func newCollector(repoRoot, gitCache string, maxRecsPerFile int, useCommits bool) (collector, error) {
	...
	var (
		ignoreOpts = ignore.Options{
			...
			IgnorePatterns: []string{
				...
				"testdata/",
				"venv/",
				"fastnode-go/navigation/offline/experiments",
				"*.css",
				"*.sh",
				"*.html",
				"*.less",
	...
	r, err := recommend.NewRecommender(fastnodectx.Background(), recOpts, ignorer, s)
	if err != nil {
		return collector{}, err
	}
	return collector{
...
func (c collector) collectRecords(repoRoot string, paths []string) ([]record, error) {
	...
	for _, path := range paths {
		...
		recs, err := c.recommender.Recommend(fastnodectx.Background(), req)
		if err != nil {
			return nil, err
		}
