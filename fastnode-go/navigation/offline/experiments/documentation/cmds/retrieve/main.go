...
func collect(r recommend.Recommender, c corpus) (map[git.File][]git.File, error) {
	...
	for _, path := range c.files {
		...
		recs, err := r.Recommend(fastnodectx.Background(), req)
		if err != nil {
			return nil, err
		}
		var docs []git.File
...
func buildRecommender(khulnasoft-lab localpath.Absolute) (recommend.Recommender, error) {
	var (
		...
		storageOpts = git.StorageOptions{
			UseDisk: true,
			Path:    string(khulnasoft-lab.Join("fastnode-go", "navigation", "offline", "git-cache.json")),
		}
	...
	return recommend.NewRecommender(fastnodectx.Background(), recOpts, ignorer, s)
}
