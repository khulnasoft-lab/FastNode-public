...
def main() -> None:
    ...
    args = parse_args()
    links = get(os.environ["GITHUB_AUTH_TOKEN"], "khulnasoft-lab/khulnasoft-lab", 12)
...
def get(token: str, repo_name: str, months: int) -> Dict[str, List[str]]:
    ...
    regexps = [
        re.compile("https://github.com/khulnasoft-lab/fastnode/issues/[0-9]+"),
        re.compile("https://github.com/khulnasoft-lab/fastnode/pull/[0-9]+"),
        re.compile("https://fastnode.quip.com/[a-zA-Z0-9]+"),
    ]
    for issue in issues:
        time.sleep(0.1)
