...
func TestFindDiffBlocks(t *testing.T) {
	tcs := []findDiffBlocksTC{
		findDiffBlocksTC{
			patch: `@@ -142,6 +142,7 @@ func (Golang) Provide(ctx fastnodectx.Context, g Global, in Inputs, out OutputFunc)
...
`,
			expected: []recommend.Block{
				recommend.Block{
					Content: ` 	// per: https://fastnode.quip.com/1ovKAL1hi2JZ/Spec-Lexical-Completions-Private-Beta#eLcACAqzrwL
 	opts.MixOptions.NestCompletions = false
