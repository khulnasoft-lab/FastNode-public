...
func Validate(opts Options) (Stats, Stats, []Record) {
	...
	recommender, err := recommend.NewRecommender(fastnodectx.Background(), recOpts, ignorer, opts.Storage)
	if err != nil {
		log.Fatal(err)
	}
...
func (e evaluator) evaluateFiles() (Stats, []Record) {
	...
	for _, files := range e.validationData {
		...
		for i, current := range files {
			...
			retrieved, err := e.recommender.Recommend(fastnodectx.Background(), request)
			if err != nil {
				continue
			}
	...
	for _, files := range e.validationData {
		...
		for i, current := range files {
			...
			for j, other := range files {
				...
				retrieved, err := e.recommender.RecommendBlocks(fastnodectx.Background(), request)
