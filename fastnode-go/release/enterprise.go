...
<rss version="2.0" xmlns:sparkle="http://www.andymatuschak.org/xml-namespaces/sparkle" xmlns:dc="http://purl.org/dc/elements/1.1/">
    <channel>
        <title>Fastnode Enterprise Client Changelog</title>
        <link>{{.AppCastURL}}</link>
        <description>Most recent changes Fastnode Enterprise client, with links to updates.</description>
...
func (s *EnterpriseServer) handleCurrent(w http.ResponseWriter, r *http.Request) {
	...
	w.Header().Set("Content-Disposition", fmt.Sprintf("attachment; filename=%s", "FastnodeEnterprise.dmg"))
	w.Write(s.osxDMG)
}
...
func (s *EnterpriseServer) pollClientVersion() {
	...
	dmgPath := filepath.Join(latestVersionDir, "FastnodeEnterprise.dmg")
