...
func TestWindowsRequests(t *testing.T) {
	...
	resp, _ := httpClient.Get(fmt.Sprintf("http://%s/windows/fastnode-app/update-check?version=%s", listener.Addr().String(), version))
	require.NotNil(t, resp)
	require.EqualValues(t, http.StatusOK, resp.StatusCode)

	// no update info for current version
	resp, _ = httpClient.Get(fmt.Sprintf("http://%s/windows/fastnode-app/update-check?will-attempt-update-if-provided=true&version=%s", listener.Addr().String(), nextVersion))
	require.NotNil(t, resp)
	require.EqualValues(t, http.StatusOK, resp.StatusCode)

	// redirect for available update info
	resp, _ = httpClient.Get(fmt.Sprintf("http://%s/windows/fastnode-app/update-check?will-attempt-update-if-provided=true&version=%s", listener.Addr().String(), version))
	require.NotNil(t, resp)
	require.EqualValues(t, http.StatusSeeOther, resp.StatusCode)
	require.EqualValues(t, fmt.Sprintf("%s/windows/%s/FastnodeUpdateInfo.xml", BunnyVolumeDownloadPrefix, nextVersion), resp.Header["Location"][0])

	// test serving of FastnodeUpdater.exe data
	url := fmt.Sprintf("http://%s/windows/%s/FastnodeUpdater.exe", listener.Addr().String(), nextVersion)
	...
	require.NotNil(t, resp)
	require.EqualValues(t, http.StatusSeeOther, resp.StatusCode)
	require.EqualValues(t, fmt.Sprintf("%s/windows/%s/deltaFrom/%s/FastnodeDeltaUpdateInfo.xml", BunnyVolumeDownloadPrefix, nextVersion, version), resp.Header["Location"][0])

	url = fmt.Sprintf("http://%s/windows/%s/FastnodePatchUpdater%s-%s.exe", listener.Addr().String(), nextVersion, version, nextVersion)
	resp, _ = httpClient.Get(url)
	require.NotNil(t, resp)
	require.EqualValues(t, http.StatusSeeOther, resp.StatusCode)
}
...
func makeTestManager(readsPublic bool) *MetadataManagerImpl {
	db := DB("postgres", "postgres://communityuser:fastnode@localhost/account_test?sslmode=disable")
