...
const BunnyVolumeDownloadPrefix = "https://fastnodedownloads.b-cdn.net"

// BunnyStandardDownloadPrefix is the BunnyCDN proxy for our S3 bucket, using the "standard" plan.
const BunnyStandardDownloadPrefix = "https://fastnodedownloadss.b-cdn.net"
...
<rss version="2.0" xmlns:sparkle="http://www.andymatuschak.org/xml-namespaces/sparkle" xmlns:dc="http://purl.org/dc/elements/1.1/">
	<channel>
		<title>Fastnode Client Changelog</title>
		<link>{{.AppCastURL}}</link>
		<description>Most recent changes Fastnode client, with links to updates.</description>
...
func NewServer(router *mux.Router, manager MetadataManager, platforms []Platform) *Server {
	...
	// Fastnode first download URL
	router.HandleFunc("/dls/{platform}/current", server.serveLatest)
	// /dls/linux/current downloads a shell script,
	// which in turn downloads the full binary from /dls/current/fastnode-installer
	router.HandleFunc("/linux/{version}/fastnode-installer", server.serveLinuxInstaller)
	...
	// macOS/Sparkle fallback updater hits /fallback/{version}/appcast.xml from FastnodeHelper
	router.HandleFunc("/fallback/{version}/appcast.xml", server.serveFallbackAppCast).Name("fallback")

	// Windows updater hits /windows/fastnode-app/update-check, which redirects to XML update manifest (on CDN)
	router.HandleFunc("/windows/fastnode-app/update-check", server.serveWinUpdateCheck)
	// XML update manifest points at /windows/{version}/FastnodeUpdater.exe
	router.HandleFunc("/windows/{version}/FastnodeUpdater.exe", server.serveWinUpdate)
	// or at /windows/{toVersion}/FastnodePatchUpdater{fromVersion}.exe
	router.HandleFunc("/windows/{toVersion}/FastnodePatchUpdater{fromVersion}-{toVersionAgain}.exe", server.serveWinDeltaUpdate)

	// Linux updater hits /linux/fastnode-app/update-check, which redirects to JSON update manifest (on CDN)
	router.HandleFunc("/linux/fastnode-app/update-check", server.serveLinuxUpdateCheck)
	// JSON update manifest points at /linux/{version}/fastnode-updater.sh or
	// /linux/{version}/{fromVersion}/fastnode-patch-updater.sh
	// which redirect to CDN download URL for the updater or patch updater package.
	router.HandleFunc("/linux/{version}/fastnode-updater.sh", server.serveLinuxUpdate)
	router.HandleFunc("/linux/{version}/{fromVersion}/fastnode-patch-updater.sh", server.serveLinuxPatchUpdate)
...
func (s *Server) serveAppCastHelper(w http.ResponseWriter, r *http.Request, release *Metadata, appCastURL *url.URL) {
	...
	u, err := url.Parse(fmt.Sprintf("%s/mac/%s/Fastnode.dmg", prefix, release.Version))
	if err != nil {
		webutils.ErrorResponse(w, r, err, nil)
		return
	}
	...
	for _, d := range deltas {
		u, err := url.Parse(fmt.Sprintf("%s/mac/%s/deltaFrom/%s/Fastnode.delta", prefix, d.ToVersion, d.FromVersion))
		if err != nil {
			webutils.ErrorResponse(w, r, err, nil)
			return
		}
...
func (s *Server) serveLatest(w http.ResponseWriter, r *http.Request) {
	...
	switch platform {
	case Mac:
		dlURL = fmt.Sprintf("%s/mac/%s/Fastnode.dmg", prefix, latest.Version)
	case Windows:
		dlURL = fmt.Sprintf("%s/windows/%s/FastnodeSetup.exe", prefix, latest.Version)
	case Linux:
		dlURL = fmt.Sprintf("%s/linux/%s/fastnode-installer.sh", prefix, latest.Version)
	}
...
func (s *Server) serveWinUpdateCheck(w http.ResponseWriter, r *http.Request) {
	...
	if isOlder {
		...
		if deltaUpdate := s.deltaForVersions(Windows, vals["version"], latestVersion.Version); deltaUpdate != nil {
			u, err := url.Parse(fmt.Sprintf("%s/windows/%s/deltaFrom/%s/FastnodeDeltaUpdateInfo.xml",
		...
		u, err := url.Parse(fmt.Sprintf("%s/windows/%s/FastnodeUpdateInfo.xml", prefix, latestVersion.Version))
		if err != nil {
			webutils.ErrorResponse(w, r, err, nil)
			return
		}
...
func (s *Server) serveWinUpdate(w http.ResponseWriter, r *http.Request) {
	...
	u, err := url.Parse(fmt.Sprintf("%s/windows/%s/FastnodeUpdater.exe", s.getAssetPrefix(BunnyVolumeDownloadPrefix, r.Host), version))
	if err != nil {
		webutils.ErrorResponse(w, r, err, nil)
		return
	}
...
func (s *Server) serveWinDeltaUpdate(w http.ResponseWriter, r *http.Request) {
	...
	u, err := url.Parse(fmt.Sprintf("%s/windows/%s/deltaFrom/%s/FastnodeDeltaUpdater.exe", s.getAssetPrefix(BunnyVolumeDownloadPrefix, r.Host), toVersion, fromVersion))
	if err != nil {
		webutils.ErrorResponse(w, r, err, nil)
		return
	}
...
func (s *Server) serveLinuxUpdate(w http.ResponseWriter, r *http.Request) {
	vars := mux.Vars(r)
	version := vars["version"]
	u, err := url.Parse(fmt.Sprintf("%s/linux/%s/%s", s.getAssetPrefix(BunnyVolumeDownloadPrefix, r.Host), version, "fastnode-updater.sh"))
	if err != nil {
		webutils.ErrorResponse(w, r, err, nil)
		return
	}
...
func (s *Server) serveLinuxPatchUpdate(w http.ResponseWriter, r *http.Request) {
	...
	version := vars["version"]
	fromVersion := vars["fromVersion"]
	u, err := url.Parse(fmt.Sprintf("%s/linux/%s/deltaFrom/%s/%s", s.getAssetPrefix(BunnyVolumeDownloadPrefix, r.Host), version, fromVersion, "fastnode-updater.sh"))
	if err != nil {
		webutils.ErrorResponse(w, r, err, nil)
		return
	}
...
func (s *Server) serveLinuxInstaller(w http.ResponseWriter, r *http.Request) {
	vars := mux.Vars(r)
	version := vars["version"]
	// "current" is an alias to retrieve the latest available version of the fastnode-installer binary
	if version == "current" {
	...
	u, err := url.Parse(fmt.Sprintf("%s/linux/%s/%s", s.getAssetPrefix(BunnyStandardDownloadPrefix, r.Host), version, "fastnode-installer"))
	if err != nil {
		webutils.ErrorResponse(w, r, err, nil)
		return
	}
