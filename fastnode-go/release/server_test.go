...
func TestServeLatest(t *testing.T) {
	...
	cases := map[string]struct {
		...
		"MacDownloadsLatest": {
			...
			expectLocContains: proto.String(fmt.Sprintf("/mac/%s/Fastnode.dmg", macNonCanaryVersion)),
		...
		"MacDownloadsNoCanary": {
			...
			route:                "/dls/mac/current",
			expectStatus:         http.StatusSeeOther,
			expectLocNotContains: proto.String(fmt.Sprintf("/mac/%s/Fastnode.dmg", macCanaryVersion)),
		...
		"LinuxDownloadsLatest": {
			...
			expectStatus:      http.StatusSeeOther,
			expectLocContains: proto.String(fmt.Sprintf("/linux/%s/fastnode-installer.sh", linuxNonCanaryVersion)),
		...
		"LinuxDownloadsNoCanary": {
			...
			route:                "/dls/linux/current",
			expectStatus:         http.StatusSeeOther,
			expectLocNotContains: proto.String(fmt.Sprintf("/linux/%s/fastnode-installer.sh", linuxCanaryVersion)),
		...
		"WindowsDownloadsLatest": {
			...
			expectStatus:      http.StatusSeeOther,
			expectLocContains: proto.String(fmt.Sprintf("/windows/%s/FastnodeSetup.exe", windowsVersion)),
