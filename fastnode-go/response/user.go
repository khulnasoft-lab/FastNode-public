...
import "github.com/khulnasoft-lab/fastnode/fastnode-go/localcode"
...
// RouterRoot is the subset of Root that is accessible from fastnoded. Most of the struct
...
// Lockout is the result type returned when Fastnode has not been enabled on
// the current file.
type Lockout struct {
	Type       string `json:"type"`
	Filename   string `json:"filename"`
...
type EditorCompletion struct {
	...
	// Source encapsulates where the completion was generated; it is not serialized to disallow use by the editor (or fastnoded if using Fastnode Cloud)
