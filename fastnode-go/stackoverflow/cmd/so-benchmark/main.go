...
const (
	dataS3Path        = "s3://fastnode-data/stackoverflow/ranking/2015-10-27_18-17-42-PM/"
...
func main() {
	...
	flag.IntVar(&fastnodeMode, "fastnode", -1, " -1 -> no fastnode, 0 -> use all fastnode, 1 -> use fastnode (cloudsearch) index only, 2 -> use fastnode PageFinder only")
	flag.Parse()
	if logsPath == "" {
		flag.Usage()
	...
	case benchmark, benchmarkPageFinder, benchmarkSearcher:
		...
		start := time.Now()
		if fastnodeMode == 0 || fastnodeMode == 2 {
