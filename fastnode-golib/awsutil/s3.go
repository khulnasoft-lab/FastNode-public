...
var (
	...
	// if FASTNODE_USE_AZURE_MIRROR is set to '1', NewS3Reader and NewCachedS3Reader will attempt to find the
	...
	useAzureMirror = envutil.GetenvDefault("FASTNODE_USE_AZURE_MIRROR", "0") == "1"
)
...
func regionURI(uri *url.URL) *url.URL {
	...
	regionBucket := fmt.Sprintf("fastnode-prod-data-%s", localRegion)
