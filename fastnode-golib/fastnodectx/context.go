// Package fastnodectx encapsulates the capability to abort computations.
//
// fastnodectx.Context is analogous to the built-in context.Context, with some helper methods
...
// another function that calls it) at the start of every function that accepts a fastnodectx.Context.
//
...
// SEE: fastnode-go/lang/python/pythoncall/model.go for an example of why this does not work
package fastnodectx
...
import (
	...
	"github.com/khulnasoft-lab/fastnode/fastnode-golib/fastnodelog"
)

// Context manages an abort condition and a Fastnode logger
...
// WithLogger returns a new Context with the provided fastnodelog.Logger set
func (ctx Context) WithLogger(l *fastnodelog.Logger) Context {
	ctx.Logger = l
	return ctx
}

// Context returns a context.Context for use with libraries/packages that don't support fastnodectx
