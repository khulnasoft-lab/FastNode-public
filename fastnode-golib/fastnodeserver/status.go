...
import (
	...
	"github.com/khulnasoft-lab/fastnode/fastnode-server/metadata/service"
...
func GetHealth(url string) (string, int64, error) {
	...
	parsedURL, err := ParseFastnodeServerURL(url)
	if err != nil {
		return "", 0, err
	}
...
func doRequest(client service.MetadataServiceClient) (string, error) {
	var res string
	errc := fastnodectx.Go(func() error {
...
// ParseFastnodeServerURL takes a string, which represents a KTS endpoint
func ParseFastnodeServerURL(s string) (*url.URL, error) {
