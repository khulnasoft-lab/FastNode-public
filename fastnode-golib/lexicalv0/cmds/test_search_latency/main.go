...
func main() {
	...
	if args.CompareLatency {
		b.MustBenchmark("predictor", nil,
			func(predictor lexicalmodels.ModelBase, in predict.Inputs) {
				preds, err := tfpred.PredictChan(fastnodectx.Background(), in)
				for range preds {
				}
				fail(<-err)
			},
		...
		b.MustBenchmark("searcher", nil,
			func(predictor lexicalmodels.ModelBase, in predict.Inputs) {
				preds, err := tfsearch.PredictChan(fastnodectx.Background(), in)
				for range preds {
				}
				fail(<-err)
			},
		...
		b.MustBenchmark("searcher-remote", nil,
			func(predictor lexicalmodels.ModelBase, in predict.Inputs) {
				preds, err := tfremote.PredictChan(fastnodectx.Background(), in)
				for range preds {
				}
				fail(<-err)
			},
	...
	if args.CompareResults {
		...
		b.MustBenchmark("predict-vs-search-vs-remote", nil,
			func(predictor lexicalmodels.ModelBase, in predict.Inputs) {
				preds, errChan := tfpred.PredictChan(fastnodectx.Background(), in)
				...
				preds, errChan = tfsearch.PredictChan(fastnodectx.Background(), in)
				...
				preds, errChan = tfremote.PredictChan(fastnodectx.Background(), in)
