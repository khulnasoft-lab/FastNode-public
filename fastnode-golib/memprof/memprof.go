...
func init() {
	path := os.Getenv("FASTNODE_MEMPROF_DIR")
	if path == "" {
		return
	}
	period := time.Second
	if periodStr := os.Getenv("FASTNODE_MEMPROF_PERIOD"); periodStr != "" {
		var err error
		period, err = time.ParseDuration(periodStr)
		if err != nil {
			log.Fatalf("$FASTNODE_MEMPROF_PERIOD was '%s', which is not a valid duration", period)
		}
	}
