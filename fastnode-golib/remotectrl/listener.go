...
func (l *Listener) processEvent(ctx fastnodectx.Context, evt event) {
	ctx.CheckAbort()
...
func (l *Listener) write(ctx fastnodectx.Context, evt event) error {
...
func (l *Listener) listen(ctx fastnodectx.Context) error {
	ctx.CheckAbort()
	...
	for backoff := minBackoff; conn == nil; backoff = min(2*backoff, maxBackoff) {
		ctx.CheckAbort()
		err := ctx.WithTimeout(15*time.Second, func(ctx fastnodectx.Context) error {
			var err error
...
func sleep(ctx fastnodectx.Context, dur time.Duration) {
	t := time.NewTimer(dur)
	defer t.Stop()
