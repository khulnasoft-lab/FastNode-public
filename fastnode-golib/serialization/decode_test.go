...
func TestGzippedJSON(t *testing.T) {
	...
	err := decodeAs(bytes.NewBuffer(d), "s3://fastnode-data/bar.json.gz", func(a *apple) {
		apples = append(apples, a)
	})
	require.NoError(t, err)
	assert.Len(t, apples, 2)
...
func TestEMRKeyValue(t *testing.T) {
	var apples []*apple
	d := fmt.Sprintf("key1\t%s\nkey2\t%s\n", base64zip(`{"Redness":1}`), base64zip(`{"Redness":2}`))
	err := decodeAs(bytes.NewBufferString(d), "s3://fastnode-data/foo.emr", func(x *awsutil.KeyValue) {
		var apple apple
		x.JSONValue(&apple)
