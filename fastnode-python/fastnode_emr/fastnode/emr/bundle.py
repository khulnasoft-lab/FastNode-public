...
from fastnode.emr.constants import FASTNODE_PYTHON
from fastnode.emr.constants import FASTNODE_EMR_BUCKET
from fastnode.emr.constants import BUNDLE_DIR
from fastnode.emr.templates import template_with_root
...
class Bundle(object):
    ...
    def _build_fastnode_python(self):
        shutil.rmtree(os.path.join(FASTNODE_PYTHON, "dist"))

        print("building fastnode-python module...")
        p = subprocess.Popen(['./setup.py', 'sdist'], cwd=FASTNODE_PYTHON,
                         stdout=subprocess.PIPE, stderr=subprocess.PIPE)
        p.wait()
        if p.returncode != 0:
            os.exit(1)

        matches = glob.glob(os.path.join(FASTNODE_PYTHON, 'dist', 'fastnode-*.tar.gz'))
        if len(matches) == 1:
            shutil.copyfile(matches[0],
                            os.path.join(self.bundle_dir,
                                         'bootstrap', 'fastnode.tar.gz'))
            shutil.copyfile(os.path.join(FASTNODE_PYTHON, 'requirements-emr.txt'),
                            os.path.join(self.bundle_dir,
                                         'bootstrap', 'requirements-emr.txt'))

    def _build_fastnode_go(self):
        env = os.environ.copy()
        env.update({
            'GOOS': 'linux',
            'GOARCH': 'amd64',
    ...
    def build(self):
        ...
        self._build_fastnode_python()
        self._build_fastnode_go()
