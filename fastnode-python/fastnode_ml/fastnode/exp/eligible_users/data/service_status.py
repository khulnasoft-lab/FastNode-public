...
def get_service_status(filename: str, users: pd.DataFrame) -> pd.DataFrame:
    ...
    for line in tqdm.tqdm(lines):
        rec = json.loads(line)
        if rec['fastnode_service_version'] == '':
            continue
        max_lifetime = 0
        if rec['fastnoded_lifetimes_in_millis']:
            max_lifetime = max(rec['fastnoded_lifetimes_in_millis'])
        ...
        obj = {
            ...
            'user_id': user_id,
            'fastnoded_running': int(rec['num_fastnoded_processes'] > 0),
            'fastnode_proxy': int(rec['send_to_fastnode_dot_com_not_segment']),
