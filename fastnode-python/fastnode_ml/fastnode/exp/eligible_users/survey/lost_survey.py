...
def get_survey(survey_file: str, fix_file: Optional[str] = None) -> pd.DataFrame:
    ...
    :param survey_file: The original survey csv (as exported by Typeform)
        see https://fastnode.quip.com/QwBYA04DuxfS/Lost-User-Survey
...
def _parse_survey(filename: str, use_fastnode_question: bool) -> List[Dict[str, Any]]:
    recs = []
    ...
    with open(filename) as csvfile:
        ...
        for i, row in enumerate(reader):
            ...
            if use_fastnode_question:
                (typeform_id, use_fastnode, last_used_py, primary_py_use, editor, other_editor,
