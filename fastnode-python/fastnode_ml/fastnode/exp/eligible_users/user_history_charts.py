...
def plot_user_history_chart(filename: str,
                            ...
                            figsize=(16,8),
                            dpi=300):
    # 0: unactivated (no fastnode_status yet)
    # 1: no fastnode_status
    # 2: fastnode_status with no python events
    # 3: fastnode_status with python events
    ...
    for u, uid in enumerate(tqdm.tqdm(user_ids)):
        ...
        for d in range(n_days):
            ...
            if day < first_day:
                # user hasn't had a fastnode_status event yet
                heatmap[u, d] = 0
                continue
            on_day = uh[uh.day == day]
            if len(on_day) == 0:
                # no fastnode_status for that user/day
                heatmap[u, d] = 1
                continue
    ...
    colors = ((255, 255, 255),  # unactivated
              (181, 110, 110),  # no fastnode_status
              (166, 206, 227),  # fastnode_status, no python
              (124, 181, 100),  # python
              )
...
def user_history_charts(out_dir: str,
    ...
    fastnode_status_users = set(valid_users[valid_users.last_day >= cutoff].index)
    active_users = set(valid_users[valid_users.last_py_event >= cutoff].index)
    dormant_users = fastnode_status_users.difference(active_users)
