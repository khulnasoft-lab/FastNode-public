""" Automatically inject fastnode runtime tracking into all modules imported after configure() is called """
import ast
import copy
import collections
import imp
...
class GlobalFastnodeTransformer(RuntimeTransformer):
    """ `exec` doesn't work in the presence of free variables, so declare the
        `fastnode` import to be global at the top of each function. """
    def visit_FunctionDef(self, node):
        ...
        new_body.append(ast.Global(self.__class__.__module__.split('.', 1)[:1]))  # `global fastnode`
...
class _TrackedList(collections.MutableSequence, list):
    ...
    def __setitem__(self, index, value):
        if index == 0:
            raise Exception('Trying to overwrite Fastnode pkgexploration runtime import hook')
        list.__setitem__(self, index, value)

    def __delitem__(self, index):
        if index == 0:
            raise Exception('Trying to remove Fastnode pkgexploration runtime import hook')
...
def configure(ast_transformers):
    ...
    # ensure that the Fastnode import hook remains the first hook
