...
env = Environment(
    loader=PackageLoader('fastnode_metrics', 'schemas'),
)

cache = {}

def _schema_exists(filename):
    return pkg_resources.resource_exists('fastnode_metrics', 'schemas/{}'.format(filename))

def _schema_open(filename):
    return pkg_resources.resource_stream('fastnode_metrics', 'schemas/{}'.format(filename))
