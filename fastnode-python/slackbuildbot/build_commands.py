...
class CommandRunner(object):
    ...
    def build_paths(self):
        ...
        self.PLUGINS_DIR = os.path.join(self.GIT_REPO, "fastnode-go", "client", "internal", "plugins")
        self.DATA_OUTPUT = "/var/fastnode/fastnodelocal-data"
    ...
    async def stage_macos_client(self, test=False):
        ...
        await self.run_subprocess(cmd, success_mesg="MacOS client build complete. Find the client on s3 in the `fastnode-downloads` folder")

        # upload to slack if test
        # NOTE: should try to get this from script output in the future
        if test:
            dmg_output = os.path.join(self.MACOS_DMG_DIR, "Fastnode-{}.dmg".format(test_version))
    ...
    async def stage_backend(self, branch_name):
        ...
        await self.run_subprocess(
            ...
            success_mesg="%s now available on staging.khulnasoft.com" % branch_name)
    ...
    async def stage_website(self):
        ...
        await self.run_subprocess(
            ...
            cwd=self.GIT_REPO,
            success_mesg="Website is now available on ga-staging.khulnasoft.com")
    ...
    async def verify_datadeps(self, **kwargs):
        ...
        self.bot.send("Starting fastnodelocal datadeps verification...")
        print("Awaiting fastnodelocal datadeps verification")
        await self.run_subprocess([os.path.join(self.SCRIPTS, "./verify_datadeps.sh")],
            success_mesg="Fastnodelocal datadeps verified",
            error_mesg="Fastnodelocal datadeps verification failed!")
    ...
    async def release(self, **kwargs):
        ...
        # npm pkg publish
        if kwargs.get("npm"):
            print("Will publish fastnode npm package suite")
            tasks.append(self.publish_fastnode_npm_pkgs())
    ...
    async def release_backend(self, release_name):
        ...
        await self.run_subprocess(
            ...
            logfile_name="albswitch",
            error_mesg="Load-balancing error!",
            success_mesg="%s now available on alpha.khulnasoft.com" % release_name)
    ...
    async def publish_fastnode_npm_pkgs(self):
        """Publish Fastnode's npm package suite"""
        # we use the git repo names here, not the npm package names -
        # which are referenced in the respective package.json's
        await self.publish_fastnode_npm("fastnode-connect-js")
        await self.publish_fastnode_npm("fastnode-installer")
        await self.publish_fastnode_npm("fastnode-api-js")

    async def publish_fastnode_npm(self, repo_name):
        """Publish the fastnode npm package indicated by repo_name"""
    ...
    async def require_deployment_tool(self, rebuild=False):
        ...
        if rebuild or not os.path.exists(os.path.join(self.SCRIPTS, DEPLOYMENTS_CMD)):
            ...
            await self.run_subprocess(
                ["go", "build", "github.com/khulnasoft-lab/fastnode/fastnode-go/cmds/"+DEPLOYMENTS_CMD],
