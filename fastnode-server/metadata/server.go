...
func main() {
	...
	flag.StringVar(&tokenFile, "tokenFile", "/run/secrets/fastnode-server-deployment-token", "File to load deployment token.")
	flag.Parse()

	f, err := os.Open(tokenFile)
	if err != nil {
		log.Fatalln("Error reading Fastnode Server deployment token", err)
	}
	defer f.Close()
