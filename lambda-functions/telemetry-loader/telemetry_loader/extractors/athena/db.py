...
SQL_FILE = os.path.join(os.path.dirname(os.path.realpath(__file__)), 'fastnode_status_1d.sql.tmpl')
TIMEZONE = 'UTC'
...
TEST_RESULT_LOC_PREFIX = 's3://fastnode-metrics/athena-results-test'


class Queries(object):
    fastnode_status_1d = ('fastnode_status', 'fastnode_status_table.sql.tmpl', 'fastnode_status_1d', 'fastnode_status_1d.sql.tmpl')
...
def _sql_create_versioned_table(prefix, create_table_template):
    ...
    return pystache.render(template_content, ctx), 'fastnode_metrics.{}'.format(ctx['table_name'])
...
def _execute_query(client, query_sql, output_prefix, output_suffix):
    ...
    return client.start_query_execution(
        ...
        QueryExecutionContext={'Database': 'fastnode_metrics'},
