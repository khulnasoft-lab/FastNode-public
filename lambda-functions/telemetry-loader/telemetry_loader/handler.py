...
from telemetry_loader.transformers.fastnode_status import transform_elastic_fastnode_status_1d
from telemetry_loader.transformers.fastnode_status import transform_mixpanel_fastnode_status_1d
...
def fastnode_status_1d(**kwargs):
    elastic, mixpanel = extract_s3()(**kwargs) | progress(logger.info) | csv_pipe() | fork(2)
    elastic = elastic | transform_elastic_fastnode_status_1d | load_elastic
    mixpanel = mixpanel | transform_mixpanel_fastnode_status_1d | null_consumer  # load_mixpanel
    return bundle(elastic, mixpanel)


def fastnode_status(**kwargs):
    return extract_s3(compressed=True, range_size=8e6)(**kwargs) | progress(logger.info) | transform_elastic_fastnode_status | load_elastic
...
def lambda_event_handler(record):
    ...
    run_query(Queries.fastnode_status_1d, day, day + datetime.timedelta(days=1))
...
s3_file_handlers = [
    (re.compile('^fastnode-metrics/firehose/fastnode_status/'), fastnode_status),
    (re.compile('^fastnode-metrics/firehose/client_events/'), index_metrics),
    (re.compile('^fastnode-backend-logs/firehose/server-logs/'), server_logs),
    (re.compile('^fastnode-metrics/athena-results/fastnode_status_1d/'), fastnode_status_1d),
    (re.compile('^fastnode-aws-billing-reports/'), aws_cost_reports),
]
