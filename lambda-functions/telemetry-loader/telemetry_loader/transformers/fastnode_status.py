...
@pipe
async def transform_elastic_fastnode_status_1d(it):
    async for rec in it:
        ...
        yield {'_index': 'fastnode_status_daily', '_id': rec_id, '_source': rec}


@pipe
def transform_mixpanel_fastnode_status_1d(rec):
    rec = rec.copy()
    rec['user_id'] = rec.pop('userId', '')
    rec['_group'] = 'firehose/fastnode_status/{}/'.format(rec['end_time'].strftime('%Y/%m/%d'))
    rec['time'] = int(rec['end_time'].timestamp())
    rec['start_time'] = int(rec['start_time'].timestamp())
    rec['end_time'] = int(rec['end_time'].timestamp())
    rec['_version'] = 0
    rec['source'] = 'fastnoded'
    rec['name'] = rec.pop('event')
    return rec


INDEX_PREFIX = 'fastnode_status'
...
@json_pipe()
async def transform_elastic_fastnode_status(it):
