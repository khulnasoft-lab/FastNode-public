...
var (
	errFastnodeNotReady = errors.New("fastnode not ready for restart")
)

// localManager handles the locally installed versions of fastnode
// and interacts with fastnoded on the local machine
...
// the default installation dir is $HOME/.local/share/fastnode
func newLocalManager() *localManager {
	return &localManager{
		basePath:   filepath.Join(os.ExpandEnv("$HOME"), ".local", "share", "fastnode"),
...
func (m *localManager) currentVersion() (string, error) {
	...
	if !strings.HasPrefix(targetFile, "fastnode-v") {
...
func (m *localManager) installedVersions() ([]string, error) {
	matches, err := filepath.Glob(filepath.Join(m.basePath, "fastnode-v*"))
	if err != nil {
		return nil, err
	}
...
func (m *localManager) lockFilePath() string {
	return filepath.Join(m.basePath, "fastnode-update.lock")
}
...
func (m *localManager) downloadTargetPath(version Version) string {
	return filepath.Join(m.basePath, fmt.Sprintf("fastnode-updater-%s.sh", version.Version))
}
...
func (m *localManager) installDirPath(version string) string {
	return filepath.Join(m.basePath, fmt.Sprintf("fastnode-v%s", version))
}
...
// isReadyForUpdate returns if fastnoded is currently ready for a restart related to an update
func (m *localManager) isReadyForUpdate() bool {
	resp, err := http.Get(m.isReadyURL)
	if err != nil {
		// Assuming fastnoded isn't reachable, meaning it's not running
		// Should distinguish between other cases by explicitly checking for `fastnoded` running via ps aux
		return true
	}
	...
	// status 200 is returned when fastnoded is ready for a restart related to an update
...
// RestartFastnoded triggers fastnoded to restart if it's running at the moment
func (m *localManager) RestartFastnoded() error {
	resp, err := http.Post(m.restartURL, "text/plain", nil)
	// don't report this when fastnoded wasn't running
