...
func main() {
	...
	c := cli.NewCLI("fastnode-installer", version)
	c.Args = os.Args[1:]
	c.Commands = map[string]cli.CommandFactory{
		"install": func() (cli.Command, error) {
			if localVersion != "" {
				log.Println("Found an installation of Fastnode. Calling update instead.")
		...
		"update": func() (cli.Command, error) {
			if localVersion == "" {
				log.Println("No installation of Fastnode was found. Calling install instead.")
				return &installCommand{localManager: localManager}, nil
			}
			return &updateCommand{localManager: localManager}, nil
		},
