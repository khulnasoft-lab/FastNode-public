...
// exit codes to communicate status back to fastnoded
const (
	statusSuccess         = 0
	statusUpToDate        = 1
...
func (i *updateCommand) Synopsis() string {
	if i.selfUpdate {
		return "updates fastnode to the latest version, intended to be called by fastnoded itself to perform a self-update"
	}
	return "updates fastnode to the latest version (use 'update force' to force an update, use 'update silent' to suppress progress information)"
}
...
func (i *updateCommand) Run(args []string) int {
	...
	if err != nil {
		...
		rollbarError("failed to create fastnode-update lock file", name, err)
	...
	if err != nil {
		...
		ui.Error("no previous fastnode installation found. Terminating.")
		return statusLocalNotFound
	}
	...
	case err == errNoUpdateAvailable:
		...
		if i.selfUpdate {
			// don't let fastnoded restart without an update
	...
	if err != nil {
		...
		rollbarError("failed to download fastnode", name, err)
		return statusDownloadFailed
	}

	if !i.selfUpdate {
		// don't install if fastnoded isn't ready yet.
		// the next activation of the fastnode-updater.timer service will perform this check again
		// and will install the already downloaded update when fastnoded is ready
		if !i.localManager.isReadyForUpdate() || forceUpdate {
			ui.Info("not updating because fastnoded isn't ready to restart, terminating")
			return statusSuccess
		}
	}
	...
	// It (re)starts the update service, unless fastnoded is calling this command
	updaterPath := i.localManager.filePathCurrent("fastnode-update")
	...
	if _, err := os.Stat(updaterPath); err == nil {
		...
		ui.Error(fmt.Sprintf("unable to locate fastnode-update binary: %s", err))
		rollbarError("enable to locate fastnode-update binary", name, err)
	}

	// if fastnoded has called this updater, skip the cleanup. The fastnoded wrapper script is doing this
	// otherwise cleanup old versions
	if !i.selfUpdate {
		if err = i.localManager.RestartFastnoded(); err != nil {
			ui.Error(fmt.Sprintf("error restarting fastnoded after update: %s", err.Error()))
			rollbarError("failed to restart fastnoded after update", name, err)
		}
