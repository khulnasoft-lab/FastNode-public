...
func install(local *localManager, remoteVersion Version) error {
	...
	// we want to avoid a situation when the update is extracting and a user starts fastnoded.
	// in this case the user would start fastnode from a not-yet unpacked installation
...
func installSystemData(ui cli.Ui, actionType string) int {
	...
	// fastnoded is capable to update itself
	ui.Info("activating fastnode-updater systemd service")
	err = enableAndStartUpdaterService()
	if err != nil {
		ui.Warn("error enabling fastnode-updater.timer. " + err.Error())
	...
	// we're not starting the service, because the install command starts fastnoded on its own
	// and the update command triggers a restart with fastnoded's HTTP endpoint.
	ui.Info("activating fastnode-autostart systemd service")
	err = enableAutostartService()
	if err != nil {
		ui.Warn("error enabling fastnode-autostart. " + err.Error())
		rollbarError("failed to enable autostart service", actionType, err)
	}

	// update the registration of our custom protocol handler
	ui.Info("registering fastnode:// protocol handler")
	err = registerFastnodeProtocolHandler()
	if err != nil {
		ui.Error("error installing fastnode protocol handler. " + err.Error())
		rollbarError("failed to enable fastnode:// protocol handler", actionType, err)
		// don't rollback, because installs without X11 may not have xdg-open installed
	}
	return 0
}
...
// removeOldVersions removes outdated versions from disk. We're not keeping old versions of fastnoded around.
...
func stopAndDisableUpdaterService() error {
	cmd := exec.Command("systemctl", "--user", "disable", "--now", "fastnode-updater.timer")
	buf, err := cmd.CombinedOutput()
	if err != nil {
		return cmdError{
			Msg:    "error disabling fastnode-updater.timer",
			Err:    err,
			Output: string(buf),
		}
	}
...
func enableAndStartUpdaterService() error {
	cmd := exec.Command("systemctl", "--user", "enable", "--now", "fastnode-updater.timer")
	buf, err := cmd.CombinedOutput()
	if err != nil {
		return cmdError{
			Msg:    "error enabling fastnode-updater.timer",
			Err:    err,
			Output: string(buf),
		}
	}
...
func stopAndDisableAutostartService() error {
	cmd := exec.Command("systemctl", "--user", "disable", "--now", "fastnode-autostart")
	buf, err := cmd.CombinedOutput()
	if err != nil {
		return cmdError{
			Msg:    "error disabling fastnode-autostart",
			Err:    err,
			Output: string(buf),
		}
	}
...
func enableAutostartService() error {
	cmd := exec.Command("systemctl", "--user", "enable", "fastnode-autostart")
	buf, err := cmd.CombinedOutput()
	if err != nil {
		return cmdError{
			Msg:    "error enabling fastnode-autostart",
			Err:    err,
			Output: string(buf),
		}
	}
...
func startAutostartService() error {
	cmd := exec.Command("systemctl", "--user", "start", "fastnode-autostart")
	buf, err := cmd.CombinedOutput()
	if err != nil {
		return cmdError{
			Msg:    "error starting fastnode-autostart",
			Err:    err,
			Output: string(buf),
		}
	}
...
func registerFastnodeProtocolHandler() error {
	cmd := exec.Command("xdg-mime", "default", "fastnode-copilot.desktop", "x-scheme-handler/fastnode")
	buf, err := cmd.CombinedOutput()
	if err != nil {
		return cmdError{
			Msg:    "error running 'xdg-mime default fastnode-copilot.desktop x-scheme-handler/fastnode'",
			Err:    err,
			Output: string(buf),
		}
	}
...
// launchFastnode first tries to launch fastnoded with systemd and then tries to run it via commandline
func launchFastnode(local *localManager) error {
	...
	// fallback to launch the fastnoded via commandline
	fastnodeLaunchScript := filepath.Join(local.basePath, "fastnoded")
	cmd := exec.Command(fastnodeLaunchScript)
	err = cmd.Start()
	if err != nil {
		return err
	}
...
func progressPrinter(ui cli.Ui) func(received, total int64) {
	if isTTY(os.Stdout) {
		...
		return func(received, total int64) {
			...
			if float64(received-lastReceived)/float64(total) > 0.001 {
				...
				fmt.Printf("\r%sDownloading Fastnode: %6.1f%% of %s", prefix, (float64(received)/float64(total))*100, humanize.IBytes(uint64(total)))
	...
	// layout: "Download <received bytes>/<total bytes>", this is used by fastnode-connect-js
