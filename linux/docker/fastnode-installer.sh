...
URL="https://linux.khulnasoft.com/linux/current/fastnode-installer"
...
function checkPrerequisites(){
    ...
    if ! grep -q '\<avx[^ ]*\>' /proc/cpuinfo; then
        echo >&2 "Sorry! Fastnode only runs on processor architectures with AVX support. Exiting now."
        exit 12
    fi
...
function promptX11Dependencies() {
    ...
    if command -v yum >/dev/null 2>&1; then
        ...
        echo
        echo "    ./fastnode-installer install"
        echo
        echo "Exiting now."
        exit 1
    fi
...
# Download the binary fastnode-installer and store it at the location defined by the first parameter
# It sets the executable flag after a successful download
function downloadFastnodeInstaller() {
    local target="$1"
...
# Uses the fastnode-installer passed as first argument to download the Fastnode installation package
function downloadFastnodePackage() {
    local target="$1"
    case "$target" in
      /*) ;; # absolute path
    ...
    "$target" install --download || { echo >&2 "Unable to download Fastnode executable package. terminating."; exit 1; }
}

function installFastnode() {
    ...
    [ -f "$downloadFile" ] || { echo "Unable to locate fastnode-installer at $downloadFile. Exiting now."; exit 1; }
    ...
    if [ "$status" != "0" ]; then
        echo
        echo "There was an error installing fastnode. Please visit https://help.khulnasoft.com/article/106-linux-install-issues for possible solutions."
        echo
        echo "Keeping fastnode-installer in the current directory in case you'd like to try again by running:"
        echo
...
mode="all"
downloadFile="$PWD/fastnode-installer"
while [[ $# -gt 0 ]]; do
    key="$1"
    shift
    case "$key" in
    ...
    Usage:
        --download [path]:  Downloads the binary installer of Fastnode and stores it at the given path. path defaults to ./fastnode-installer.
        --install [path]:   Installs Fastnode using the provided path to the binary installer. path defaults to ./fastnode-installer.
EOF
        exit 0
        ;;
    "--download")
...
"all")
    echo
    echo "This script will install Fastnode!"
    echo
    echo "We hope you enjoy! If you run into any issues, please report them at https://github.com/khulnasoft-lab/issue-tracker."
    echo
    echo "- The Fastnode Team"
    echo
    read -r -e -p "Press enter to continue..."

    if [ "$(id -u)" = "0" ]; then
        echo >&2
        echo >&2 "You're installing Fastnode as root."
    ...
    if [[ -z "$DISPLAY" && -z "$WAYLAND_DISPLAY" ]]; then
      ...
      echo "Fastnode Copilot UI won't be launched after the installation."
      echo "To login, run this command in a terminal:"
      echo -e "\t~/.local/share/fastnode/login-user"
      echo
      read -r -e -p "Press enter to continue..."
    else
      promptX11Dependencies
    fi
    downloadFastnodeInstaller "$downloadFile"
    installFastnode "$downloadFile"
    echo "Removing fastnode-installer"
    rm -f "$downloadFile"
    ;;
"download")
    downloadFastnodeInstaller "$downloadFile"
    downloadFastnodePackage "$downloadFile"
    ;;
"install")
    [ -f "$downloadFile" ] || downloadFastnodeInstaller "$downloadFile"
    installFastnode "$downloadFile" "--no-launch"
    ;;
esac
