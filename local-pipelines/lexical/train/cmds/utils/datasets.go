...
func LocalFilesDataset(group lexicalv0.LangGroup, content string) ([]string, error) {
	...
	for _, d := range dirs {
		err := filepath.Walk(d, func(path string, info os.FileInfo, err error) error {
			...
			// Python Filters
			if strings.Contains(path, "fastnode_ml/env") || strings.Contains(path, "fastnode_ml/tfenv") {
				return nil
			}
...
// LocalFilesKhulnasoft_lab returns a list of files located on disk that are compatible with the
// given language. localDataRoot is assumed to be the root of the khulnasoft-lab repo.
// TODO: kind of nasty, should we just upload a copy of khulnasoft-lab to s3?
func LocalFilesKhulnasoft_lab(group lexicalv0.LangGroup, localDataRoot string) ([]string, error) {
	if localDataRoot == "" {
		// try getting local data root from gopath
		localDataRoot = os.Getenv("GOPATH")
		if localDataRoot != "" {
			localDataRoot = filepath.Join(localDataRoot, "src/github.com/khulnasoft-lab/fastnode")
		}
	}
	...
	for _, l := range group.Langs {
		...
		case lang.Golang:
			dirs = []string{
				filepath.Join(localDataRoot, "fastnode-golib"),
				filepath.Join(localDataRoot, "fastnode-go"),
			}
		case lang.JavaScript:
			dirs = []string{
				filepath.Join(localDataRoot, "sidebar"),
				filepath.Join(localDataRoot, "fastnode-answers"),
				filepath.Join(localDataRoot, "web"),
			}
		case lang.Python:
			dirs = []string{
				filepath.Join(localDataRoot, "fastnode-python"),
			}
		}
	}
...
// given language. Default is khulnasoft-lab repo.
func LocalFiles(group lexicalv0.LangGroup, localDataRoot string) ([]string, error) {
	if localDataRoot == "" {
		return LocalFilesKhulnasoft_lab(group, localDataRoot)
	}
