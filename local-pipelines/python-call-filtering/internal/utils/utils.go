...
func Parse(src []byte) (*pythonast.Module, []pythonscanner.Word, error) {
	...
	err := fastnodectx.Background().WithTimeout(2*time.Second, func(ctx fastnodectx.Context) error {
		var err error
...
func Resolve(ast *pythonast.Module, rm pythonresource.Manager) (*pythonanalyzer.ResolvedAST, error) {
	...
	err := fastnodectx.Background().WithTimeout(1*time.Second, func(ctx fastnodectx.Context) error {
		var err error
...
func FindCalls(s Sample) ([]*pythonast.CallExpr, error) {
	...
	ast, err := pythonparser.ParseWords(fastnodectx.Background(), s.Source, words, parseOpts)
	if err != nil {
...
func ValueSupported(res Resources, val pythontype.Value) bool {
	for _, sym := range python.GetExternalSymbols(fastnodectx.Background(), res.RM, val) {
