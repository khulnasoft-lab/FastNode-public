...
func Extract(ctx fastnodectx.Context, rm pythonresource.Manager, maxPerSym int, src []byte, rast *pythonanalyzer.ResolvedAST) []Call {
...
func symbolsForFunc(ctx fastnodectx.Context, rm pythonresource.Manager, v pythontype.Value) []pythonpatterns.Symbol {
...
func symbolsForArgValue(ctx fastnodectx.Context, rm pythonresource.Manager, v pythontype.Value) []pythonpatterns.Symbol {
	var vs []pythontype.Value
	for _, elem := range pythontype.Disjuncts(ctx, v) {
