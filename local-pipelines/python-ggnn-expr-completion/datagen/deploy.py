...
def make_bundle(bundle: str, meta_info: str, fastnode_ml_path: str, env_vars: Dict[str, str]):
    ...
    run("go build -o {}/graph_data_server github.com/khulnasoft-lab/fastnode/fastnode-go/lang/python/cmds/graph-data-server".format(
        bundle
    ))
    run("cp -v {} {}/metainfo.json".format(meta_info, bundle))
    run("mkdir {}/fastnode_ml".format(bundle))
    run("cp -v {}/requirements.txt {}/fastnode_ml".format(fastnode_ml_path, bundle))
    run("cp -rv {}/fastnode {}/fastnode_ml".format(fastnode_ml_path, bundle))
...
def main():
    ...
    parser.add_argument('--fastnode_ml_path', type=str, help='path to Fastnode ML')
    ...
    env_vars = {
        ...
        'FASTNODE_AZUREUTIL_STORAGE_KEY': 'XXXXXXX',
        'FASTNODE_AZUREUTIL_STORAGE_NAME': 'fastnodes3mirror',
        'FASTNODE_USE_AZURE_MIRROR': '1',
    }

    make_bundle(args.bundle, args.meta_info, args.fastnode_ml_path, env_vars)
