...
echo "graph-data-server logs go to /var/fastnode/log/graph-data-server.log"
graph-data-server >> /var/fastnode/log/graph-data-server.log 2>&1 &
...
cd /var/fastnode/bundle/khulnasoft-lab/local-pipelines/python-ggnn-expr-completion
...
azure-cluster bundle $METAINFO_BUNDLE_FILE $TMP_DIR/metainfo-run.sh \
    ...
    --go-binary github.com/khulnasoft-lab/fastnode/fastnode-go/lang/python/cmds/graph-data-server \
	--go-binary github.com/khulnasoft-lab/fastnode/local-pipelines/python-ggnn-expr-completion/traindata \
	--go-binary github.com/khulnasoft-lab/fastnode/fastnode-go/lang/python/pythonexpr/cmds/convert \
    --khulnasoft-lab-path local-pipelines/python-ggnn-expr-completion/Makefile.cluster \
    --khulnasoft-lab-path local-pipelines/python-ggnn-expr-completion/packagelist.txt \
    --khulnasoft-lab-path local-pipelines/python-ggnn-expr-completion/packagelist-cluster1.txt \
    --khulnasoft-lab-path local-pipelines/python-ggnn-expr-completion/packagelist-cluster2.txt
...
azure-cluster bundle $DATAGEN_BUNDLE_FILE $TMP_DIR/datagen-run.sh --fastnode-ml \
    --go-binary github.com/khulnasoft-lab/fastnode/fastnode-golib/pipeline/cmds/rundb \
    --go-binary github.com/khulnasoft-lab/fastnode/fastnode-go/lang/python/cmds/graph-data-server \
    --khulnasoft-lab-path local-pipelines/python-ggnn-expr-completion/datagen/get_data.py \
    --khulnasoft-lab-path local-pipelines/python-ggnn-expr-completion/Makefile.cluster
...
echo "running sync-data in background; logs go to /var/fastnode/log/sync-data.log"
make -f Makefile.cluster sync_on_cluster >> /var/fastnode/log/sync-data.log 2>&1 &

echo "running train script; logs go to /var/fastnode/log/train.log"
make -f Makefile.cluster train_on_cluster >> /var/fastnode/log/train.log 2>&1
make -f Makefile.cluster RUNDB=$EXPR_RUNDB_ROOT/$SHARD_NAME validate_attribute >> /var/fastnode/log/validate-attribute.log 2>&1
make -f Makefile.cluster RUNDB=$EXPR_RUNDB_ROOT/$SHARD_NAME validate_calls >> /var/fastnode/log/validate-calls.log 2>&1
...
azure-cluster bundle $TRAIN_BUNDLE_FILE $TMP_DIR/train-run.sh --fastnode-ml --cuda \
    --go-binary github.com/khulnasoft-lab/fastnode/fastnode-golib/pipeline/cmds/rundb \
    --go-binary github.com/khulnasoft-lab/fastnode/fastnode-go/traindata/cmds/sync-data \
    --go-binary github.com/khulnasoft-lab/fastnode/local-pipelines/python-offline-metrics/cmds/ggnn-attribute-completions \
	--go-binary github.com/khulnasoft-lab/fastnode/local-pipelines/python-offline-metrics/cmds/ggnn-call-completions-validate \
    --khulnasoft-lab-path local-pipelines/python-ggnn-expr-completion/train/train.py \
    --khulnasoft-lab-path local-pipelines/python-ggnn-expr-completion/codebook/compress_embeddings.py \
    --khulnasoft-lab-path local-pipelines/python-ggnn-expr-completion/attr-fallback-hardcoded.txt \
    --khulnasoft-lab-path local-pipelines/python-ggnn-expr-completion/Makefile.cluster \
...
echo "  2) source /var/fastnode/bundle/env.sh"
echo "  3) source /var/fastnode/bundle/khulnasoft-lab/fastnode-python/fastnode_ml/venv/bin/activate"
echo "  4) tensorboard --logdir=/var/fastnode/bundle/khulnasoft-lab/local-pipelines/python-ggnn-expr-completion/tensorboard"
echo "  5) Enjoy!"
echo
