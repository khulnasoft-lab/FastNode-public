...
azure-cluster bundle $DATAGEN_BUNDLE_FILE $TMP_DIR/datagen-run.sh --fastnode-ml \
    ...
    --khulnasoft-lab-path local-pipelines/python-ggnn-expr-completion/Makefile \
    --khulnasoft-lab-path local-pipelines/python-ggnn-expr-completion/out/metainfo.json
...
cd /var/fastnode/bundle/khulnasoft-lab/local-pipelines/python-ggnn-expr-completion
make sync_on_cluster >> /var/fastnode/log/sync-data.log 2>&1 &

echo "running train script; logs go to /var/fastnode/log/train.log"
make train_on_cluster >> /var/fastnode/log/train.log 2>&1
EOF
...
azure-cluster bundle $TRAIN_BUNDLE_FILE $TMP_DIR/train-run.sh --fastnode-ml --cuda \
    ...
    --go-binary github.com/khulnasoft-lab/fastnode/fastnode-go/cmds/kfsput \
...
echo "In short:" 
echo "  ssh $TRAIN_HOST && source /var/fastnode/bundle/env.sh && source /var/fastnode/bundle/khulnasoft-lab/fastnode-python/fastnode_ml/venv/bin/activate && tensorboard --logdir=/var/fastnode/bundle/khulnasoft-lab/local-pipelines/python-ggnn-expr-completion/tensorboard"
