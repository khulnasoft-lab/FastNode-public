...
import (
	...
	"github.com/khulnasoft-lab/fastnode/local-pipelines/python-import-exploration/helpers"
)

// see khulnasoft-lab/python-import-exploration Dockerfile at fastnode-python/fastnode_pkgexploration/Dockerfile
var (
...
ENTRYPOINT ["/root/.pyenv/shims/python", "-m", "fastnode.pkgexploration", "{{.Name}}", "{{.Version}}", "{{.Out}}", "{{.Log}}"]
`))
)
