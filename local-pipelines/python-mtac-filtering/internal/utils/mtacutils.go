...
func TryName(src []byte, name *pythonast.NameExpr, rast *pythonanalyzer.ResolvedAST, global pythonproviders.Global) (NameInput, error) {
	...
	inputs, err := pythonproviders.NewInputs(fastnodectx.Background(), global, data.NewBuffer(string(src)).Select(data.Cursor(cursor)), false, false)
	if err != nil {
...
func Resolve(ast *pythonast.Module, rm pythonresource.Manager) (*pythonanalyzer.ResolvedAST, error) {
	...
	err := fastnodectx.Background().WithTimeout(5*time.Second, func(ctx fastnodectx.Context) error {
		var err error
...
func (i NameInput) TypeValueForName(ctx fastnodectx.Context, rm pythonresource.Manager, name string) (pythontype.Value, error) {
