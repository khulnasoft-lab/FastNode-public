...
import (
	...
	"github.com/khulnasoft-lab/fastnode/local-pipelines/python-mtac-filtering/internal/utils"
)
...
func getSortedScoreAndID(res utils.Resources, mip metricInputs) (revFloat32Slice, []threshold.MTACScenario, error) {
	mtacconfPred, err := res.Models.MTACConf.Infer(fastnodectx.Background(), mip.Input)
	if err != nil {
...
func findNames(s sample, res utils.Resources) ([]*pythonast.NameExpr, *pythonanalyzer.ResolvedAST, error) {
	ast, err := pythonparser.Parse(fastnodectx.Background(), s.Source, parseOpts)
	if err != nil {
...
func main() {
	...
	for scanner.Scan() {
		...
		for _, name := range names {
			...
			exprPred, err := res.Models.Expr.Predict(fastnodectx.Background(), exprInput)
			if err != nil {
			...
			pythongraph.Inspect(exprPred.OldPredictorResult, func(n *pythongraph.PredictionTreeNode) bool {
				...
				case n.AttrBase != "":
					...
					if val, err := inputs.TypeValueForName(fastnodectx.Background(), global.ResourceManager, n.AttrBase); err == nil {
						c.Referent = val
					}
					c.MixData = mtacconf.GetMixData(fastnodectx.Background(), global.ResourceManager, inputs.Selection, inputs.Words(), inputs.ResolvedAST(), inputs.Name)
					c.Score = prob
