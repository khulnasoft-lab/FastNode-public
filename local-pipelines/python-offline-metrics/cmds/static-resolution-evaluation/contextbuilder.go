...
func addFile(file *localfiles.File, ctx fastnodectx.Context, libraryPath string) *pythonbatch.SourceUnit {
	content, err := ioutil.ReadFile(file.Name)
	maybeQuit(err)

	var mod *pythonast.Module
	err = ctx.WithTimeout(pythonparser.ParseTimeout, func(ctx fastnodectx.Context) error {
...
func buildContext(targetProject *projectDescription, rm pythonresource.Manager) func() pythonstatic.Importer {
	...
	batch, err := batchManager.Build(fastnodectx.Background())
	maybeQuit(err)
