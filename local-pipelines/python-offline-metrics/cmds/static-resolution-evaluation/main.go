...
const (
	...
	s3Region                    = "us-west-1"
	s3ProjectPath               = "s3://fastnode-data/offline-processing/analysis-comparison/" + target + "-projects"
	s3RASTStoragePath           = "s3://fastnode-data/offline-processing/analysis-comparison/" + target + "-results/AST"
	s3JSONStoragePath           = "s3://fastnode-data/offline-processing/analysis-comparison/" + target + "-results/json"
...
func resolveWithContext(importer pythonstatic.Importer, parsed *pythonast.Module, filepath string) (*pythonanalyzer.ResolvedAST, error) {
	...
	err := fastnodectx.Background().WithTimeout(maxResolvingInterval, func(ctx fastnodectx.Context) error {
		var err error
...
func buildPipeline(project *projectDescription, rm pythonresource.Manager, outputFile string, importerFactory func() pythonstatic.Importer, astOutputFolder string, runDbOutput bool, maxRefPerProject int, execTimestamp string, samplingRate float64) pipeline.Pipeline {
	...
	pipe := pipeline.Pipeline{
		Name:    "fastnode-vs-intelliJ-analysis-comparison",
		Parents: pm,
