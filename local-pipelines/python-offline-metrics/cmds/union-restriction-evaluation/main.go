...
func compareLastUnionCompletions(rm pythonresource.Manager, models *pythonmodels.Models) func(s pipeline.Sample) pipeline.Sample {
	return func(s pipeline.Sample) pipeline.Sample {
		...
		inputs, err := pythonproviders.NewInputs(fastnodectx.TODO(), g, buf, false, false)
		if err != nil {
			return pipeline.CoerceError(err)
		}
		var completionsBefore, completionsAfter []pythonproviders.MetaCompletion

		err = pythonproviders.Attributes{UseDefaultReferences: true}.Provide(fastnodectx.Background(), g, inputs, func(ctx fastnodectx.Context, sb data.SelectedBuffer, mc pythonproviders.MetaCompletion) {
		...
		err = pythonproviders.Attributes{}.Provide(fastnodectx.Background(), g, inputs, func(ctx fastnodectx.Context, sb data.SelectedBuffer, mc pythonproviders.MetaCompletion) {
...
func main() {
	//repo := "git@github.com:khulnasoft-lab/etl.git"
