...
- (void)applicationDidFinishLaunching:(NSNotification *)aNotification {
    // If this is the enterprise version of FastnodeHelper, terminate
    ...
    // Initialize sparkle updater with Fastnode bundle
    NSBundle *bundle = [NSBundle bundleWithPath:menubarPath];
    self.updater = [SUUpdater updaterForBundle:bundle];

    // Note: These settings (automatically checks and automatically downloads) are shared with Fastnode.app as well.
...
- (void)applicationWillTerminate:(NSNotification *)aNotification {
    NSLog(@"terminating FastnodeHelper");
    if (self.updateInvocation != nil) {
        // If terminating to apply an update, terminate running instances of Fastnode.
        NSString *fastnodeBundle = [self bundleNameForApp:@"Fastnode"];
        ...
        for (NSRunningApplication *app in runningMenuItems) {
            NSLog(@"terminating running menubar: %@, %@", fastnodeBundle, app);
            [app terminate];
        }
        // Allow Sparkle enough time to update the Fastnode.app bundle in /Applications.
...
- (NSString *)feedURLStringForUpdater:(SUUpdater *)updater {
    // Since we set the bundle for the Sparkle updater to point to com.fastnode.Fastnode and since Sparkle stores values set by the set* functions in the user defaults based on the bundle, any settings created by the set* functions are shared between Fastnode and FastnodeHelper. We want the backup updates to check against the fallback endpoint, so we set the feed URL dynamically using this delegate method. Do NOT call setFeedURL as that will add SUFeedURL to the user's defaults for com.fastnode.Fastnode.
...
- (void)updater:(SUUpdater *)updater willInstallUpdateOnQuit:(SUAppcastItem *)item immediateInstallationInvocation:(NSInvocation *)invocation {
    ...
    // Invoke the update to relaunch Fastnode
