...
KHULNASOFT-LAB=os.path.join(GOPATH, "src/github.com/khulnasoft-lab/fastnode")

ARTIFACTS=os.path.join("/mnt/fastnode", "artifacts")
RELEASES="/var/fastnode/releases"
LOGDIR="/var/fastnode/log"

BUILD_TARGETS = {
    "user-node": "github.com/khulnasoft-lab/fastnode/fastnode-go/cmds/user-node",
    "user-mux": "github.com/khulnasoft-lab/fastnode/fastnode-go/cmds/user-mux",
}
    ...
    with shell_env(GOPATH=GOPATH, CGO_LDFLAGS_ALLOW='.*'):
        with cd(KHULNASOFT-LAB):
            ...
            for target, path in BUILD_TARGETS.items():
                ...
                run("s3cmd put %s s3://fastnode-deploys/%s/%s" % (
...
def build_release_server(branch):
    with shell_env(GOPATH=GOPATH, CGO_LDFLAGS_ALLOW='.*'):
        with cd(KHULNASOFT-LAB):
            ...
            target = 'release'
            go_pkg_path = 'github.com/khulnasoft-lab/fastnode/fastnode-go/cmds/release'
            ...
            run("s3cmd put %s s3://fastnode-deploys/%s/%s" % (artifacts_path, branch, target))
...
def deploy_release_server(branch):
    ...
    run('s3cmd get s3://fastnode-deploys/%s/release %s' % (branch, branch_dir))
