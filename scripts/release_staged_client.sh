...
# add release to prod
(
    eval $(ssh release2-vpn.khulnasoft.com 'bash --login -c "env | grep ^RELEASE_DB_DRIVER="' | sed 's/.*/export &/')
    eval $(ssh release2-vpn.khulnasoft.com 'bash --login -c "env | grep ^RELEASE_DB_URI="' | sed 's/.*/export &/')
    ./release add
)
set -e
