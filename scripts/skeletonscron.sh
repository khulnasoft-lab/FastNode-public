...
# Reminder, that this script is run on build.khulnasoft.com!
...
(
	...
	if [[ `hostname` != "build.khulnasoft.com" ]]; then
		echo "this is not build.khulnasoft.com. exiting."
		exit 1
	fi
	...
	echo "verifying khulnasoft-lab repo and pulling most recent version"
	cd $KHULNASOFT-LAB
	...
	cd $KHULNASOFT-LAB/local-pipelines/curation-python-skeletons
	...
	git commit fastnode-go/lang/python/pythonskeletons/bindata.go -m "[autocommit] updating curation python skeletons dump"  &> $LOGDIR/git_commit.log
