terraform {
  backend "s3" {
    ...
    workspace_key_prefix = "services/t.khulnasoft.com"
...
data "aws_subnet" "private1" {
  vpc_id = data.aws_vpc.fastnode_prod.id
  filter {
    name   = "tag:Name"
    values = ["az1-private"]
  }
...
data "aws_acm_certificate" "fastnode_com" {
...
resource "aws_lb_listener" "service" {
  ...
  certificate_arn = data.aws_acm_certificate.fastnode_com.arn
}
