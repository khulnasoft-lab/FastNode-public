...
var streams = map[string]bool{
	"fastnode_status":   true,
	"client_events": true,
	"fastnode_service":  true,
}
...
func (m *manager) handleStream(w http.ResponseWriter, r *http.Request) {
	...
	tag := fmt.Sprintf("fastnode_metrics.valid.%s", streamID)
	...
	if schema, ok := m.schemas[streamID]; ok {
		...
		if err != nil {
			...
			tag = fmt.Sprintf("fastnode_metrics.error.%s", streamID)
			...
			tag = fmt.Sprintf("fastnode_metrics.invalid.%s", streamID)
		}
	}
