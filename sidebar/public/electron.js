...
const { fastnodeProcess } = require('./utils/fastnode-process')
...
function handleArgs(args) {
  ...
  // <path to Fastnode.exe>,--allow-file-access-from-files,--original-process-start-time=<time>,fastnode://docs/python;;;;;json
  //
  ...
  var urls = args.filter(s => s.startsWith('fastnode://'))
  if (urls.length > 0) {
    openURL(urls[0])
    return
  }
...
async function startFastnode() {
  try {
    ...
    if ((await fastnodeProcess.isFastnodeRunning()).running) {
      await openSidebar()
      return
    }
    await fastnodeProcess.startFastnode()
    await openSidebar()
  } catch (err) {
    logger.error("unhandled error ", err)
    var message = `Error trying to run Fastnode Copilot. We have to exit`
    var detail = `Please open an issue at https://github.com/khulnasoft-lab/issue-tracker with the contents of ${logFilePath} for assistance.`
    if (err.type && err.type === 'tasklist_error') {
      // Display fix for spawn tasklist ENOENT errors on Windows.
      message = `Error trying to run Fastnode Copilot. Please ensure %SystemRoot%\system32 is in your PATH.`
...
function main() {
  ...
  app.on('activate', () => app.whenReady().then(startFastnode))
  ...
  if (process.env.REACT_APP_ENV === 'development' && process.platform === 'win32') {
    ...
    app.setAsDefaultProtocolClient('fastnode', process.execPath, [path.resolve(process.argv[1])])
  } else {
    app.setAsDefaultProtocolClient('fastnode')
  }
