...
function openSidebar() {
  if (!sidebarPromise) {
    sidebarPromise = new Promise(function (resolve, reject) {
      ...
      ipcMain.removeAllListeners('restart-fastnode')
      ipcMain.on('restart-fastnode', async (event) => {
        try {
          await fastnodeProcess.launchFastnode()
          await event.sender.send('restart-fastnode-success')
        } catch (err) {
          if (err === 'MOCK REJECTION') {
            event.sender.send('no-restart-support')
          } else {
            event.sender.send('restart-fastnode-error')
          }
        }
      })
      ...
      sidebar.fastnode_os = process.platform // linux, win32, darwin
