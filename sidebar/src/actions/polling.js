/**
 * For seeing what fastnoded's status is
 */
import { fastnodedStatusPath } from '../utils/urls'
import * as request from '../utils/fetch'
import { catchFastnodedNotReachable, catchFastnodedNotHealthy } from './fetch'

/**
 * Duplicated so as to not dispatch to reportFastnodedReachable action
 * as in fetch.js
 */
const wrapMethod = method => params => dispatch =>
  method(params)
    ...
    .then(res => {
      ...
      if(!request.isOK(res)) {
        return dispatch(catchFastnodedNotHealthy(res))
      }
      return res
    })
...
export const getFastnodedStatus = () => dispatch => {
  return dispatch(GET({ url: fastnodedStatusPath() }))
    .then((response) => {
      return response
    })
}
