...
import {
  ...
  proxyModePath,
  proxyURLPath,
  fastnodeServerURLPath,
  fastnodeServerStatusPath,
...
export const getHasSeenFastnodeLocalNotification = () => dispatch =>
  dispatch(GET({ url: hasSeenFastnodeLocalPath() }))
    .then(({ success, data }) => {
      if (success) {
        return data === 'true'
      }
...
export const setHasSeenFastnodeLocalNotification = hasSeen => dispatch =>
  dispatch(POST({
    url: hasSeenFastnodeLocalPath(),
    options: {
      body: hasSeen,
    },
  }))
...
export const GET_FASTNODE_SERVER_URL = 'get Fastnode server url'
export const getFastnodeServerURL = () => dispatch => (
  dispatch(GET({ url: fastnodeServerURLPath() }))
    .then(({ success, data, response }) => {
      if (success) {
        return dispatch({
          type: GET_FASTNODE_SERVER_URL,
          success,
          data: typeof data === 'string' ? data : '',
        })
      } else if (response) {
...
export const setFastnodeServerURL = url => dispatch => {
  return dispatch(POST({
    url: fastnodeServerURLPath(),
    options: {
      body: url,
    },
  })).then(({ success, error }) => {
    if (success) {
      return dispatch(getFastnodeServerURL())
    }
    throw error
  })
}

export const GET_FASTNODE_SERVER_STATUS = 'get Fastnode server status'
export const getFastnodeServerStatus = () => dispatch =>
  dispatch(GET({ url: fastnodeServerStatusPath() }))
    .then(({ success, data }) => {
      if (success) {
        return dispatch({
          type: GET_FASTNODE_SERVER_STATUS,
          success,
          data,
        })
      }
