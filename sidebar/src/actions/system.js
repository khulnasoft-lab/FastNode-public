...
export const GET_FASTNODED_READY = 'get fastnoded ready'
export const getFastnodedReady = () => dispatch =>
  dispatch(GET({ url: fastnodedReadyPath() }))
    .then(({ success, data }) => {
      if(success) {
        return dispatch({
          type: GET_FASTNODED_READY,
          success,
          fastnodedReady: data.online
        })
      }
    })
    ...
    dispatch(GET({ url: onlinePath() }))
      .then(({ success, data }) => {
        if(success) {
          ...
          //there was a response from fastnoded, as
