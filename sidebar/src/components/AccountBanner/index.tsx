...
interface AccountBannerProps {
  ...
  email: string
  fastnodeServerAvailable: boolean
...
class AccountBanner extends React.Component<AccountBannerProps, AccountBannerState> {
  ...
  helpAction() {
    shell.openExternal(`https://${Domains.Help}/article/128-fastnode-pro`)
  }
  ...
  computeUsagePaywallProRow(licenseInfo: LicenseInfo): IProRow | null {
    ...
    if (licenseInfo.product === Product.Free) {
      return {
        textField: {
          get: (onClick) => (
            <LearnMore onClick={onClick} body="You are using Fastnode Free – Upgrade now to Fastnode Pro"/>
          ),
      ...
      return {
        textField: {
          get: () => <span>You are using Fastnode Pro</span>,
          onClick: () => {},
        },
        button: {
          text: "Manage",
      ...
      case Product.Free:
        if (licenseInfo.trial_available_duration) {
          ...
          ret = {
            textField: {
              get: (onClick) => (
                <LearnMore onClick={onClick} body={`Try out Fastnode Pro free for ${value} ${pluralize(unit, value)}`}/>
              ),
      ...
      case Product.Pro:
        if (licenseInfo.plan === Plan.Trial) {
          ret = {
            textField: {
              get: (onClick) => (
                <LearnMore onClick={onClick} body={`You have ${licenseInfo.days_remaining} days left in your Fastnode Pro trial`}/>
              ),
  ...
  componentDidUpdate(prevProps: AccountBannerProps) {
    if (this.props.fastnodeServerAvailable !== prevProps.fastnodeServerAvailable) {
...
function Avatar(props: { email: string, useGravatar: boolean }) {
  ...
  return (
    <div className={classes}>
      <div className={styles.fastnodeIcon}/>
    </div>
  )
}
...
function mapStateToProps (state: any) {
  return {
    ...
    fastnodeServerAvailable: state.settings.fastnodeServerAvailable,
  }
}
