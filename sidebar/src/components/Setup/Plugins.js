  ...
  render() {
    ...
    return (
      <div className="setup__plugins__container">
        { !this.props.plugins &&
          <Spinner text="Detecting Fastnode support on your computer..."/>
        }
        { this.state.loading &&
        ...
        `}>
          <h2 className="setup__title">Fastnode Setup</h2>
          { this.state.error &&
          ...
          <div className="setup__plugins__list showup__animation">
            ...
            { installed_editors.length === 0 && uninstalled_editors.length === 0 && this.props.plugins &&
              <p>We were unable to find any editors on your system that Fastnode supports.</p>
            }
            { fully_installed_editors.length > 0 && <p>Fastnode is already integrated with the following editors on your system:
              &nbsp;{ fully_installed_editors.map(e => e.name).join(", ") }.</p>}
            <p>
              Fastnode also supports JupyterLab v2.2 and above. Learn how to <a className="plugins__more-info" href={`https://${Domains.Help}/article/143-how-to-install-the-jupyterlab-plugin`} target="_blank">integrate Fastnode with JupyterLab</a>.
            </p>
            { uninstalled_editors.length > 0 &&
              <p>Fastnode could not detect the following editors on your system:&nbsp;
            ...
            }
            {
              uninstalled_editors.some(e => e.id === 'spyder') && (
                <p>
                  If you installed Fastnode from Spyder directly, you can ignore
                  this message. Fastnode has automatically been integrated with
                  Spyder.
                </p>
              )
            }
