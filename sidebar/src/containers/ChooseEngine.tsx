...
import localIcon from '../assets/ic_fastnode_local.svg'
import cloudIcon from '../assets/ic_fastnode_cloud.svg'
    ...
    return <div className={`setup-main ${styles.chooseEngineMain}`}>
      ...
      <div className={`setup ${styles.setup__engine}`}>
        {!showDetails && <div>
          ...
          <p className="setup__text">Please select where Fastnode's completions are computed.</p>
          { localFirst ? <div>
        ...
        {showDetails && engine === "cloud" && <div>
          <h2 className="setup__title">Fastnode Cloud is currently in development</h2>
          <p className="setup__text">Fastnode Cloud is currently in development by our engineering team. Once we are
            live you will automatically get access to smarter completions with lower memory and CPU usage.</p>
          <p className="setup__text">In the meantime Fastnode will return completions from our local engine.</p>
        </div>}
...
function CloudButton(props: { onClick: () => void }) {
  const info = [
    "Sends your code to Fastnode's powerful servers where it remains private",
