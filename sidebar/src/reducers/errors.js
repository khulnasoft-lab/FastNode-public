...
const disconnected = (state, action) => {
  return {
    ...
    last: action,
    show: true,
    message: "Unable to connect to Fastnode",
    online: false,
    responsive: false,
...
const unresponsive = (state, action) => {
  return {
    ...
    last: action,
    show: true,
    message: "Fastnode is unresponsive",
    online: true,
    responsive: false,
    pollCount: state.pollCount + 1
  }
...
const errors = (state = defaultState, action) => {
  switch (action.type) {
    case fetch.REPORT_FASTNODED_UNREACHABLE:
      return disconnected(state, action)
    case fetch.REPORT_FASTNODED_REACHABLE:
      return connected(state, action)
    case fetch.REPORT_FASTNODED_UNHEALTHY:
