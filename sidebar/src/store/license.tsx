...
// Not to be mistaken with getConversionCohort, which only queries fastnoded and the local settings
...
export function startTrial(source: CTASource): ThunkAction<void, any, {}, Action> {
  return function(dispatch) {
    ...
    // desktoplogin, and redirect to alpha.khulnasoft.com/web/account/start-trial
...
function showAutostartTrialNotif(duration: TrialAvailableDuration): ThunkAction<void, any, {}, Action> {
  return function(dispatch) {
    dispatch(notif.notify({
      ...
      payload: {
        title: "Free Fastnode pro trial",
        text: `
          Fastnode Pro gives you ML-powered autocompletions for Python.
...
function showOptInTrialNotif(duration: TrialAvailableDuration): ThunkAction<void, any, {}, Action> {
  return function(dispatch) {
    ...
    dispatch(notif.notify({
      ...
      component: notif.NotifType.Default,
      payload: {
        title: "Start Your Free Fastnode Pro Trial",
        text: `
          Fastnode Pro gives you ML-powered autocompletions for Python.
          You can try it out for free for ${value} ${pluralize(unit, value)}.
          After the trial is over, you can continue to use the basic version of Fastnode for free.
        `,
        buttons: [{
          text: "Try Fastnode Pro for free",
...
function showUsagePaywallNotif(): ThunkAction<void, any, {}, Action> {
  return function(dispatch) {
    dispatch(notif.notify({
      ...
      component: notif.NotifType.Default,
      payload: {
        title: "Welcome to Fastnode Free",
        text: `
          Fastnode Free gives you a limited number of ★ completions to
          use per day. Upgrade to Fastnode Pro to code faster with
  ...
  return function(dispatch) {
    dispatch(notif.notify({
      ...
      payload: {
        ...
        text: `
          Fastnode will unlock with more completions tomorrow. You
          can also upgrade to Fastnode Pro now to get unlimited
          completions powered by machine learning.`,
        noDismiss: true,
        buttons: [{
          text: 'Upgrade to Fastnode Pro',
