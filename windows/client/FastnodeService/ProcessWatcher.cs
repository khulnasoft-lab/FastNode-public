...
namespace FastnodeService
{
    ...
    {
        private static readonly string k_reportCrashUrl = "https://alpha.khulnasoft.com/windowscrash?platform={0}&machineid={1}&filename={2}";
        ...
        internal static void Watch()
        {
            ...
            processStartEvent = new ManagementEventWatcher("SELECT * FROM Win32_ProcessStartTrace WHERE ProcessName = 'fastnoded.exe'");
            processStopEvent = new ManagementEventWatcher("SELECT * FROM Win32_ProcessStopTrace WHERE ProcessName = 'fastnoded.exe'");
        ...
        internal static void processStopEvent_EventArrived(object sender, EventArrivedEventArgs e)
        {
            try
            {
                ...
                if (exitStatus > 1)
                {
                    ...
                    string logPath = Path.Combine(appDir, "Fastnode");
