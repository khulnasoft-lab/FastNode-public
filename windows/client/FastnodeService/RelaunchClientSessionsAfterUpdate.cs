...
namespace FastnodeService {
    ...
    internal static class ClientAppSessions {
        ...
        private static List<uint> GetRunningClientAppSessionIDsOrderedByStartTime() {
            Process[] clientAppProcesses = Process.GetProcessesByName("fastnoded");
            ...
            for (int i = 0; i < clientAppProcesses.Length; i++) {
                try {
                    ...
                    Log.LogError("Exception trying to get fastnoded process start time", e);
        ...
        internal static void RememberClientAppSessionIDsForUpdate() {
            ...
            try {
                ...
                using (var regKey = Registry.LocalMachine.CreateSubKey(FastnodeService.PerInstallationRegistryPath)) {
        ...
        internal static void RestartAllClientAppSessionsFromUpdate() {
            try {
                // read session IDs to launch fastnoded in from the registry (and delete them from the registry)
                string[] sessionIDStrings;
                long sessionIDsTimestamp;
                using (var regKey = Registry.LocalMachine.OpenSubKey(FastnodeService.PerInstallationRegistryPath, true)) {
                    if (regKey == null) {
                        return;
                    }
                ...
                // check which session IDs, if any, have fastnoded already running
                var sessionIDsAlreadyRunningFastnoded = GetRunningClientAppSessionIDsOrderedByStartTime();

                // if the same user had multiple instances of fastnoded open, only relaunch one per user.
                ...
                foreach (var sessionIDString in sessionIDStrings) {
                    ...
                    if (sessionIDsAlreadyRunningFastnoded.Contains(sessionID)) {
                        continue;
                    }
        ...
        private static void RestartClientAppSessionFromUpdate(uint sessionID, long sessionIDTimestamp) {
            ...
            // try to find the path to the Fastnode client app
