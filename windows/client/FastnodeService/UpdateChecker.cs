...
namespace FastnodeService {
    ...
    internal static class UpdateChecker {
        ...
        private static readonly string k_updateUrl = "https://windows.khulnasoft.com/windows/fastnode-app/update-check?version={0}&machine-id={1}&will-attempt-update-if-provided={2}&user-wants-beta-builds={3}";
        private static readonly TimeSpan k_checkFrequency = TimeSpan.FromMinutes(5);

        private static readonly string k_fastnodedReadyToRestartUrl = "http://127.0.0.1:46624/clientapi/update/readyToRestart";
        private static readonly TimeSpan k_maxFastnodedWaitDuration = TimeSpan.FromMinutes(120);  // NOTE: 2 hours
        private static readonly TimeSpan k_fastnodedWaitPollInterval = TimeSpan.FromSeconds(30);  // NOTE: 30 seconds
                ...
                if (Monitor.TryEnter(k_checkForAndRunUpdatesLockObject)) {
                    try {
                        ...
                        var verifiedUpdateInfo = VerifiedUpdateInfo.FromUpdateXml(updateInfoXml,
                            FastnodeService.IsServiceDebugFlagSet());
                        ...
                        var downloadFilePath = Path.Combine(downloadFileDir, "FastnodeUpdater.exe");
                        ...
                        if (File.Exists(downloadFilePath)) {
                            ...
                            //     a) appreciable delays between downloading and executing the update (because of waiting on fastnoded),
                            //     b) short duration runs of FastnodeService (e.g. they restart their machine often), and
                        ...
                        // verify with fastnoded that it is "readyToRestart", i.e. it isn't in the middle of a UI-facing operation
                        //   or doing a batch file update.
                        // if fastnoded isn't running or returns an error then apply update immediately.
                        // if fastnoded returns "no" for too long then run the update anyway.
                        var fastnodedWaitDurationSoFar = TimeSpan.Zero;
                        while(fastnodedWaitDurationSoFar <= k_maxFastnodedWaitDuration && !IsFastnodedReadyToRestart()) {
                            Log.LogMessage("fastnoded is NOT ready to restart.  update is in holding pattern; trying again in a bit...");

                            fastnodedWaitDurationSoFar += k_fastnodedWaitPollInterval;
                            Thread.Sleep(k_fastnodedWaitPollInterval);
                        }
                        if(fastnodedWaitDurationSoFar > TimeSpan.Zero) {
                            // we waited on fastnoded to be "ready to restart"
                            // we're about to run the update
                            // re-check the updater exe hash
                            if (HashFile(downloadFilePath) != verifiedUpdateInfo.DownloadHash) {
                                throw new Exception("Updater executable hash is mismatched (after fastnoded readyToRestart step)");
                            }
                        }
                        Log.LogMessage(string.Format("fastnoded update ready-to-restart step complete (waited a total of {0})", 
                            fastnodedWaitDurationSoFar));

                        // we're about the run the updater.  if it sees fit it will TerminateProcess() any running fastnoded instances,
                        //   which could be many across multiple user accounts.  it can't restart them all, but FastnodeService can, so
                        //   before we run the updater remember which user account sessions are running fastnoded, so we know where to
        ...
        private static string AskServerForUpdate() {
            ...
            try {
                using (var regKey = Registry.LocalMachine.OpenSubKey(FastnodeService.PerInstallationRegistryPath)) {
                    if (regKey != null) {
        ...
        private static bool IsFastnodedReadyToRestart() {
            // for all error cases, return true

            var request = (HttpWebRequest)WebRequest.Create(k_fastnodedReadyToRestartUrl);
            request.KeepAlive = false;
            request.AllowAutoRedirect = false;
            request.Timeout = 10000;  // 10 seconds (the default is 100 seconds)

            // if there is a machine-wide/IE proxy, ignore it
            // note, we should honor the Proxy where applicable for non-localhost hosts like windows.khulnasoft.com
            ...
            try {
                using (var responseObject = GetResponseNoExceptionOnErrorStatusCode(request) as HttpWebResponse) {
                    if(responseObject == null) {
                        ...
                        //   HttpWebResponse.
                        Log.LogWarning("Fastnoded ready-to-restart check returned null; proceeding with update.");
                        return true;
                    }
                ...
                Log.LogWarning("Exception while checking fastnoded ready-to-restart; proceeding with update", e);
                return true;
            }
        }
        ...
        private static bool TryToDeletePreviousUpdater(string previousUpdaterFilePath) {
            ...
            //   e.g. UnauthorizedAccessException if file is in use and it's in C:\Program Files\Fastnode.
            ...
            try {
                ...
                int nKilled = 0;
                foreach (var runningUpdater in Process.GetProcessesByName("FastnodeUpdater")) {
                    runningUpdater.Kill();
                    try {
        ...
        private static bool IsImmediateUpdateCheckFlagSet() {
            try {
                using (var regKey = Registry.LocalMachine.OpenSubKey(@"Software\Fastnode", false)) {
                    if (regKey == null) {
                        return false;
                    }
        ...
        #region Fastnode update certificates
