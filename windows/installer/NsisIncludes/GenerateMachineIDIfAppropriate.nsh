...
Function ReadMachineIDOrGenerateIfAppropriate
    ...
    ;   64-bit HKLM
    ;   32-bit HKLM
	ReadRegStr $0 HKLM "Software\Fastnode" "MachineID"
	${If} $0 == ""
...
Function WriteTentativeOrActualMachineIDToRegistry
	...
	${If} $tentative_or_actual_machine_id == ""
		...
		MessageBox MB_OK "Installation failed due to empty MachineID.  Please visit https://github.com/khulnasoft-lab/issue-tracker to report this failure."
		Quit
	${EndIf}

	WriteRegStr HKLM "Software\Fastnode" "MachineID" "$tentative_or_actual_machine_id"

	; fastnoded, FastnodeService, and Fastnode.exe are 64 bit so also write it there
	SetRegView 32
	WriteRegStr HKLM "Software\Fastnode" "MachineID" "$tentative_or_actual_machine_id"
	SetRegView 64
FunctionEnd
