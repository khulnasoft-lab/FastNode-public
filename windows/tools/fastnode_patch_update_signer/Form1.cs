...
namespace FastnodeUpdateSigner {
    ...
    public partial class Form1 : Form {
        ...
        private void Form1_Load(object sender, EventArgs e) {
            ...
            // try to find the path to the latest build's FastnodeUpdater.exe, and also use that
            ...
            if (curPath.Length > 0) {
                ...
                var patchUpdaterFiles = Directory.GetFiles(Path.Combine(curPath, "builds", targetBuild), "FastnodePatchUpdater*.exe");
                ...
                prevRelease = patchUpdaterFilename.Substring("FastnodePatchUpdater".Length,
                                      patchUpdaterFilename.LastIndexOf("-") - "FastnodePatchUpdater".Length);
                Console.WriteLine(prevRelease);

                var patchUpdaterDownloadFilename = String.Format("FastnodePatchUpdater{0}.exe", prevRelease);
                ...
                txtOutFilePath.Text = Path.Combine(Path.GetDirectoryName(txtUpdateExecutableFilePath.Text),
                    string.Format("FastnodePatchUpdateInfo{0}.xml", prevRelease));
                txtDownloadUrl.Text = String.Format("https://windows.khulnasoft.com/windows/{0}/{1}", targetBuild, patchUpdaterDownloadFilename);
            } else {
            ...
            if(Environment.GetCommandLineArgs().Length > 1 && Environment.GetCommandLineArgs()[1] == "--sign-with-test") {
                ...
                txtOutFilePath.Text = Path.Combine(Path.GetDirectoryName(txtUpdateExecutableFilePath.Text),
                    String.Format("FastnodePatchUpdateInfo{0}-SignedWithTESTKey.xml", prevRelease));
                btnUseTestKey_Click(null, EventArgs.Empty);
