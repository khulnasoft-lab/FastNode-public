...
namespace FastnodeUpdateSignerCmd {
    ...
    static class Program {
        ...
        private static BuildParams buildParams()
        {
            ...
            bp.ExecutablePath = Path.Combine(curPath, @"builds\" + targetBuild + string.Format(@"\FastnodeUpdater{0}.exe", targetBuild));
            bp.XMLPath = Path.Combine(Path.GetDirectoryName(bp.ExecutablePath),
                "FastnodeUpdateInfo.xml");
            bp.DownloadURL = String.Format("https://windows.khulnasoft.com/windows/{0}/FastnodeUpdater.exe", targetBuild);
            return bp;
        }
